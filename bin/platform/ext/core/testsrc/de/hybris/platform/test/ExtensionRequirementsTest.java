...
public class ExtensionRequirementsTest extends HybrisJUnit4Test
{
	...
	public void testCorrectPlatformClasspathEntry() throws Exception
	{
		...
		for (final ExtensionInfo extension : includedExtensions)
		{
			...
			if (platformExtensions.contains(extension) || excludedExtensions.contains(
					extension) || extension.isExternalExtension())
			{
				continue;
			}
	...
	public void testIllegalWebExports() throws Exception
	{
		...
		for (final ExtensionInfo extension : includedExtensions)
		{
			...
			{
				for (final ClasspathEntry classpathEntry : EclipseClasspathReader.readClasspath(extension)
				                                                                 .getAllClasspathEntries())
				{
					if (classpathEntry.exported && classpathEntry.path.startsWith("web"))
					{
						illegalWebExports.append(
								extension + "/.classpath contains illegal web export : " + classpathEntry + "\n");
					}
				}
			}
		}
