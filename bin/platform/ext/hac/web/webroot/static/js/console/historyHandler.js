...
$(function(){
	...
	function getCookieSize(){
		...
		$.each(cookieHistory, function(name, value){
			cookieSize += calcsize(Cookies.get(name));
		});
	...
	function createCookie(name, content){
		Cookies.set(name, content, { expires: 365, path: cookiePath });
	}

	//delete cookie
	function deleteCookie(name){
		Cookies.set(name, "", { expires: -1, path: cookiePath });
	}
	...
	function getNewCookieName(){
		var i = 0;
		while(true){
			var cookie = Cookies.get(cookieName + i);
			if(!cookie){
				return cookieName+i;
			}
			i++;
	...
	function getAllCookies() {
        ...
        return Object.keys(allCookies)
            .filter(key => key.match("^" + cookieName))
            .reduce((obj, key) => {
	...
	function historyEntry(statement, additionalDescription, name){
        ...
        var em = $("<em class='small'>").text(additionalDescription);
		this.html = $("<li/>").append(this.linkExpand).append("<br/>").append(em).append(this.expandedDiv);
