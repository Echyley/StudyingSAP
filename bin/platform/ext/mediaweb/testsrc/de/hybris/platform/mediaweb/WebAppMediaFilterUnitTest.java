...
public class WebAppMediaFilterUnitTest
{
	...
	private static final String CONTEXT_PATH = "/testextension";
	...
	private static final String ALLOWED_REQUEST_URI_FROM_CLASSLOADER = String.format("%s/medias/fromjar/somepath/xxxx/file.png", CONTEXT_PATH);
	private static final String NOT_ALLOWED_REQUEST_URI_FROM_CLASSLOADER = String.format("%s/medias/fromjar/somepath/xxxx/file.foo", CONTEXT_PATH);
	...
	public void setUp() throws Exception
	{
		...
		given(httpRequest.getParameter("context"))
				.willReturn("bWFzdGVyfHJvb3R8MTIzNDV8aW1hZ2UvanBlZ3xhREF4TDJnd01pOW1iMjh1YW5Cbnxxd2VydHkxMjM0NQ");
	...
	{
		// given
		given(httpRequest.getRequestURI()).willReturn(NOT_ALLOWED_REQUEST_URI_FROM_CLASSLOADER);
		given(httpRequest.getContextPath()).willReturn(CONTEXT_PATH);
	...
	{
		// given
		given(httpRequest.getRequestURI()).willReturn(ALLOWED_REQUEST_URI_FROM_CLASSLOADER);
		given(httpRequest.getContextPath()).willReturn(CONTEXT_PATH);
		given(httpResponse.getOutputStream()).willReturn(servletOutputStream);
		given(config.getParameter(ALLOWED_EXTENSIONS_FOR_CLASSLOADER_KEY)).willReturn(ALLOWED_EXTENSIONS_FOR_CLASSLOADER);
	...
	{
		// given
		given(httpRequest.getRequestURI()).willReturn(String.format("%s/medias/realFilename.jpg", CONTEXT_PATH));
		given(httpRequest.getContextPath()).willReturn(CONTEXT_PATH);
		given(mediaMgr.getMediaAsStreamWithSize("root", "h01/h02/foo.jpg")).willThrow(
	...
	public void shouldSend403WhenRequestTriesToAccessSecuredFolderViaNonSecureMediaFilter() throws Exception
	{
		...
		given(httpRequest.getParameter("context"))
				.willReturn(
						"bWFzdGVyfHJvb3R8MTMzNTV8dGV4dC9wbGFpbnxjSEpwZG1GMFpXMWxaR2xoTDJnMk55OW9ZakF2T0RjNU5qRTFPRGsxTVRRMU5DNTBlSFF8LQ");
	...
	public void shouldServeMediaFileAsStreamWhenRequestingNormalResourceWithPrettyURL() throws Exception
	{
		// given
		enablePrettyUrl();
		given(httpRequest.getRequestURI())
				.willReturn(String.format("%s/medias/sys_master/folder/folderPath/h01/h02/123456789/pretty-url-needs-extra-work.jpg", CONTEXT_PATH));
		given(httpRequest.getContextPath()).willReturn(CONTEXT_PATH);
		setUpMocksBehaviorForStandardMediaResource();
	...
	public void shouldServeMediaFileAsStreamWhenRequestingNormalResourceWithPrettyURLFromFolderWithFancyName() throws Exception
	{
		...
		enablePrettyUrl();
		given(httpRequest.getRequestURI())
				.willReturn(String.format("%s/medias/sys_master/f-olde2_3r/folderPath/h01/h02/123456789/pretty-url-needs-extra-work.jpg", CONTEXT_PATH));
		given(httpRequest.getContextPath()).willReturn(CONTEXT_PATH);
		setUpMocksBehaviorForStandardMediaResource();
		given(mediaMgr.getMediaAsStreamWithSize("f-olde2_3r", "folderPath/h01/h02/123456789.jpg")).willReturn(
				inputStreamWithSize);
	...
	public void shouldServeMediaFileAsStreamWhenRequestingNormalResourceWithPrettyURLFromFancyFolderPath() throws Exception
	{
		...
		enablePrettyUrl();
		given(httpRequest.getRequestURI())
				.willReturn(String.format("%s/medias/sys_master/folder/_f_o-l2d3erPath_/h01/h02/123456789/pretty-url-needs-extra-work.jpg", CONTEXT_PATH));
		given(httpRequest.getContextPath()).willReturn(CONTEXT_PATH);
		setUpMocksBehaviorForStandardMediaResource();
		given(mediaMgr.getMediaAsStreamWithSize("folder", "_f_o-l2d3erPath_/h01/h02/123456789.jpg")).willReturn(
				inputStreamWithSize);
	...
	public void shouldServeMediaFileAsStreamWhenRequestingNormalResourceWithPrettyURLFromRootMediaFolder() throws Exception
	{
		// given
		enablePrettyUrl();
		given(httpRequest.getRequestURI()).willReturn(String.format("%s/medias/sys_master/root/h01/h02/123456789/pretty-url-needs-extra-work.jpg", CONTEXT_PATH));
		given(httpRequest.getContextPath()).willReturn(CONTEXT_PATH);
		setUpMocksBehaviorForStandardMediaResource();
	...
	public void shouldSend400ResponseWhenRequestingResourceHasMissingTenantPartForPrettyURL() throws Exception
	{
		// given
		enablePrettyUrl();
		given(httpRequest.getRequestURI()).willReturn(String.format("%s/medias/folderPath/some.jpg", CONTEXT_PATH));
		given(httpRequest.getContextPath()).willReturn(CONTEXT_PATH);
	...
	{
		// given
		given(httpRequest.getRequestURI()).willReturn(String.format("%s/medias.zul", CONTEXT_PATH));
		given(httpRequest.getContextPath()).willReturn(CONTEXT_PATH);
	...
	public void shouldSend400ResponseWhenRequestingResourceHasInvalidTenantIdPrefixForPrettyURL() throws Exception
	{
		...
		enablePrettyUrl();
		given(httpRequest.getRequestURI())
				.willReturn(String.format("%s/medias/wrong_tenant_id/root/h01/h02/123456789/pretty-url-needs-extra-work.jpg", CONTEXT_PATH));
		given(httpRequest.getContextPath()).willReturn(CONTEXT_PATH);
		setUpMocksBehaviorForStandardMediaResource();
	...
	{
		// given
		given(httpRequest.getRequestURI()).willReturn(String.format("%s/medias/sys_master/realFilename.xls", CONTEXT_PATH));
		given(httpRequest.getContextPath()).willReturn(CONTEXT_PATH);
		setUpMocksBehaviorForStandardMediaResource();
	...
	public void shouldSend304ResponseWhenRequestingResourceAndRequestContainMatchingETagHeader() throws Exception
	{
		...
		Mockito.lenient().when(config.getParameter("media.header.Last-Modified")).thenReturn(LAST_MODIFIED_DATE);
	...
	{
		// given
		given(httpRequest.getRequestURI()).willReturn(String.format("%s/medias/foobar.xls", CONTEXT_PATH));
		given(httpRequest.getContextPath()).willReturn(CONTEXT_PATH);
		setUpMocksBehaviorForStandardMediaResource();
	...
	{
		Mockito.lenient().when(config.getParameter("media.force.download.dialog.fileextensions")).thenReturn(
