...
public class MediaFilterTest
{
	...
	private OperationInfo info = null;
	...
	public void setUp() throws Exception
	{
		...
		configuredHeaders = new HashMap<String, String>();
		...
		given(httpRequest.getParameter("context")).willReturn(
				"bWFzdGVyfHJvb3R8MTIzNDV8aW1hZ2UvanBlZ3xhREF4TDJnd01pOW1iMjh1YW5Cbnxxd2VydHkxMjM0NQ");
			...
			throws Exception
	{
		...
		verify(servletOutputStream, times(1)).println(
				"not allowed to load media '/somepath/xxxx/file.foo' from classloader. Check parameter "
	...
	@Test
	public void shouldRegisterThread() throws Exception
	{
		// given
		...
		// when
		final String testMediaFilterRegisterThread = "TestMediaFilterRegisterThread";
		final Thread thread = new Thread(runnable, testMediaFilterRegisterThread);
		thread.start();
		latch.await(30, TimeUnit.SECONDS);

		// then
		assertThat(this.info.toString()).contains(testMediaFilterRegisterThread);
		assertThat(this.info.getCategory()).isEqualTo("WEB_REQUEST");
	}
	...
	public void shouldServeMediaFileAsStreamWhenRequestingNormalResourceWithPrettyURL() throws Exception
	{
		...
		given(httpRequest.getServletPath()).willReturn(
				"/sys_master/folder/folderPath/h01/h02/123456789/pretty-url-needs-extra-work.jpg");
		setUpMocksBehaviorForStandardMediaResource();
	...
	public void shouldServeMediaFileAsStreamWhenRequestingNormalResourceWithPrettyURLFromFolderWithFancyName() throws Exception
	{
		...
		enablePrettyUrl();
		given(httpRequest.getServletPath()).willReturn(
				"/sys_master/f-olde2_3r/folderPath/h01/h02/123456789/pretty-url-needs-extra-work.jpg");
		setUpMocksBehaviorForStandardMediaResource();
	...
	public void shouldServeMediaFileAsStreamWhenRequestingNormalResourceWithPrettyURLFromFancyFolderPath() throws Exception
	{
		...
		enablePrettyUrl();
		given(httpRequest.getServletPath()).willReturn(
				"/sys_master/folder/_f_o-l2d3erPath_/h01/h02/123456789/pretty-url-needs-extra-work.jpg");
		setUpMocksBehaviorForStandardMediaResource();
	...
	public void shouldServeMediaFileAsStreamWhenRequestingNormalResourceWithPrettyURLFromRootMediaFolder() throws Exception
	{
		// given
		enablePrettyUrl();
		given(httpRequest.getServletPath()).willReturn("/sys_master/root/h01/h02/123456789/pretty-url-needs-extra-work.jpg");
		setUpMocksBehaviorForStandardMediaResource();
	...
	public void shouldSend400ResponseWhenRequestingResourceHasInvalidTenantIdPrefixForPrettyURL() throws Exception
	{
		...
		given(httpRequest.getServletPath())
				.willReturn("/wrong_tenant_id/root/h01/h02/123456789/pretty-url-needs-extra-work.jpg");
		setUpMocksBehaviorForStandardMediaResource();
	...
	private void setUpMocksBehaviorForStandardMediaResource() throws IOException
	{
		...
		given(inputStreamWithSize.getSize()).willReturn(12345L);
	}
	...
	{
		verify(httpResponse, times(1)).setContentLengthLong(12345L);
		verify(httpResponse, times(1)).setHeader("Last-Modified", LAST_MODIFIED_DATE);
	...
	private class TestMediaFilter extends MediaFilter
	{
		...
		@Override
		protected void addContentType(final HttpServletResponse httpResponse, final MediaFilterLogicContext ctx)
		{
		...
		{
			// No needed for the test
		}

		@Override
		protected void unsetCurrentTenant()
		{
			MediaFilterTest.this.info = OperationInfo.current();
		}
