...
public abstract class BaseMediaFilterContentTypeTest extends ServicelayerBaseTest
{
	...
	@Test
	public void shouldReturnNameForMediaWithPipeInName() throws ServletException, IOException
	{
		final MediaModel media = createMediaModel("some||Name|.jpg");
		assertThat(media.getMime()).isEqualToIgnoringCase(IMAGE_JPEG_VALUE);

		final String url = media.getURL();
		assertThat(url).isNotEmpty().contains("context");

		final MockHttpServletRequest request = requestForMedia(media);
		tamperContextInMediaRequest(request, MediaFilterLogicContext.ContextPart.LOCATION, "");
	}
