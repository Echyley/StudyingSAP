...
public class DefaultCronJobServiceTest
{
	...
	public void testGetCronJobOneResult()
	{
		...
		Assert.assertEquals(
				"CronJobs list returned from service.getCronJob  should return first element of the  dao.findCronJob.",
	...
	public void testGetJobNullResult()
	{
		...
		when(jobMockDao.findJobs("someCode")).thenReturn(Collections.emptyList());
	...
	public void shouldFailWhenCronJobToPerformIsNotSaved() throws Exception
	{
		...
		given(modelServiceMock.isNew(cronJob)).willReturn(Boolean.TRUE);
	...
	public void shouldFailWhenCronJobToPerformIsSetToRunOnDifferentNodeAndClusteringIsDisabled() throws Exception
	{
		...
		final CronJobModel cronJob = new CronJobModel();
		cronJob.setNodeID(3);
		given(clusterService.isClusteringEnabled()).willReturn(Boolean.FALSE);
	...
	public void shouldScheduleCronJobToBeExecutedViaTaskEngine() throws Exception
	{
		// given
		final CronJobModel cronJob = new CronJobModel();
		cronJob.setNodeID(123456);
		given(modelServiceMock.create(TaskModel.class)).willReturn(task);
		given(clusterService.isClusteringEnabled()).willReturn(Boolean.TRUE);
	...
	public void testIsCronJobAbortable()
	{
		...
		when(cronJobMockDao.isAbortable(one)).thenReturn(Boolean.FALSE);

		Assert.assertFalse("Result from dao should be the same as from service", service.isAbortable(one));
	...
	public void testIsCronJobPerformable()
	{
		...
		when(cronJobMockDao.isPerformable(one)).thenReturn(Boolean.FALSE);

		Assert.assertFalse("Result from dao should be the same as from service", service.isPerformable(one));
	...
	public void testRequestAbortNotAvoided()
	{
		...
		when(cronJobMockDao.isAbortable(cronJobModel)).thenReturn(Boolean.TRUE);
	...
	public void testIsFinished()
	{
		...
		for (final CronJobResult result : CronJobResult.values())
		{
			for (final CronJobStatus status : CronJobStatus.values())
			{
				...
				{
					Assert.assertTrue(
							"Should only return finished as true  when status is FINISHED or ABORTED  (but was " + status
									+ " " + result + ")", service.isFinished(one));
				}
				else
				{
					Assert.assertFalse(
							"Should only return finished as false   when status is  not FINISHED or not ABORTED (but was "
	...
	public void testIsPaused()
	{
		...
		for (final CronJobResult result : CronJobResult.values())
		{
			for (final CronJobStatus status : CronJobStatus.values())
			{
				...
				{
					Assert.assertTrue(
							"Should only return pasued as true  when status is FINISHED, ABORTED  (but was " + status + " "
	...
	public void testGetLogTextInvalid()
	{
		...
		Mockito.lenient().when(jobLogDao.findJobLogs(one, -213, true)).thenReturn(oneResult);
			...
			for (final CronJobStatus status : CronJobStatus.values())
			{
				...
				if (status == CronJobStatus.FINISHED || status == CronJobStatus.ABORTED)
				{
					...
					else
					{
						...
						{
							Assert.assertFalse(
									"Should only return successfull as true  when result is SUCCESS  (but was " + status + " "
		...
		for (final CronJobResult result : CronJobResult.values())
		{
			for (final CronJobStatus status : CronJobStatus.values())
			{
				...
				if (status == CronJobStatus.FINISHED || status == CronJobStatus.ABORTED)
				{
					...
					{
						Assert.assertTrue(
								"Should only return error  as true  when result is ERROR or FAILURE   (but was " + status
	...
	{
		when(cronJobModel.getStatus()).thenReturn(status);
		when(cronJobModel.getJob()).thenReturn(new JobModel());
