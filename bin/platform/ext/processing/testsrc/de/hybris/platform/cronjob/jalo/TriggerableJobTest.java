...
public class TriggerableJobTest extends HybrisJUnit4Test
{
	...
	public void testCanAssignCronJobToTriggerWithAJob()
	{
		...
		catch (final Exception e)
		{
			// The exception should contain this string..
			assertTrue(
					"Expected " + CRONJOB_NOT_CHANGEABLE + " but got " + e.getMessage() + " : " + Utilities.getStackTraceAsString(
							e),
	...
	public void testCanAssignJobToTriggerWithACronJob()
	{
		...
		catch (final Exception e)
		{
			// The exception should contain this string..
			assertTrue(
					"Expected " + JOB_NOT_CHANGEABLE + " but got " + e.getMessage() + " : " + Utilities.getStackTraceAsString(e),
			...
			throws JaloDuplicateCodeException
	{
		...
		{
			job_ct = typeManager.createComposedType(typeManager.getComposedType(ServicelayerJob.class),
	...
	private Job prepareCustomJobTypes(final String jobName) throws Exception
	{
		...
		try
		{
			typeManager.getComposedType(CustomAttributesTriggerableJob.STATICCRONJOBCODE);
		}
		catch (final JaloItemNotFoundException e)
		{
		...
		try
		{
			type = typeManager.getComposedType(CustomAttributesTriggerableJob.STATICCJOBCODE);
		}
		catch (final JaloItemNotFoundException e)
		{
			type = typeManager.createComposedType(typeManager.getComposedType(Job.class),
	...
	public void testTriggerForNullAttributes() throws Exception
	{
		...
		final Trigger trigger = makeTrigger(Trigger.ACTIVE, Boolean.TRUE, Trigger.ACTIVATIONTIME, nowMinusTSeconds(1),
				Trigger.CRONJOB, cronJob, Trigger.ACTIVATIONTIME, null, Trigger.ACTIVE, null,
				Trigger.DATERANGE, null, Trigger.DAY,
				null, Trigger.DAYSOFWEEK, null, Trigger.HOUR,
				null, /* Trigger.MAXACCEPTABLEDELAY, null */
				Trigger.MINUTE, null, Trigger.MONTH, null, Trigger.RELATIVE, null, Trigger.SECOND,
				null, /*
				 * Trigger.TIMETABLE
				 * , null,
				 */
