...
public class SapBillingInvoiceClientImpl {
	...
	/**
	 * @deprecated since 2108, this method logic will be moved to
	 *             SapBillingInvoiceSalesStrategyImpl
	 */
	...
	/**
	 * @deprecated since 2108, this method logic will be moved to
	 *             SapBillingInvoiceServiceStrategyImpl in sapserviceorder extension
	 */
	@Deprecated(since = "2108", forRemoval = true)
	...
	public Map<String, Object> callS4forBillingDocuments(final SAPOrderModel sapOrder, final String targetSuffixUrl) {
		...
		if (!extBillingCpiConfigModel.getUrl().isEmpty()) {
			getOutboundServiceFacade()
					.send(extBillingCpiConfigModel, EXT_BIILING_OUTBOUND_OBJECT, EXT_BIILING_OUTBOUND_DESTINATION)
					.subscribe(responseEntityMap -> {

						if (isSentSuccessfully(responseEntityMap)) {
							LOG.info("Successful to get billing document form SCPI ");
	...
	public String getPropertyValue(final HttpEntity<Map> responseEntityMap, final String property) {
	...
	public byte[] getPDFData(final String billingDocumentId, final SAPOrderModel serviceOrderModelData) {
		...
		final SAPCpiOutboundConfigModel extBillingCpiConfigModel = getSAPCpiOutboundConfigModelForExtBilling(
				serviceOrderModelData, targetSuffixUrl, SapSystemType.SAP_S4HANA);
			...
			getOutboundServiceFacade()
					...
					.subscribe(responseEntityMap -> {
						...
						if (isSentSuccessfully(responseEntityMap)) {
							...
							final String billingDocumentBinaryAsString = pdfData.get("BillingDocumentBinary")
									.getAsString();
							parametersMap.put(DECODE_BYTES, Base64
									.decodeBase64(billingDocumentBinaryAsString.getBytes(StandardCharsets.UTF_8)));
