...
public class RepositoryRecording100 implements RepositoryRecording
{
	 ...
	 * Logger.
	 */
	static final Logger LOG = Logger.getLogger(RepositoryRecording100.class.getName());
	
	private static final int UPPER_TYPE_RANGE = 99;
	private static final int LOWER_TYPE_RANGE = 17;
	...
	private FunctionParameter populateFunction(final JCoFunction function, final int count)
	{
		...
		functionStub.setChanging(
				(ChangingParameterList) populateRecords(function.getChangingParameterList(), new ChangingParameterList()));
	...
	private void populateEmptyStructure(final JCoField field, final MetaStructure s)
	{
		...
		for (int i = 0; i < rmd.getFieldCount(); i++)
		{
			...
			if (rmd.getType(i) != UPPER_TYPE_RANGE && rmd.getType(i) != LOWER_TYPE_RANGE)
			{
				e.setFieldType(Utils100.fieldTypeFromInt(rmd.getType(i)));
			}
			else if (rmd.getType(i) == UPPER_TYPE_RANGE)
			{
				LOG.debug("!!! TABLE WITHOUT ROWS WITH TABLE !!!");
			}
			else if (rmd.getType(i) == LOWER_TYPE_RANGE)
			{
				LOG.debug("!!! TABLE WITHOUT ROWS WITH STRUCTURE !!!");
			}
	...
	public void writeRepositoryToFile() throws JCoRecException
	{
		...
		try(FileWriter fw= new FileWriter(repoFile))
		{
