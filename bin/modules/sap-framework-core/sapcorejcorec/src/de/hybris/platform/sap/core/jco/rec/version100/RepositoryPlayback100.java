...
{
	private static final int STANDARD_LENGTH_ZERO = 0;
	private static final int STANDARD_LENGTH_ONE = 1;
	private static final int STANDARD_LENGTH_TWO = 2;
	private static final int STANDARD_LENGTH_THREE = 3;
	private static final int STANDARD_LENGTH_FOUR = 4;
	private static final int STANDARD_LENGTH_SIX = 6;
	private static final int STANDARD_LENGTH_EIGHT = 8;
	private static final int STANDARD_LENGTH_SIXTEEN = 16;
	...
	private JCoFunction createAndPopulateFunction(final FunctionParameter fp)
	{
		...
		final JCoFunctionTemplate functionTemplate = createFunctionTemplateFromParameters(
				fp, existImport, existExport, existChanging, existTables);
	...
	private JCoFunctionTemplate createFunctionTemplateFromParameters(
			final FunctionParameter fp, final boolean existImport,
			final boolean existExport, final boolean existChanging,
			final boolean existTables) {
	...
	{
		final JCoRecordMetaData rmd = JCo.createRecordMetaData(e.getName());
		final int bl = getLength(e);
		final int decimals = e.getBCDDecimals() == null ? 0 : e.getBCDDecimals().intValue();
		lmd.add(e.getName(), Utils100.fieldTypeToInt(e.getFieldType()), bl, STANDARD_LENGTH_TWO * bl, decimals, " ", e.getDescription(),
				JCoListMetaData.CHANGING_PARAMETER, rmd, null);
	}
	...
	private void addStructureToListMetaData(final Structure s, final JCoListMetaData lmd)
	{
		...
		lmd.add(s.getName(), JCoMetaData.TYPE_STRUCTURE, bl, STANDARD_LENGTH_TWO * bl, 0, " ", s.getDescription(),
				JCoListMetaData.CHANGING_PARAMETER, rmd, null);	
	}
	...
	private void addTableToListMetaData(final Table t, final JCoListMetaData lmd)
	{
		...
		lmd.add(t.getName(), JCoMetaData.TYPE_TABLE, bl, STANDARD_LENGTH_TWO * bl, 0, " ", t.getDescription(), JCoListMetaData.OPTIONAL_PARAMETER,
				rmd, null);
	}
	...
	{
		final JCoRecordMetaData rmd = JCo.createRecordMetaData(tableName);
		int bo = 0;
			...
			final FieldType elementFieldType, final String elementDescription, final int bo, final JCoRecordMetaData rmd)
	{
		...
		rmd.add(elementName, Utils100.fieldTypeToInt(elementFieldType), bl, bo, STANDARD_LENGTH_TWO * bl, STANDARD_LENGTH_TWO * bo, decimals, elementDescription, null,
				null);
		return bl;
	}
			...
			final JCoRecordMetaData subRmd, final int subRmdLength, final JCoRecordMetaData rmd)
	{
		...
		rmd.add(structureName, JCoMetaData.TYPE_STRUCTURE, bl, bo, STANDARD_LENGTH_TWO * bl, STANDARD_LENGTH_TWO * bo, 0, structureDescription, subRmd, null);
		return bl;
	}
	...
	private int addTableToRecordMetaData(final Table t, final MetaTable mt, final int bo, final JCoRecordMetaData rmd)
	{
		...
		rmd.add(name, JCoMetaData.TYPE_TABLE, bl, bo, STANDARD_LENGTH_TWO * bl, STANDARD_LENGTH_TWO * bo, 0, description, subRmd, null);
		return bl;
	}
	...
	public JCoRecord getRecord(final String recordName) throws JCoRecException
	{
		...
		if (parts.length == STANDARD_LENGTH_THREE)
		{
			functionName = parts[0];
			parameterType = parts[1];
			splitRecordName = parts[STANDARD_LENGTH_TWO];

			newResult = findResult(functionName, parameterType,
                    splitRecordName);
			if(newResult!=null)
			{
			    checkResult=true;
    ...
    private JCoRecord findResult(String functionName,
            String parameterType, String splitRecordName)
            throws JCoRecException {
        JCoRecord result=null;
        for (final FunctionParameter fp : repo.getFunctions().getFunction())
        {
            if (fp.getFunctionName().equals(functionName))
            {           
                 result = getResultFromParameter(parameterType,
                        splitRecordName, fp);
                if (result != null)
                {
                    return result;
	...
	private JCoRecord getResultFromParameter(String parameterType, String splitRecordName,
			final FunctionParameter fp) throws JCoRecException {
		JCoRecord result;
	...
	{
		int standardLength;
		standardLength=getStandardLengthIfNumbers(type);
		if(standardLength==0)
		{
			switch (type)
			{
				case BCD:
					standardLength=Utils100.STANDARD_LENGTH_BCD;
					break;
				case BYTE:
					standardLength=1;
					break;
				case CHAR:
					standardLength=Utils100.STANDARD_LENGTH_SYMBOLCHAIN;
					break;
				case DATE:
					standardLength=STANDARD_LENGTH_EIGHT;
					break;
				case STRING:
					standardLength=Utils100.STANDARD_LENGTH_SYMBOLCHAIN;
					// in ABAP it will be initialized with Integer.valueOf(0)
					break;
				case TIME:
					standardLength=STANDARD_LENGTH_SIX;
					break;
				case XSTRING:
					standardLength=Utils100.STANDARD_LENGTH_SYMBOLCHAIN;
					// in ABAP it will be initialized with Integer.valueOf(0)
	...
	private int getStandardLengthIfNumbers(FieldType type) 
	{
		int standardLength;
		switch(type)
		{
			case DECF_16:
				...
				break;
			case DECF_34:
				standardLength=STANDARD_LENGTH_SIXTEEN;
				break;
			case FLOAT:
				standardLength=STANDARD_LENGTH_EIGHT;
				break;
			case INT:
				standardLength=STANDARD_LENGTH_FOUR;
				break;
			case INT_1:
				standardLength=STANDARD_LENGTH_ONE;
				break;
			case INT_2:
				standardLength=STANDARD_LENGTH_TWO;
				break;
			case NUM:
				standardLength=Utils100.STANDARD_LENGTH_SYMBOLCHAIN;
				break;
			default:
				standardLength=STANDARD_LENGTH_ZERO;
				break;
		}
		return standardLength;
	}
