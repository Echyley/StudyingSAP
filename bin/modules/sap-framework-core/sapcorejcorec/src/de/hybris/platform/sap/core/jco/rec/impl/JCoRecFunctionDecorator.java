 ...
 * {@link #decoratedFunction}, the execution against the XML repository is
 * implemented here.
 */
public class JCoRecFunctionDecorator implements JCoFunction, Cloneable {

    /**
     * Logger.
     */
    private static final Logger LOG = Logger
            .getLogger(JCoRecFunctionDecorator.class.getName());
    ...
    /**
     * The {@link JCoFunction} that is executed against the backend or taken
     ...
     */
    public JCoRecFunctionDecorator(final JCoFunction function,
            final int counter, final JCoRecMode mode) {
        this.decoratedFunction = function;
        this.counter = counter;
        this.mode = mode;
        this.functionKey = RecorderUtils.getFunctionKey(function.getName(),
                counter);
    }
    ...
    protected void setDecoratedFunction(final JCoFunction function) {
        if (function instanceof JCoRecFunctionDecorator) {
            decoratedFunction = ((JCoRecFunctionDecorator) function)
     ...
     */
    private void executeWrapper(final JCoDestination destination)
            throws JCoException {
     ...
     */
    @Override
    public void execute(final JCoDestination destination, final String tid)
            throws JCoException {
    ...
    @Override
    public void execute(final JCoDestination destination, final String tid,
            final String queueName) throws JCoException {
        throw new UnsupportedOperationException("This method is NOT supported!");
    }
     ...
     *         {@link RecorderUtils#getFunctionKey(String, int)} with the name
     *         of the decorated function and the local counter as parameter.
     */
    ...
    public boolean isAbapClassExceptionEnabled() {
        throw new JCoRecRuntimeException(
                "Method is not delegated due to a bug "
    ...
    public JCoRecFunctionDecorator clone() {
        JCoFunction clonnedDecoratedFunction = null;
        if (decoratedFunction != null) {
            try {
                super.clone();
                clonnedDecoratedFunction = decoratedFunction.clone();
            } catch (CloneNotSupportedException e) {
                LOG.info(e);
            }

        }
        return new JCoRecFunctionDecorator(clonnedDecoratedFunction, counter,
                mode);
    }
