...
{
	private static final int FUNCTION_KEY_PARTS = 2;
    ...
    private void checkMetaDataRepository(final JCoRecRepository repo)
            throws JCoRecXMLParserException {
	...
	private void parseRecords(final NodeList recordsList, final JCoRecRepository repo) throws JCoRecXMLParserException
	{
		for (int i = 0; i < recordsList.getLength(); i++)
		{
			...
			if(record!=null)
			{
			    ...
			    try
			    {
			        createNewRecordAndAddToRepo(repo, record, recordType, recordKey,
                            emptyStructOrTable);
    ...
    private void createNewRecordAndAddToRepo(final JCoRecRepository repo,
            final Node record, final String recordType, final String recordKey,
            String emptyStructOrTable) throws JCoException,
	...
	private void parseFunctions(final NodeList list, final JCoRecRepository repo) throws JCoRecXMLParserException
	{
		for (int i = 0; i < list.getLength(); i++)
		{
			...
			throwMissingAttributeException(rfc, functionNameNode,
                    functionKeyNode);

			functionName = functionNameNode.getNodeValue();
			functionKey = functionKeyNode.getNodeValue();

			JCoFunction newFunction = getNewFunction(repo,
                    functionName);
			...
			for (int j = 0; j < rfcChildren.getLength(); j++)
			{
				...
				{
					parseJCoRecord(rfcChild, newFunction.getImportParameterList());
				}
				else if (rfcChild.getNodeName().equalsIgnoreCase(Utils000.TAGNAME_EXPORT_PARAM))
				{
					parseJCoRecord(rfcChild, newFunction.getExportParameterList());
				}
				else if (rfcChild.getNodeName().equalsIgnoreCase(Utils000.TAGNAME_TABLE_PARAM))
				{

					parseJCoRecord(rfcChild, newFunction.getTableParameterList());
				}
				else if (rfcChild.getNodeName().equalsIgnoreCase(Utils000.TAGNAME_CHANGING_PARAM))
				{

					parseJCoRecord(rfcChild, newFunction.getChangingParameterList());
				}
			...
			repo.put(newFunction, counter);
		}

	}

    private JCoFunction getNewFunction(
            final JCoRecRepository repo, String functionName)
            throws JCoRecXMLParserException {
        JCoFunction newFunc;
        try
        {
    ...
    private void throwMissingAttributeException(final Node rfc,
            final Node functionNameNode, final Node functionKeyNode)
            throws JCoRecXMLParserException {
        if (functionKeyNode == null)
        {
        	throw new JCoRecXMLParserException(
        			"Missing Attribute (" + Utils000.ATTNAME_FUNCTION_KEY + ") in " + rfc.getNodeName());
        }
        if (functionNameNode == null)
        {
        	throw new JCoRecXMLParserException(
        			"Missing Attribute (" + Utils000.ATTNAME_FUNCTION_NAME + ") in " + rfc.getNodeName());
        }
    }
	...
	private int convertFunctionKeyToExecutionOrder(final String functionKey) throws JCoRecXMLParserException
	{
		...
		if (parts.length < FUNCTION_KEY_PARTS)
		{
	...
	private void parseJCoRecord(final Node child, final JCoRecord record) throws JCoRecXMLParserException
	{
		...
		for (int i = 0; i < childChildren.getLength(); i++)
		{
			...
			throwXMLParserExceptionFromParseJCoRecord(currChild);
			...
			else if (isFirstChildTextNode)
			{
				...
				setValueOfRecordByType(record, currChildTagName, set, type);
			}
			else if (hasItemTag)
			{
    ...
    private void throwXMLParserExceptionFromParseJCoRecord(final Node currChild)
            throws JCoRecXMLParserException {
    ...
    private void setValueOfRecordByType(final JCoRecord record,
            final String currChildTagName, String set, final int type)
            throws JCoRecXMLParserException {
        switch (type)
        {
        	...
        	case JCoMetaData.TYPE_BCD:
        		String newSet = handleBCD(set);
        		record.setValue(currChildTagName, newSet);
        		break;
        	default:
