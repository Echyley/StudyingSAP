...
public class SAPConfigurationServiceImpl implements SAPConfigurationService
{
	...
	public String getBackendType()
	{
		...
		if (backendType == null || backendType.isEmpty())
		{
			throw new ConfigurationRuntimeException(
					"No Backend type is defined for RFC Destination '" + getRFCDestination().getRfcDestinationName() + "'!");
		}
		return backendType;
	}
	...
	private ConfigurationPropertyAccess getBaseStoreCPA()
	{
		...
		this.unload(threadLocalBaseStoreConfigurationPropertyAccess);
		if (cpa == null)
		{
	...
	private ConfigurationPropertyAccess getBaseStoreCPAInternal()
	{
		...
		ConfigurationPropertyAccessImpl configurationPropertyAccess = (ConfigurationPropertyAccessImpl) threadLocalBaseStoreConfigurationPropertyAccess
				.get();
		if (configurationPropertyAccess != null && !configurationPropertyAccess.getConfigurationModel().equals(currentBaseStore))
		{
	...
	/**
	 * Cleanup threadloal value
	 *
	 * @param threadLocal
	 */
	private void unload(ThreadLocal threadLocal)
	{
		threadLocal.remove();
	}
