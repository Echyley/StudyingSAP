...
public class DataHubTransferHandler
{
	...
	private void prepareForDataHubDeleteItem(final String code, List<ItemModel> models,
	                                         final DataHubTransferLog dataHubTransferLog,
	                                         final DataHubTransferConfiguration dataHubTransferConfiguration)
	{
		for (final ItemModel model : models)
        {
			...
			if (propertyMap == null)
			{
				final String logMessage = Localization.getLocalizedString("dataTransfer.dataHub.invokeDeleteItem.exception",
						new Object[]{ model.toString() });
				dataHubTransferLog.addLog(logMessage);
				dataHubTransferLog.setTransferException(new DataHubTransferException(logMessage));
				LOGGER.warn("{}: {}", code, logMessage);
				return;
			}
            try
