...
/**
 * This is the extension manager of the Sapcoreconfiguration extension.
 */
public class SapcoreconfigurationManager extends GeneratedSapcoreconfigurationManager
{
	/** Edit the local|project.properties to change logging behavior (properties 'log4j.*'). */
	private static final Logger LOG = Logger.getLogger(SapcoreconfigurationManager.class.getName());
	 ...
	 */
	public static SapcoreconfigurationManager getInstance()
	{
		return (SapcoreconfigurationManager) Registry.getCurrentTenant().getJaloConnection().getExtensionManager().getExtension(
				SapcoreconfigurationConstants.EXTENSIONNAME);
	}
	 ...
	 */
	public SapcoreconfigurationManager() 
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("constructor of SapcoreconfigurationManager called.");
		}
	}
	...
	public void init()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("init() of SapcoreconfigurationManager called. " + getTenant().getTenantID());
		}
	}
	...
	public void destroy()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("destroy() of SapcoreconfigurationManager called, current tenant: " + getTenant().getTenantID());
		}
	}
