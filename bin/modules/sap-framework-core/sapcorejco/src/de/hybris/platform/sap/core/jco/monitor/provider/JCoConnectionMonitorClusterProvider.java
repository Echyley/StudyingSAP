 ...
 */
public class JCoConnectionMonitorClusterProvider extends
        JCoConnectionMonitorProvider {

    private static final Logger LOG = Logger
            .getLogger(JCoConnectionMonitorClusterProvider.class.getName());
     ...
     */
    public void initializeCache(final Map<Integer, String> snapshotsPerNode,
            final long snapshotTimestamp) throws JCoMonitorException {
        reset();
    ...
    /**
     * Returns the cached result for number of JCo connections whose lifetime
     * exceed a specific threshold parameter.
     * 
     * @return number of JCo connections whose lifetime exceed the defined
     *         threshold
     */
    ...
    /**
     * Returns the cached result for number of RFC destinations whose pool size
     * is reached.
     * 
     ...
     *             {@link JCoMonitorException}
     */
    public String getSnapshotXML(final Map<Integer, String> snapshotsPerNode,
            final long snapshotTimestamp) throws JCoMonitorException {
     ...
     *             {@link JCoMonitorException}
     */
    public File createSnapshotFile(final Map<Integer, String> snapshotsPerNode,
            final long snapshotTimestamp) throws JCoMonitorException {
        String statusMessage = "";
        final File snapshotFile=createSnapshotFile(
              SapcorejcoConstants.JCO_CONNECTION_CLUSTERS_SNAPSHOT_FILE_NAME,
        ...
        try( FileOutputStream os = new FileOutputStream(snapshotFile)) {
            ...
            statusMessage = "Error when creating file output stream for snapshot file: "
                    + e.getMessage();
     ...
     *             {@link JCoMonitorException}
     */
    protected void createSnapshotCache(
            final Map<Integer, String> snapshotsPerNode,
            final long cacheTimestamp) throws JCoMonitorException {
        ...
        for (final NodeType node : nodeList) {
            ...
            if (node.getJcoConnections() != null) {
                calculateLongRunnerCount(node);
            }
        }
        final Map<String, List<JcoConnectionType>> rfcDestinationJCoConnectionsMap = getJCoConnectionByRFCDestinationMap(nodes);
        for (final String rfcDestinationName : rfcDestinationJCoConnectionsMap
                .keySet()) {
            final RFCDestination rfcDestination = rfcDestinationService
                    .getRFCDestination(rfcDestinationName);
            final int poolSize = Integer.parseInt(rfcDestination.getPoolSize());
            final List<JcoConnectionType> jCoconnectionList = rfcDestinationJCoConnectionsMap
                    .get(rfcDestinationName);
    ...
    private void calculateLongRunnerCount(NodeType node) {

        totalCount += node.getJcoConnections().getJcoConnection().size();
        final List<JcoConnectionType> jcoConnectionList = node
                .getJcoConnections().getJcoConnection();
        for (final JcoConnectionType jcoConnection : jcoConnectionList) {
            if (isLongLifetimeJCoConnection(cacheTimestamp, jcoConnection
                    .getLastActivityTimestamp().longValue(),
     ...
     */
    protected Map<String, List<JcoConnectionType>> getJCoConnectionByRFCDestinationMap(
            final Nodes nodes) {
        final Map<String, List<JcoConnectionType>> rfcDestinationMap = new HashMap<String, List<JcoConnectionType>>();
        final List<NodeType> nodeList = nodes.getNode();
        for (final NodeType node : nodeList) {
            if (node.getJcoConnections() != null) {
                getRFCDestinationMapByIteration(node, rfcDestinationMap);
            }
        }
        return rfcDestinationMap;
    }

    private void getRFCDestinationMapByIteration(NodeType node,
            Map<String, List<JcoConnectionType>> rfcDestinationMap) {
        final List<JcoConnectionType> jcoConnectionList = node
                .getJcoConnections().getJcoConnection();
        for (final JcoConnectionType jcoConnection : jcoConnectionList) {
            final String rfcDestinationName = jcoConnection
                    .getRfcDestinationName();
            List<JcoConnectionType> jCoConnectionList = rfcDestinationMap
                    .get(rfcDestinationName);
    ...
    protected void safeClose(final OutputStream resource) {
        if (resource != null) {
            try {
                ...
                LOG.error(
                        "Error when closing the output stream of the snapshot file! "
                                + ex.getMessage(), ex);
            }
        }
    }
