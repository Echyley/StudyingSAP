...
public class JCoManagedFunction implements JCoFunction, Cloneable {
    /**
     * Delegator instance.
     */
    ...
    private static final Logger LOG = Logger.getLogger(JCoManagedFunction.class
            .getName());
     ...
     *            connection parameters
     */
    public JCoManagedFunction(final JCoFunction delegate,
            final JCoConnectionParameter connectionParameter) {
        super();
    ...
    public String getName() {
        if (hasConnectionParameters()) {
            ...
            if (p.getFunctionModuleToBeReplaced() != null
                    && !p.getFunctionModuleToBeReplaced().isEmpty()) {
    ...
    public JCoFunction clone() {
        JCoFunction clonedDelegate = null;

        if (delegate != null) {
            try {
                super.clone();
                clonedDelegate = delegate.clone();
            } catch (CloneNotSupportedException e) {
                LOG.info(e);
            }
        }
        // We dont need to clone the connection parameter as this is singleton

        return new JCoManagedFunction(clonedDelegate, connectionParameter);
    }
