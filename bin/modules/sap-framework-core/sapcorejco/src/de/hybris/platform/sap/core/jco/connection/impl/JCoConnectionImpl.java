...
public class JCoConnectionImpl implements JCoConnection
{
	...
	public JCoFunction getFunction(String functionName) throws BackendException
	{
		...
		String replacedFunctionName = functionName;
		...
		if (connectionParameters != null && connectionParameters.isFunctionModuleConfigured(functionName))
		{
			...
			replacedFunctionName = getReplacedFunctionModuleName(functionName);
		}

		try
		{
			function = getDestination().getRepository().getFunction(replacedFunctionName);
		}
	...
	{
	    String replacedFunctionModuleName = functionName;
		...
		if (connectionParameter.getFunctionModuleToBeReplaced() != null)
		{
			...
			replacedFunctionModuleName = connectionParameter.getFunctionModuleToBeReplaced();
		}
		return replacedFunctionModuleName;
	}
	...
	protected JCoCustomDestination getDestination()
	{
		if (destination == null)
		{
			...
			try
			{
				...
				getDestinationAndSetLanguageInCustomDestination(destinationName);
			}
    ...
    private void getDestinationAndSetLanguageInCustomDestination(final String destinationName) throws BackendException {
        try
        {
            final String scopeTypeProperty = properties.getProperty(JCoConnection.JCO_SCOPE_TYPE, getScopeType());
        	destination = JCoDestinationManager.getDestination(destinationName, scopeTypeProperty).createCustomDestination();
        	log.debug("JCoCustomDestination has been created for parent destination " + destinationName
        			+ " and with scopeType " + scopeTypeProperty);
