...
public class JCoManagedFunctionCache {
    ...
    /**
     * Parameter value for connection parameter cacheType which indicates that
     * this function should be cached. Parameter is set in spring definition of
    ...
    private static final Logger LOG = Logger
            .getLogger(JCoManagedFunctionCache.class.getName());
    ...
    public JCoManagedFunctionCache() {
        jcoCacheRegion = GenericFactoryProvider.getInstance().getBean(
                "sapCoreConnectionCacheRegion");
    }
    ...
    public boolean isFunctionCached(final JCoManagedFunction function) {
        ...
        if (!function.getConnectionParameter().getCacheType()
                .equalsIgnoreCase(DEFAULT_CACHE_TYPE)) {
            return false;
        }
     ...
     * First call gets the values from the backend and stores the result in the
     * cache. Subsequent calls are served from the cache.
     * 
     * @param destination
     *            destination.
     * @param function
    ...
    public void executeCachedFunction(final JCoDestination destination,
            final JCoManagedFunction function) throws BackendException {
        if (jcoCacheRegion == null) {
        ...
        try {
            ...
            LOG.debug("Serving Function Module \"" + function.getName()
                    + "\" from cache");

            final JCoFunctionAttr jCoFunctionAttr = new JCoFunctionLoader.JCoFunctionAttr(
                    destination, function);
            final JCoFunctionLoader jcoFuncLoader = new JCoFunctionLoader();
            jcoFuncLoader.setFuncAttr(jCoFunctionAttr);

            final JCoFunction cachedFunc = (JCoFunction) jcoCacheRegion
                    .getWithLoader(cacheKey, jcoFuncLoader);

            Iterator<JCoField> iterator;
            final JCoParameterList expParam = cachedFunc
            ...
            final JCoParameterList tabParam = cachedFunc
            ...
            LOG.log(Priority.ERROR, "Error during execution JCoFunction "
                    + errMsg);
     ...
     * The key is constructed using the function name, the language and a
     * concatenation of the import parameters and values.
     * 
     * @param destination
     *            destination
     * @param function
     ...
     *             Exception
     */
    private CacheKey getJCoFunctionKey(final JCoDestination destination,
            final JCoManagedFunction function) throws JCoException {
        final StringBuilder key = new StringBuilder(destination.getAttributes()
                .getClient());
        ...
        if (function.getConnectionParameter().getFunctionModuleToBeReplaced() != null) {
            key.append("[R]->").append(
                    function.getConnectionParameter().getFunctionModule());
        }
