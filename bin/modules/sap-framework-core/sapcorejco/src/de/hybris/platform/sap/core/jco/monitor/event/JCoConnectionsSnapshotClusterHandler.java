 ...
 */
public class JCoConnectionsSnapshotClusterHandler extends
        AbstractEventListener<JCoConnectionsSnapshotClusterResultEvent>
        implements JCoConnectionMonitor {

    private static final Logger LOG = Logger
            .getLogger(JCoConnectionsSnapshotClusterHandler.class.getName());
     ...
     */
    @Required
    public void setCollectionTimeoutInSeconds(
            final long collectionTimeoutInSeconds) {
    ...
    /**
     * Check current state of snapshot in buffer and refreshes it if lifetime is
     * outdated.
     */
    protected void checkState() {
        synchronized (this) {
            if (clusterProvider.getCacheTimestamp() == null
                    || System.currentTimeMillis() > (clusterProvider
                            .getCacheTimestamp() + cacheLifetimeInMSecs)) {
    ...
    /**
     * Initiates the creation of one jco connections snapshot file for all
     * cluster.
     * 
     * @param withFile
     ...
     */
    protected synchronized String createClusterSnapshot(final boolean withFile,
        ...
        final PingBroadcastHandler pingHandler = PingBroadcastHandler
                .getInstance();
        ...
        eventService.publishEvent(new JCoConnectionsSnapshotClusterEvent(
                currentSnapshotUuid));
        ...
        while ((clusterIdsWaitingForResult != null && !clusterIdsWaitingForResult
                .isEmpty())
                && (System.currentTimeMillis() < snapshotStartTimestamp
                        + collectionTimeoutInMSecs)) {
            try {
                wait(1000);
            } catch (final InterruptedException e) {
                final String errorMsg = "Error when waiting for cluster result: "
                        + e.getMessage();
                LOG.error(errorMsg, e);
                return errorMsg;
            }
        }
        return createSnapshotFile(withFile, updateCache);
    }
     ...
     * @return
     */
    private String createSnapshotFile(final boolean withFile,
            final boolean updateCache) {
        try {
            final Long snapshotTimestamp = System.currentTimeMillis();
            finalizeSnapshot(currentSnapshotUuid);
            if (withFile) {
                final File snapshotFile = clusterProvider.createSnapshotFile(
                        nodeResultMap, snapshotTimestamp);
                return "Snapshot file " + snapshotFile.getAbsolutePath()
                        + " has successfully been generated!";
            } else {
                if (updateCache) {
                    clusterProvider.initializeCache(nodeResultMap,
                            snapshotTimestamp);
                    return null;
                } else {
                    return clusterProvider.getSnapshotXML(nodeResultMap,
                            snapshotTimestamp);
                }
            }
        } catch (final JCoMonitorException e) {
            final String errorMsg = "Error when generating cluster snapshot: "
                    + e.getMessage();
            LOG.error(errorMsg, e);
            return errorMsg;
        } finally {
     ...
     */
    protected void finalizeSnapshot(final String snapshotUuid)
            throws JCoMonitorException {
        if (currentSnapshotUuid == null
                || !currentSnapshotUuid.equals(snapshotUuid)) {
            return;
        }
        synchronized (this) {
            if (currentSnapshotUuid == null
    ...
    protected void onEvent(final JCoConnectionsSnapshotClusterResultEvent event) {
        if (event.getSnapshotUuid().equals(currentSnapshotUuid)) {
            LOG.info("Receiving jco connections snapshot result from node with cluster id "
                    + event.getClusterId());
            ...
            if (!event.hasFailed()) {
                ...
                LOG.error(
                        "Snapshot result of node with cluster id "
                                + event.getClusterId() + " has failed: "
