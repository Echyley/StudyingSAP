...
public class RequestSequencerFilter extends GenericFilterBean
{
	 ...
	 */
	protected static String SESSION_REQUEST_QUEUE_COUNTER_ATTRIB = RequestSequencerFilter.class.getName() + ".RequestQueueCounter";

	/** Name of the session attribute, used to sync requests for the same session. */
	protected static String SESSION_REQUEST_SYNC_OBJECT = RequestSequencerFilter.class.getName() + ".RequestSyncObject";
	...
	private boolean matchesUrlRegExPattern(final HttpServletRequest httpRequest, final ConfigurerEntitiesList<Pattern> urlPatterns)
	{
		...
		for (final Pattern urlPattern : urlPatterns.getEntities())
		{
			...
			{
				log.debug("RequestSequencerFilter: URL Pattern: " + urlPattern.pattern() + " fits to the requested URI: "
						+ urlToLog + ".");
			...
			{
				log.debug("RequestSequencerFilter: URL Pattern: " + urlPattern.pattern()
