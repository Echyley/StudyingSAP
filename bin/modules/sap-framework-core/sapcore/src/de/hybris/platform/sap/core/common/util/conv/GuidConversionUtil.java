...
public class GuidConversionUtil
{
	...
	private static final int STRING_BUILDER_LENGTH = 32;

	private GuidConversionUtil() {
		throw new IllegalStateException("Utility class");
	}

	private static BigInteger maxVal = new BigInteger("100000000000000000000000000000000", BIG_INTEGER_RADIX);
	...
	public static byte[] convertToByteArray(final String arg)
	{
		...
		BigInteger value = new BigInteger(arg, BIG_INTEGER_RADIX);
	...
	public static String convertToString(final byte[] arg)
	{
		...
		final StringBuilder sb = new StringBuilder(value.toString(16));
		while (sb.length() < STRING_BUILDER_LENGTH)
		{
			sb.insert(0, "0");
		}
