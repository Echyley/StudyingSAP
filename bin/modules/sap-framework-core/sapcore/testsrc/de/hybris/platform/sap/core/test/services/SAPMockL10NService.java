...
public class SAPMockL10NService extends DefaultL10NService
{
	...
	private Properties loadLocalizations(final String lang)
	{
		...
		for (final String extName : c)
		{
			...
			try (final InputStream fis = TypeLocalization.class.getResourceAsStream(locFileName))
			{
				if (fis != null)
				{
					try (final Reader reader = new InputStreamReader(fis, DEFAULT_RESOURCE_BUNDLE_ENCODING))
					{
						props.load(reader);
					}
