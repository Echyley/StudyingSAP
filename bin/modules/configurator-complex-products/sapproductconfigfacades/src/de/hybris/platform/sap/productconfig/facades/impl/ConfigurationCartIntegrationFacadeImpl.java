		...
		implements ConfigurationCartIntegrationFacade
{
	...
	public String addConfigurationToCart(final ConfigurationData configContent) throws CommerceCartModificationException
	{
		...
		{
			LOG.debug(
					"Added product '" + product.getCode() + WITH_CONFIG_ID + configContent.getConfigId() + "' to cart with quantity '"
							+ cartItem.getQuantity() + REFERENCED_BY_CART_ENTRY_PK + cartItem.getPk().toString() + "'");
		}
			...
			throws CommerceCartModificationException
	{
		...
		//In case the configuration id is provided from outside, we take the product code from it. This is
		//needed to support adding a configuration for a product variant
		...
		if (LOG.isDebugEnabled())
		{
			LOG.debug(String.format("addProductConfigurationToCart for (variant) product %s and configurable product %s",
	...
	protected void removeLinkToProduct(final String code)
	{
		...
		getSessionAccessFacade().removeUiStatusForProduct(code);
	}
			...
			final boolean copyRequired, final String extConfig)
	{
		...
		{
			getConfigurationService().releaseSession(oldDraft);
		}
		getAbstractOrderEntryLinkStrategy().setDraftConfigIdForCartEntry(cartItemHandle, newConfigId);
		final ConfigModel configModel = getConfigurationService().retrieveConfigurationModel(newConfigId, null);
	...
	public ConfigurationData configureCartItemOnExistingDraft(final String cartEntryKey)
	{
		...
		if (configId == null)
		{
			LOG.debug("No draft found for cart entry " + cartEntryKey);
			return null;
		}
		final ConfigModel configModel = this.getConfigurationService().retrieveConfigurationModel(configId, null);
		final KBKeyData kbkey = new KBKeyData();
		kbkey.setProductCode(item.getProduct().getCode());
		final ConfigurationData configData = convert(kbkey, configModel);
		updateKBKeyForVariants(configData);
		return configData;
