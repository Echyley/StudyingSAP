...
public class ClassificationSystemCPQAttributesProviderImpl implements ClassificationSystemCPQAttributesProvider
{
	...
	private static final String LOCALE_AWARE_KEY = "%s_%s";
	...
	/**
	 * Classification system type, compare the S4H classification
	 */
	public static final String CLASSIFICATION = "300";
	...
	{
		final String locale = getI18NService().getCurrentLocale().toString();
		final String key = String.format(LOCALE_AWARE_KEY, name, locale);
		ClassificationSystemCPQAttributesContainer cpqAttributes = null;
		if (nameMap != null)
		{
			cpqAttributes = nameMap.get(key);
		}
		else
		{
			...
			final ClassificationSystemVersionModel systemVersion)
	{
		...
		else
		{
			...
			final String description = getClassificationAttributeDescriptionAccess().getDescription(attr);
	...
	protected Map<String, Collection<MediaModel>> addMediaEntries(final Collection<ClassificationAttributeValueModel> attrValues)
	{
		final int size = (int) (attrValues.size() / 0.75 + 1);
	...
	protected void checkForFormatQualifiers(final Map<String, CPQImageType> mediaFormatQualifiers)
	{
		if (MapUtils.isEmpty(mediaFormatQualifiers))
		{
			LOGGER.warn("No media format qualifiers available, media will be ignored");
		}
	}
	...
	public List<ImageData> getCsticMedia(final ClassificationSystemCPQAttributesContainer cpqAttributes)
	{
		...
		final Map<String, CPQImageType> cpqMediaFormatQualifiers = getCpqCsticImageFormatMapping().getCPQMediaFormatQualifiers();
		if (CollectionUtils.isNotEmpty(csticMedia) && MapUtils.isNotEmpty(cpqMediaFormatQualifiers))
		{
			images = convertMediaToImages(csticMedia, cpqMediaFormatQualifiers);
		}
		else
		{
			checkForFormatQualifiers(cpqMediaFormatQualifiers);
		}
		return images;
	}
			...
			final ClassificationSystemCPQAttributesContainer cpqAttributes)
	{
		...
		if (MapUtils.isNotEmpty(csticValueMedia))
		{
			...
			final Map<String, CPQImageType> cpqMediaFormatQualifiers = getCpqCsticValueImageFormatMapping()
					.getCPQMediaFormatQualifiers();
			if (CollectionUtils.isNotEmpty(media) && MapUtils.isNotEmpty(cpqMediaFormatQualifiers))
			{
				images = convertMediaToImages(media, cpqMediaFormatQualifiers);
			}
			else
			{
				checkForFormatQualifiers(cpqMediaFormatQualifiers);
			...
			final Map<String, CPQImageType> cpqMediaFormatQualifiers)
	{
		...
		{
			final String mediaFormatQualifier = medium.getMediaFormat().getQualifier();
			if (cpqMediaFormatQualifiers.containsKey(mediaFormatQualifier))
			{
				final ImageData image = new ImageData();
				getImagePopulator().populate(medium, image);
				final CPQImageType cpqImageQualifier = cpqMediaFormatQualifiers.get(mediaFormatQualifier);
	...
	/**
	 * @return Bean for accessing the classification attribute description. Actual implementation depends on the
	 *         existence of SAP integrations
	 */
	public ClassificationAttributeDescriptionAccess getClassificationAttributeDescriptionAccess()
	{
		return classificationAttributeDescriptionAccess;
	}

	/**
	 * @param classificationAttributeDescriptionAccess
	 *           the classificationAttributeDescriptionAccess to set
	 */
	public void setClassificationAttributeDescriptionAccess(
			final ClassificationAttributeDescriptionAccess classificationAttributeDescriptionAccess)
	{
		this.classificationAttributeDescriptionAccess = classificationAttributeDescriptionAccess;
	}
