...
public class ConfigurationPricingFacadeImpl implements ConfigurationPricingFacade
{
	...
	public PricingData getPriceSummary(final String configId)
	{
		...
		if (LOG.isDebugEnabled())
		{
			...
			LOG.debug("get price summary by configId [CONFIG_ID='" + sanitizedConfigId + "']");
		}
		final PriceSummaryModel priceSummary = getPricingService().getPriceSummary(sanitizedConfigId);
	...
	public List<PriceValueUpdateData> getValuePrices(final List<String> csticUiKeys, final String configId)
	{
		...
		{
			startTime = System.currentTimeMillis();
			LOG.debug("get value prices by configId [CONFIG_ID='" + sanitizedConfigId + "']");
		}
		final List<PriceValueUpdateData> valuePrices = new ArrayList();

		final ConfigModel configModel = getConfigurationService().retrieveConfigurationOverview(sanitizedConfigId);
	...
	protected List<PriceValueUpdateModel> retrieveValueUpdateModel(final List<String> csticUiKeys,
			...
			final InstanceModel rootInstance, final InstanceModel lastFound)
	{
		...
		final Optional<CsticModel> csticModelOptional = instance.getCstics().stream()
				.filter(cstic -> cstic.getName().equals(qualifier.getCsticName())).findFirst();
		if (csticModelOptional.isPresent())
		{
			final CsticModel csticModel = csticModelOptional.get();
			return Pair.of(csticModel, instance);
		}
		else
		{
			throw new IllegalStateException("We expect to find characteristic for ID: " + qualifier.getCsticName());
		}
