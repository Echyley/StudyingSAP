...
public class ValueFormatTranslatorImpl implements ValueFormatTranslator
{
	...
	private static final ThreadLocal<Map<Locale, DecimalFormat>> decimalFormatCache = ThreadLocal.withInitial(HashMap<Locale, DecimalFormat>::new);
	private static final ThreadLocal<DecimalFormat> serviceFormatCache = ThreadLocal.withInitial(ValueFormatTranslatorImpl::initialValue);
	...
	protected DecimalFormat createFormatterForUI(final Locale locale)
	{
		...
		return formatCache.computeIfAbsent(locale, k -> getDecimalFormat(locale));
