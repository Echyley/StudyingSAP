...
public class ConfigurationBaseFacadeImpl
{
	...
	private GroupStatusChecker groupStatusChecker;
	...
	protected ConfigurationData convert(final KBKeyData kbKey, final ConfigModel configModel)
	{
		...
		configData.setKbKey(convertKbKey(configModel.getKbKey(), kbKey.getProductCode()));
	...
	protected void populateKbKey(final KBKeyData target, final KBKey source)
	{
		target.setKbLogsys(source.getKbLogsys());
		target.setKbName(source.getKbName());
		target.setKbVersion(source.getKbVersion());
	...
	protected void populateConfigDataFromModel(final ConfigurationData configData, final ConfigModel configModel)
	{
		...
		configData.setImmediateConflictResolution(configModel.hasImmediateConflictResolution());
	...
	protected List<UiGroupData> getCsticGroupsFromModel(final ConfigModel configModel, final List<UiGroupData> csticGroupsFlat)
	{
		...
		return getGroupsFromInstance(configModel, configModel.getRootInstance(), hybrisNamesMap, csticGroupsFlat, 0);
	}
	 ...
	 */
	protected List<UiGroupData> getGroupsFromInstance(final ConfigModel configModel, final InstanceModel instance,
			...
			final int level)
	{
		...
		{
			final UiGroupData csticDataGroup = createCsticGroup(configModel, csticModelGroup, instance, nameMap);
		...
		{
			final UiGroupData uiGroup = createUiGroup(configModel, subInstance, nameMap, csticGroupsFlat, nextLevel);
	 ...
	 * @return UI group
	 */
	protected UiGroupData createCsticGroup(final ConfigModel configModel, final CsticGroup csticModelGroup,
			final InstanceModel instance, final Map<String, ClassificationSystemCPQAttributesContainer> nameMap)
	{
		...
		uiGroupData.setCollapsed(true);
		uiGroupData.setGroupType(GroupType.CSTIC_GROUP);
		uiGroupData.setFirstOrLastGroup(FirstOrLastGroupType.INTERJACENT);

		uiGroupData.setCstics(getListOfCsticData(configModel, csticModelGroup.getCstics(), groupKey, nameMap));
		if (getGroupStatusChecker() != null)
		{
			uiGroupData.setComplete(getGroupStatusChecker().checkCompleteness(uiGroupData));
			uiGroupData.setConsistent(getGroupStatusChecker().checkConsistency(csticModelGroup));
		}
		if (LOG.isDebugEnabled())
		{
	 ...
	 */
	protected UiGroupData createUiGroup(final ConfigModel configModel, final InstanceModel instance,
			final Map<String, ClassificationSystemCPQAttributesContainer> nameMap, final List<UiGroupData> csticGroupsFlat,
			final int level)
	{
		if (LOG.isDebugEnabled())
		...
		final List<UiGroupData> subGroups = getGroupsFromInstance(configModel, instance, nameMap, csticGroupsFlat, level);
		uiGroup.setSubGroups(subGroups);
		if (getGroupStatusChecker() != null)
		{
			uiGroup.setComplete(getGroupStatusChecker().checkCompletenessForSubGroups(subGroups));
			uiGroup.setConsistent(getGroupStatusChecker().checkConsistencyForSubGroups(subGroups));
		}
		uiGroup.setCstics(new ArrayList<>());
	...
	protected List<CsticData> getListOfCsticData(final ConfigModel configModel, final List<CsticModel> csticModelList,
			final String prefix, final Map<String, ClassificationSystemCPQAttributesContainer> nameMap)
	{
		final boolean expertModeActive = getConfigurationExpertModeFacade().isExpertModeActive();
		final List<CsticData> cstics = new ArrayList<>();
		for (final CsticModel model : csticModelList)
		{
			if (model.isConsistent() && !model.isVisible() && !expertModeActive)
			{
				continue;
			}

			final CsticData csticData = csticTypeMapper.mapCsticModelToData(configModel, model, prefix, nameMap);
	...
	{
		return groupStatusChecker;
	}

	/**
	 * @param groupStatusChecker
	 *           the groupStatusChecker to set
	 */
	public void setGroupStatusChecker(final GroupStatusChecker groupStatusChecker)
	{
		this.groupStatusChecker = groupStatusChecker;
	}
	...
	/**
	 * @param configurationExpertModeFacade
	 *           expert mode facade
	 */
	@Required
	...
	public KBKeyData convertKbKey(final KBKey kbKey, final String productCode)
	{
		final KBKeyData convertedKBKey = new KBKeyData();
		convertedKBKey.setProductCode(productCode);
		populateKbKey(convertedKBKey, kbKey);
		return convertedKBKey;
	}
}
