...
/**
 * Unit test for {@link CsticTypeMapperImpl}
 */
@UnitTest
public class CsticTypeMapperImplTest
{
	...
	@Mock
	private ClassificationAttributeDescriptionAccess classificationAttributeDescriptionAccess;
	@Mock
	...
	public void setup()
	{
		...
		given(pricingConfigurationParameters.showDeltaPrices(PRODUCT_CODE)).willReturn(true);
		...
		nameProvider.setClassificationAttributeDescriptionAccess(classificationAttributeDescriptionAccess);
	...
	{
		final CsticModel csticModel = ConfigurationTestData.createRadioButtonCstic();
		final CsticValueModel valueModel = csticModel.getAssignableValues().get(0);
		preparePrices(valueModel);
		final CsticData csticData = typeMapper.mapCsticModelToData(configModel, csticModel, GROUP_NAME, null);
		final CsticValueData valueData = csticData.getDomainvalues().get(0);
		assertEquals("delta price mapped incorrectly", 0, new BigDecimal("125.99").compareTo(valueData.getPrice().getValue()));
		assertEquals("EUR", valueData.getPrice().getCurrencyIso());
	}
	...
	public void testPrice_delta()
	{
		...
		assertEquals("delta price mapped incorrectly", 0, new BigDecimal("125.99").compareTo(valueDTO.getPrice().getValue()));
	...
	public void testSetPrice_delta()
	{
		...
		assertEquals("delta price mapped incorrect", 0, new BigDecimal("125.99").compareTo(valueDTO.getPrice().getValue()));
		assertEquals("EUR", valueDTO.getPrice().getCurrencyIso());
	...
	public void testSetPrice_absolute()
	{
		...
		assertEquals("absolute price mapped incorrect", 0, new BigDecimal("345.77").compareTo(valueDTO.getPrice().getValue()));
		assertEquals("EUR", valueDTO.getPrice().getCurrencyIso());
	...
	public void testPrice_absoluteValuePrice()
	{
		...
		assertEquals("absolute value price mapped incorrect", 0,
				new BigDecimal("345.77").compareTo(valueDTO.getPrice().getValue()));
		assertEquals("EUR", valueDTO.getPrice().getCurrencyIso());
	}
	...
	public void testCstiModelToDatacMapping() throws Exception
	{
		...
		final CsticData data = typeMapper.mapCsticModelToData(configModel, model, GROUP_NAME, null);
	...
	public void testInstanceId()
	{
		...
		final CsticData csticData = typeMapper.mapCsticModelToData(configModel, model, "P",
	...
	public void testCstiModelToDatacMappingForSimpleCheckBoxTrue() throws Exception
	{
		...
		CsticData data = typeMapper.mapCsticModelToData(configModel, model, GROUP_NAME, null);
		...
		data = typeMapper.mapCsticModelToData(configModel, model, GROUP_NAME, null);
	...
	public void testCsticModelToDatacMappingRetractBlocked() throws Exception
	{
		...
		assertFalse(data.isRetractBlocked());
	}
	...
	public void testCsticModelToDatacMappingRetractBlockedIfReadOnly() throws Exception
	{
		...
		assertTrue(data.isRetractBlocked());
	}
	...
	{
		//format 1.23
		checkMaxLength(2, 3, 4);
	}

	private void checkMaxLength(final int numberScale, final int typeLength, final int expectedMaxLength)
	{
		final CsticModel model = ConfigurationTestData.createFloatCstic();

		model.setNumberScale(numberScale);
		model.setTypeLength(typeLength);

		final CsticData data = typeMapper.mapCsticModelToData(configModel, model, GROUP_NAME, null);
		assertEquals("Wrong max length", expectedMaxLength, data.getMaxlength());
	}
	...
	public void testUpdateCsticModelValuesFromDataDropDownListNoSelection()
	{
		final CsticModel model = ConfigurationTestData.createDropDownCstic();
		final CsticData data = typeMapper.mapCsticModelToData(configModel, model, GROUP_NAME, null);
		data.setValue("NULL_VALUE"); //nothing selected
		typeMapper.updateCsticModelValuesFromData(data, model);

		assertFalse("retract was set to 'true', but this is not a retract case", model.isRetractTriggered());
	...
	public void testUpdateCsticModelValuesFromDataEmptyValueAndLastValidValue()
	{
		...
		final CsticData data = typeMapper.mapCsticModelToData(configModel, model, GROUP_NAME, null);
		data.setValue("");
		data.setLastValidValue("last valid value");

		typeMapper.updateCsticModelValuesFromData(data, model);

		assertTrue("retract was not set to 'true'", model.isRetractTriggered());
	}
	...
	private void checkRetractAfterUpdate(final String value, final String lastValidValue)
	{
		final CsticModel model = ConfigurationTestData.createDropDownCstic();
		final CsticData data = typeMapper.mapCsticModelToData(configModel, model, GROUP_NAME, null);
		data.setValue(value);
		data.setLastValidValue(lastValidValue);

		typeMapper.updateCsticModelValuesFromData(data, model);

		assertFalse("retract was not set to 'false'", model.isRetractTriggered());
	}

	@Test
	public void testUpdateCsticModelValuesFromDataBlankValueAndEmptyLastValidValue()
	{
		checkRetractAfterUpdate(" ", "");
	}

	@Test
	public void testUpdateCsticModelValuesFromDataValueAndBlankLastValidValue()
	{
		checkRetractAfterUpdate("value", "     ");
	}

	@Test
	public void testUpdateCsticModelValuesFromDataValueAndLastValidValue()
	{
		checkRetractAfterUpdate("value", "last valid value");
	}
	...
	public void testHandlePriceData_oneValueWithPrice()
	{
		...
		assertEquals(0, BigDecimal.ONE.compareTo(domainValues.get(0).getDeltaPrice().getValue()));
		assertEquals(0, BigDecimal.ZERO.compareTo(domainValues.get(1).getDeltaPrice().getValue()));
		assertEquals(0, BigDecimal.ONE.compareTo(domainValues.get(0).getPrice().getValue()));
		assertEquals(0, BigDecimal.ZERO.compareTo(domainValues.get(1).getPrice().getValue()));
		assertNotSame(ConfigPricing.NO_PRICE, domainValues.get(1).getPrice());
	}
	...
	public void testMapCsticModelToData_csticLongTextInHybris()
	{
		...
		given(classificationAttributeDescriptionAccess.getDescription(attribute)).willReturn(longText);
	...
	private void checkValueNullAndLastValueNotNull(final String attributeValue)
	{
		csticData.setLastValidValue("last valid value");
		final String value = attributeValue;
		assertTrue(typeMapper.isValueNullAndLastValidValueNotNull(csticData, value));
	}

	private void checkValueNullAndLastValueNotNullFalse(final String lastValidValue)
	{
		csticData.setLastValidValue(lastValidValue);
		final String value = "new value";
		assertFalse(typeMapper.isValueNullAndLastValidValueNotNull(csticData, value));
	}

	@Test
	public void testIsValueNullAndLastValidValueNotNullWithNullValue()
	{
		checkValueNullAndLastValueNotNull(null);
	}

	@Test
	public void testIsValueNullAndLastValidValueNotNullWithEmptyValue()
	{
		checkValueNullAndLastValueNotNull("");
	}
	...
	{
		checkValueNullAndLastValueNotNullFalse("last valid value");
	}
	...
	{
		checkValueNullAndLastValueNotNullFalse(null);
	}

	@Test
	public void testIsValueNullAndLastValidValueNotNullWithEmptyLastValidValue()
	{
		checkValueNullAndLastValueNotNullFalse("");
	}
	...
	{
		csticModel.setAssignableValues(Collections.emptyList());
		csticModel.setAssignedValues(Collections.emptyList());
		assertEquals(Collections.emptyList(), typeMapper.createDomainValues(csticModel, null, false, false));
	}
