...
/**
 * Unit test for {@link ConfigurationPricingFacadeImpl}
 */
@UnitTest
public class ConfigurationPricingFacadeImplTest
{
	...
	private CsticQualifier csticQualifierNotPresent;
	...
	public void setUp()
	{
		...
		csticQualifierNotPresent = createCsticQualifier("NOT PRESENT", INSTANCE_NAME_2, instanceId2);
		Mockito.when(uiKeyGenerator.splitId(CSTIC_UI_KEY_2)).thenReturn(csticQualifier2);
		Mockito.when(configurationService.retrieveConfigurationOverview(configId)).thenReturn(configModel);
	...
	public void testRetrieveCsticAndInstanceCsticNotFound()
	{
		final InstanceModel lastFound = configModel.getRootInstance().getSubInstances().get(0);
		assertTrue(pricingFacade.isLastFoundInstanceMatching(csticQualifier1, lastFound));
		pricingFacade.retrieveCsticAndInstance(csticQualifierNotPresent, configModel.getRootInstance(), lastFound);
