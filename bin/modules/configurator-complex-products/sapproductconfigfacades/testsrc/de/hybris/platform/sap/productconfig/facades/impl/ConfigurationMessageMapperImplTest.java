...
/**
 * Unit test for {@link ConfigurationMessageMapperImpl}
 */
@UnitTest
public class ConfigurationMessageMapperImplTest
{
	...
	private void checkMessagesForConfig(final boolean complete, final boolean consistent, final int expectedNumberOfMessages)
	{
		when(mockConfigurationAbstractOrderEntryLinkStrategy.getCartEntryForDraftConfigId(anyString())).thenReturn("cartEntry");
		configModel.setComplete(complete);
		configModel.setConsistent(consistent);
		...
		assertEquals(expectedNumberOfMessages, configData.getMessages().size());
	}

	@Test
	public void testMapMessagesFromModelToDraftCompleteInconsistentConfigDataWithMessages()
	{
		checkMessagesForConfig(true, false, 2);
	}
	...
	{
		checkMessagesForConfig(true, true, 1);
	}
	...
	{
		checkMessagesForConfig(false, false, 2);
	}

	@Test
	public void testMapMessagesFromModelToDataIgnoringThrowsUnsupportedEncodingException() throws UnsupportedEncodingException
	{
		classUnderTest = spy(classUnderTest);
		final String unresolvedIssueMessage = classUnderTest
				.getLocalizedText(ConfigurationMessageMapperImpl.UNRESOLVABLE_ISSUES_MSG);
		willThrow(UnsupportedEncodingException.class).given(classUnderTest).encodeHTML(unresolvedIssueMessage);
		when(mockConfigurationAbstractOrderEntryLinkStrategy.getCartEntryForDraftConfigId(anyString())).thenReturn("cartEntry");
		configModel.setConsistent(true);
		classUnderTest.mapMessagesFromModelToData(configData, configModel);
		assertEquals(0, configData.getMessages().size());
	}
