...
/**
 * Unit test for {@link ConfigurationCartRestorationPopulator}
 */
@UnitTest
...
public class ConfigurationCartRestorationPopulatorTest
{
	...
	{
		given(cpqConfigurableChecker.isCPQConfiguratorApplicableProduct(Mockito.nullable(ProductModel.class))).willReturn(true);
	...
	{
		given(cpqConfigurableChecker.isCPQConfiguratorApplicableProduct(Mockito.nullable(ProductModel.class))).willReturn(false);
		classUnderTest.populate(source, target);
		verify(configInfoPopulator, times(0)).populate(Mockito.any(), Mockito.any());
		verify(configurationPricingOrderIntegrationService, times(0)).fillSummaryMap(Mockito.any());
		verify(modelService, times(0)).save(Mockito.any());
