...
/**
 * Unit test for {@link ConfigurationOrderEntryProductInfoModelPopulator}
 */
@UnitTest
public class ConfigurationOrderEntryProductInfoModelPopulatorTest
{
	...
	public void setUp()
	{
		...
		doReturn(ClassificationSystemCPQAttributesContainer.NULL_OBJ).when(nameProvider).getCPQAttributes(Mockito.nullable(String.class),
				(Map<String, ClassificationSystemCPQAttributesContainer>) Mockito.nullable(Map.class));
	}
	...
	public void testConfigInfoInlineContainsSemicolon()
	{
		...
		assertEquals("There should be " + expectedNumberOfSemicolons + "';' in 'configInfoInline': ", expectedNumberOfSemicolons,
				counter);
	}
	...
	public void testNoAssignedValues()
	{
		final int numberOfMaxCstics = 5;
		final List<AbstractOrderEntryProductInfoModel> configInfoModelList = new ArrayList();
		final ConfigModel configModel = ConfigurationTestData.createEmptyConfigModel();
		classUnderTest.setMaxNumberOfDisplayedCsticsInCart(numberOfMaxCstics);
		classUnderTest.populate(configModel, configInfoModelList);
	...
	@Test
	public void testNoGroupsCsticOnInstance()
	{
		final List<AbstractOrderEntryProductInfoModel> configInfoModelList = new ArrayList();
		...
		assertEquals("Configurator Type should be set to CPQCONFIGURATOR ", ConfiguratorType.CPQCONFIGURATOR,
				firstEntry.getConfiguratorType());
		assertEquals("Radio Button Group", firstEntry.getCpqCharacteristicName());
		assertEquals("Value 1", firstEntry.getCpqCharacteristicAssignedValues());
	}
