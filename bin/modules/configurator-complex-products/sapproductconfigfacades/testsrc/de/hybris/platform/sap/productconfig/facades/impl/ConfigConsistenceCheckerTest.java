...
/**
 * Unit test for {@link ConfigConsistenceChecker}
 */
@UnitTest
public class ConfigConsistenceCheckerTest
{
	...
	public void testSingleRadioButtonConsistence()
	{
		...
		configChecker.checkConfiguration(configData);


		assertNotNull("No ConfigurationData found", configData);
		assertNotNull("No CsticData found", configData.getGroups().get(0).getCstics());

		final CsticData checkedCstic = configData.getGroups().get(0).getCstics().get(0);
	...
	public void testSingleRadioButtonConsistenceOnSubInstance()
	{
		...
		final CsticData checkedCstic = configData.getGroups().get(0).getSubGroups().get(0).getCstics().get(0);
		assertEquals(UiType.READ_ONLY, checkedCstic.getType());
	}
