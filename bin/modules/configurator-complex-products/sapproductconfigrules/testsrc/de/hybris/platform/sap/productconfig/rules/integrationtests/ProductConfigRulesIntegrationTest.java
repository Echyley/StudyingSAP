...
public class ProductConfigRulesIntegrationTest extends ProductConfigRulesTest
{
			...
			throws CommerceCartModificationException, RuleEngineServiceException
	{
		...
		ruleAwareService.retrieveConfigurationOverview(config.getId());
		addConfigurationToCart(config, KB_Y_SAP_SIMPLE_POC);
		config = ruleAwareService.retrieveConfigurationOverview(config.getId());
	...
	public void testRulesAreAppliedOnCreateFromExternal() throws CommerceCartModificationException
	{
		...
		final String groupId = serviceConfigValueHelper.getGroup(config, CPQ_DISPLAY).getName();
		// release session
		ruleAwareService.releaseSession(config.getId());
		...
		config = ruleAwareService.retrieveConfigurationModel(config.getId(), groupId);
	...
	@Test
	public void testExecuteOnlyRuleWithHighestPriorityInExclusiveGroup()
			throws CommerceCartModificationException, RuleEngineServiceException
	{
		// Show only one message (from the rule with highest priority) for CPQ_LAPTOP (default configuration)
		// Only One (Info) message for product expected to be shown

		final Map<String, String> expectedMessagesByName = prepareAndPublishRule("cpq_test_rule_exclusive_group_priority_high",
				"cpq_test_rule_exclusive_group_priority_low");
		final String messageForProduct = expectedMessagesByName.get("cpq_test_rule_exclusive_group_priority_high");

		// Default configuration
		final ConfigModel config = ruleAwareService.createDefaultConfiguration(KB_CPQ_LAPTOP);

		assertSingleMessage(config, ProductConfigMessageSeverity.INFO, messageForProduct);
	}
	...
	public void testVerifyProductParameterFormat() throws CommerceCartModificationException, RuleEngineServiceException
	{
		...
		final String conditionDefinitionIdCurrentConfigProduct = "y_configurable_product";
		//Parameter name for "product" in the Product Configuration rules
		final String parameterProduct = "product";

		final Map<String, String> nameToMessage = new HashMap<>();
		final SourceRuleModel sourceRuleModel = prepareSourceRule(nameToMessage, "cpq_test_rule_exclusive_group_priority_high");

		final Class<?> ruleType = sourceRuleModel.getClass();
		...
		{
			if (conditionDefinitionIdCurrentConfigProduct.equalsIgnoreCase(conditionData.getDefinitionId()))
			{
				final Map<String, RuleParameterData> parameters = conditionData.getParameters();
				final RuleParameterData productParameter = parameters.get(parameterProduct);

				if (productParameter != null)
				{
					final String productCodeWithCatalog = productParameter.getValue();
					// Expected that productCodeWithCatalog has the format: productCode::catalog
					final String[] parsedProductParameter = productCodeWithCatalog.split("::");
					assertEquals("Wrong ProductCode", PRODUCT_CODE_CPQ_LAPTOP, parsedProductParameter[0]);
					assertTrue("Missing Catalog in productParameter", StringUtils.isNotEmpty(parsedProductParameter[1]));
					parameterFound = true;
				}
			}
		}
		assertTrue("Expected productParameter was not found", parameterFound);
	}
