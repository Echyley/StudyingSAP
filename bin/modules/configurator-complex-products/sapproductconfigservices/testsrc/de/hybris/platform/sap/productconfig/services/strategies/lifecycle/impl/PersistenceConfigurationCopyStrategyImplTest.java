...
/**
 * Unit test for {@link PersistenceConfigurationCopyStrategyImpl}
 */
@UnitTest
...
public class PersistenceConfigurationCopyStrategyImplTest
{
	...
	// does not contain a config id as it is excluded from clone
	private final ProductConfigurationModel clonedConfigModel = new ProductConfigurationModel();
	...
	public void setUp()
	{
		...
		cloneEntry.setEntryNumber(Integer.valueOf(0));
	...
	@Test
	public void testFinalizeCloneShallowCopy()
	{
		classUnderTest.finalizeCloneShallowCopy(order, clone);
		assertEquals(CONFIG_ID, cloneEntry.getProductConfiguration().getConfigurationId());
	}

	@Test(expected = None.class)
	public void testFinalizeCloneShallowCopyNoTargetEntryies()
	{
		//must not crash
		clone.setEntries(null);
		classUnderTest.finalizeCloneShallowCopy(order, clone);
	...
	@Test
	public void testfinalizeCloneEntryNumberMismatch()
	{
	...
	@Test
	public void testFinalizeCloneShallowCopyEntryNumberMismatch()
	{
		entry.setEntryNumber(Integer.valueOf(13));
		classUnderTest.finalizeCloneShallowCopy(order, clone);
		assertEquals(CONFIG_ID, cloneEntry.getProductConfiguration().getConfigurationId());
	}
	...
	public void testReplaceProductConfiguration()
	{
		...
		classUnderTest.replaceProductConfiguration(entry, cloneEntry,
				(config) -> classUnderTest.replaceWithDeepCopy(entry, config, oldConfigId2newConfigIdMap));
		verify(persistenceService).getByConfigId(NEW_CONFIG_ID);
	...
	public void testReplaceProductConfigurationNoProductConfigurationModel()
	{
		...
		classUnderTest.replaceProductConfiguration(entry, entry,
				(configId) -> classUnderTest.replaceWithDeepCopy(entry, configId, new HashMap()));
	...
	public void testReplaceProductConfigurationNoConfigId()
	{
		...
		classUnderTest.replaceProductConfiguration(entry, cloneEntry,
				(configId) -> classUnderTest.replaceWithDeepCopy(entry, configId, new HashMap()));
		verify(persistenceService, never()).getByConfigId(NEW_CONFIG_ID);
		assertNull(cloneEntry.getProductConfiguration());
	}
