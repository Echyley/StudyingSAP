...
/**
 * Unit test for {@link ProductConfigAfterInitializationStartEventListener}
 */
@UnitTest
public class ProductConfigAfterInitializationStartEventListenerTest
{
	...
	public void setUp()
	{
		...
		classUnderTest = new ProductConfigAfterInitializationStartEventListenerForTest(false);
	...
	{
		this.classUnderTest = new ProductConfigAfterInitializationStartEventListenerForTest(true);
		final StringBuilder failedIds = new StringBuilder();
		this.classUnderTest.releaseConfigs(Collections.singletonList(model), failedIds);
		
		verifyZeroInteractions(configurationProvider);
		assertEquals(model.getConfigurationId() + ProductConfigAfterInitializationStartEventListener.ID_SEPERATOR, failedIds.toString());
	}
			...
			extends ProductConfigAfterInitializationStartEventListener
	{
		...
		protected boolean throwBeanNotFoundException = false;

		public ConfigurationProvider cpsProvider = new CPSConfiguratioProviderForTest();


		public ProductConfigAfterInitializationStartEventListenerForTest(final boolean throwBeanNotFoundException)
		{
			this.throwBeanNotFoundException = throwBeanNotFoundException;
		}
		...
		{
			if (throwBeanNotFoundException)
			{
				throw new NoSuchBeanDefinitionException("sapConfigurationProviderCPS");
			}
			return cpsProvider;
		}
