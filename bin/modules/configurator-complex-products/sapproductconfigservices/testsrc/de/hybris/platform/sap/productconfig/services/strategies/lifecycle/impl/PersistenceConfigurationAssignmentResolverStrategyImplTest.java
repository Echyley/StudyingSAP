...
/**
 * Unit test for {@link PersistenceConfigurationAssignmentResolverStrategyImpl}
 */
@UnitTest
public class PersistenceConfigurationAssignmentResolverStrategyImplTest
{
	...
	private static final String SAVED_CART_CODE = "222";
	private static final String CART_CODE = "111";
	...
	@InjectMocks
	private final PersistenceConfigurationAssignmentResolverStrategyImpl classUnderTest = new PersistenceConfigurationAssignmentResolverStrategyImpl();
	...
	public void setUp()
	{
		...
		savedCart.setCode(SAVED_CART_CODE);
		savedCart.setSaveTime(new Date());
		...
		given(commerceCartService.getCartForCodeAndUser(SAVED_CART_CODE, userModel)).willReturn(savedCart);
		given(commerceCartService.getCartForCodeAndUser(CART_CODE, userModel)).willReturn(cart);
	}
	...
	{
		assertTrue(classUnderTest.isSavedCart(SAVED_CART_CODE));
	}

	@Test
	public void testIsSavedCartCheckForSessionCart()
	{
		assertFalse(classUnderTest.isSavedCart(CART_CODE));
	}

	@Test(expected = IllegalStateException.class)
	public void testIsSavedCartNoCartFound()
	{
		classUnderTest.isSavedCart("Unknown");
	}
	...
	public void testDoesCartCodeBelongToSessionCartStateless()
	{
		...
		assertFalse(classUnderTest.doesCartCodeBelongsToSessionCart(SAVED_CART_CODE));
	}
	...
	{
		given(cartService.hasSessionCart()).willReturn(false);
		assertTrue(classUnderTest.doesCartCodeBelongsToSessionCart(CART_CODE));
	}
}
