...
/**
 * Unit test for {@link ProductConfigurationPersistenceServiceImpl}
 */
@UnitTest
public class ProductConfigurationPersistenceServiceImplTest
{
	...
	public void setUp()
	{
		...
		queryMatcher = matchQueryWithParams(ProductConfigurationPersistenceServiceImpl.GET_ENTRY_BY_PK,
				Collections.singletonMap("cartEntryKey", CART_ENTRY_PK.getLong()));
		mockGivenQuery(queryMatcher, entryModel);

		queryMatcher = matchQueryWithParams(ProductConfigurationPersistenceServiceImpl.GET_ENTRY_BY_CONFIG_PK,
				Collections.singletonMap(ProductConfigurationPersistenceServiceImpl.PARAM_NAME_CONFIG_PK, CONFIG_MODEL_PK.getLong()));
		mockGivenQuery(queryMatcher, entryModel);

		queryMatcher = matchQueryWithParams(ProductConfigurationPersistenceServiceImpl.GET_ENTRY_BY_CONFIG_PK, Collections
				.singletonMap(ProductConfigurationPersistenceServiceImpl.PARAM_NAME_CONFIG_PK, OTHER_CONFIG_MODEL_PK.getLong()));
		mockGivenQuery(queryMatcher, otherEntryModel);

		queryMatcher = matchQueryWithParams(ProductConfigurationPersistenceServiceImpl.GET_ENTRY_BY_DRAFT_CONFIG_PK,
				Collections.singletonMap(ProductConfigurationPersistenceServiceImpl.PARAM_NAME_CONFIG_PK, CONFIG_MODEL_PK.getLong()));
		mockGivenQuery(queryMatcher, entryModel);
		...
		expectedParams.put("userPk", Long.valueOf(USER_PK));
	...
	protected ArgumentMatcher<FlexibleSearchQuery> matchQueryWithParams(final String queryString, final Map<String, Object> params)
	{
	...
	{
		return new ArgumentMatcherPSFP<PaginatedFlexibleSearchParameter>(queryString, params, pageSize, currentPage);
	}
	...
	public class ArgumentMatcherFSQ<FlexibleSearchQuery> implements ArgumentMatcher
	{
		...
		ArgumentMatcherFSQ(final String queryString, final Map<String, Object> params)
		{
		...
		public boolean matches(final Object argument)
		{
			...
			boolean matches = queryString.equals(((de.hybris.platform.servicelayer.search.FlexibleSearchQuery) query).getQuery())
					&& ((de.hybris.platform.servicelayer.search.FlexibleSearchQuery) query).getQueryParameters().size() == params
							.size();
			for (final Entry<String, Object> expectedParam : params.entrySet())
			{
				matches = matches
						&& expectedParam.getValue().equals(((de.hybris.platform.servicelayer.search.FlexibleSearchQuery) query)
								.getQueryParameters().get(expectedParam.getKey()));
			}
			return matches;
		}
	...
	public class ArgumentMatcherPSFP<PaginatedFlexibleSearchParameter> implements ArgumentMatcher
	{
		...
		ArgumentMatcherPSFP(final String queryString, final Map<String, String> params, final int pageSize, final int currentPage)
		{
		...
		public boolean matches(final Object argument)
		{

			{
				...
				final FlexibleSearchQuery query = ((de.hybris.platform.servicelayer.search.paginated.PaginatedFlexibleSearchParameter) pageableQuery)
						.getFlexibleSearchQuery();
				boolean matches = queryString.equals(query.getQuery()) && query.getQueryParameters().size() == params.size();
				matches = matches
						&& ((de.hybris.platform.servicelayer.search.paginated.PaginatedFlexibleSearchParameter) pageableQuery)
								.getSearchPageData().getPagination().getCurrentPage() == currentPage;
				matches = matches
						&& ((de.hybris.platform.servicelayer.search.paginated.PaginatedFlexibleSearchParameter) pageableQuery)
								.getSearchPageData().getPagination().getPageSize() == pageSize;
