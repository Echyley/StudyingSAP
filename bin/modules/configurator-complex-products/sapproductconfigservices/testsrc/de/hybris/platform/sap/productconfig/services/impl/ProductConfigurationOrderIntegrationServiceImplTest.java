...
/**
 * Unit test for {@link ProductConfigurationOrderIntegrationServiceImpl}
 */
@UnitTest
public class ProductConfigurationOrderIntegrationServiceImplTest
{
	...
	public void setup()
	{
		...
		when(configurationService.retrieveConfigurationOverview(configId)).thenReturn(configModel);
	...
	public void testEnsureConfigurationInSessionWithIdAndModel()
	{
		...
		classUnderTest.ensureConfigurationInSession(cartEntryKey, mockCartEntry.getProduct().getCode(),
				externalConfigurationAccess.getExternalConfiguration(mockCartEntry));
		verify(configurationService, times(1)).retrieveConfigurationOverview(configId);
	...
	public void testEnsureConfigurationInSessionWithoutIdAndWithoutExternal()
	{
		...
		classUnderTest.ensureConfigurationInSession(mockCartEntry.getPk().toString(), mockCartEntry.getProduct().getCode(),
				externalConfigurationAccess.getExternalConfiguration(mockCartEntry));
		verify(configurationService, times(0)).retrieveConfigurationOverview(configId);
	...
	{
		when(externalConfigurationAccess.getExternalConfiguration(mockCartEntry)).thenReturn(DUMMY_XML);
		final String cartEntryKey = mockCartEntry.getPk().toString();
