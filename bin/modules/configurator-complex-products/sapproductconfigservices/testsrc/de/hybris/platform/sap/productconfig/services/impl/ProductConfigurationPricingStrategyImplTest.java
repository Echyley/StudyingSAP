...
/**
 * Unit test for {@link ProductConfigurationPricingStrategyImpl}
 */
@UnitTest
public class ProductConfigurationPricingStrategyImplTest
{
	...
	public void setup()
	{
		...
		when(configurationService.retrieveConfigurationOverview(CONFIG_ID_NO_PRICE)).thenReturn(configModel);
		when(configurationService.retrieveConfigurationOverview(NEW_CONFIG_ID)).thenReturn(newConfigModel);
	...
	{
		when(configurationService.retrieveConfigurationOverview(CONFIG_ID)).thenReturn(modelMock);
		...
		verify(configurationService).retrieveConfigurationOverview(CONFIG_ID);
	}
	...
	public void testRetrieveCurrentTotalPriceCPS()
	{
		...
		verify(configurationService, times(0)).retrieveConfigurationOverview(CONFIG_ID);
	}
