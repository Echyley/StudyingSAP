...
/**
 * Unit test for {@link PersistenceConfigurationLifecycleStrategyImpl}
 */
@UnitTest
public class PersistenceConfigurationLifecycleStrategyImplTest
{
	...
	private static final String OTHER_SESSION_ID = "9882";
	...
	private static final String VERSION_NUMERIC = "6";
	private static final String UPDATED_VERSION_NUMERIC = "7";
	private static final String VERSION_NUMERIC_QUOTES = "\"6\"";
	private static final String UPDATED_VERSION_NUMERIC_QUOTES = "\"7\"";
	private static final String VERSION_NUMERIC_QUOTES_WEAK = "W/\"6\"";
	private static final String UPDATED_VERSION_NUMERIC_QUOTES_WEAK = "W/\"7\"";

	private static final String USER_SESSION_ID = "userSessionId";
	private static final String GROUP_ID = "groupId";
	private static final boolean OVERVIEW_ONLY = false;
	...
	public void setUp() throws ConfigurationEngineException
	{
		...
		given(otherUser.getName()).willReturn("cpq02");
		given(otherUser.getPk()).willReturn(PK.fromLong(1234l));
		given(userService.getCurrentUser()).willReturn(currentUser);
		given(userService.isAdmin(currentUser)).willReturn(false);
		...
		given(providerMock.retrieveConfigurationModel(eq(CONFIG_ID), eq(GROUP_ID), eq(OVERVIEW_ONLY),
				any(ConfigurationRetrievalOptions.class))).willReturn(config);
	...
	{
		given(sessionService.getAttribute(SapproductconfigservicesConstants.SESSION_NOT_BOUND_TO_CONFIGURATIONS))
	...
	{
		productConfigModel.setUser(otherUser);
		productConfigModel.setUserSessionId(OTHER_SESSION_ID);
		assertFalse(classUnderTest.isConfigForCurrentUser(CONFIG_ID));
	}
	...
	public void testIsConfigForCurrentUserWithAnonUserButNotAllowedToRead()
	{
		productConfigModel.setUserSessionId(OTHER_SESSION_ID);
		productConfigModel.setUser(otherUser);
		given(userService.isAnonymousUser(otherUser)).willReturn(true);
		given(sessionService.getAttribute(SapproductconfigservicesConstants.SESSION_NOT_BOUND_TO_CONFIGURATIONS))
	...
	public void testIsAnonymousConfigAndAllowedToReadWithAnonUserButNotAllowedToRead()
	{
		...
		given(sessionService.getAttribute(SapproductconfigservicesConstants.SESSION_NOT_BOUND_TO_CONFIGURATIONS))
				.willReturn(Boolean.valueOf((false)));
		assertFalse(classUnderTest.isAnonymousConfigAndAllowedToRead(productConfigModel));
	}
	...
	{
		productConfigModel.setUser(otherUser);
		given(sessionService.getAttribute(SapproductconfigservicesConstants.SESSION_NOT_BOUND_TO_CONFIGURATIONS)).willReturn(null);
		assertFalse(classUnderTest.isConfigForCurrentUser(CONFIG_ID));
	}
	...
	{
		assertTrue(classUnderTest.isUpdateETagNeeded(UPDATED_VERSION_NUMERIC, VERSION_NUMERIC));
	}

	@Test
	public void testIsUpdateETagNeededDecreaseOfVersionCounter()
	{
		assertFalse(classUnderTest.isUpdateETagNeeded(VERSION_NUMERIC, UPDATED_VERSION_NUMERIC));
	}

	@Test
	public void testIsUpdateETagNeededDecreaseOfVersionCounterIgnoreQuotes()
	{
		assertFalse(classUnderTest.isUpdateETagNeeded(VERSION_NUMERIC_QUOTES, UPDATED_VERSION_NUMERIC_QUOTES));
	}

	@Test
	public void testIsUpdateETagNeededDecreaseOfVersionCounterIgnoreQuotesAndWeakPrefix()
	{
		assertFalse(classUnderTest.isUpdateETagNeeded(VERSION_NUMERIC_QUOTES_WEAK, UPDATED_VERSION_NUMERIC_QUOTES_WEAK));
	}

	@Test
	public void testIsUpdateETagNeededNonNumericInputWillCheckForEquality()
	{
		assertTrue(classUnderTest.isUpdateETagNeeded(VERSION, UPDATED_VERSION));
	}

	@Test
	public void testIsUpdateETagNeededForNullInput()
	{
		assertFalse(classUnderTest.isUpdateETagNeeded(null, null));
	}
	...
	{
		final ConfigurationRetrievalOptions options = new ConfigurationRetrievalOptions();
		classUnderTest.retrieveConfigurationModel(CONFIG_ID, GROUP_ID, OVERVIEW_ONLY, options);
		verify(providerMock).retrieveConfigurationModel(CONFIG_ID, GROUP_ID, OVERVIEW_ONLY, options);
	}
