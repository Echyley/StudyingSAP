...
public class PersistenceConfigurationLifecycleStrategyImpl implements ConfigurationLifecycleStrategy
{
	...
	protected static final String WEAK_ETAG_PREFIX = "W/";
	...
	public boolean isConfigForCurrentUser(final String configId)
	{
		...
		final boolean isAnonymousConfigAndAllowedToRead = isAnonymousConfigAndAllowedToRead(configPersistenceModel);
		final boolean isAdminUser = isAdminUser(currentUser);
		return isSameUser || isSameSession || isAnonymousConfigAndAllowedToRead || isAdminUser;
	}

	/**
	 * Returns true if the configuration's user is anonymous and the flag "SESSION_NOT_BOUND_TO_CONFIGURATIONS" is set to
	 * true.
	 *
	 * @param productConfiguration
	 *
	 */
	protected boolean isAnonymousConfigAndAllowedToRead(final ProductConfigurationModel productConfiguration)
	{
		boolean isAnonymousConfigAndAllowedToRead = false;
		...
		{
			isAnonymousConfigAndAllowedToRead = getUserService().isAnonymousUser(productConfiguration.getUser());
		}

		return isAnonymousConfigAndAllowedToRead;
	}
	...
	{
		LOG.debug("update etag from configuration model");
	...
	{
		final String existingVersion = persistenceModel.getVersion();
		if (LOG.isDebugEnabled())
		{
			LOG.debug(String.format("update etag. New value %s is going to replace %s", newVersion, existingVersion));
		}
		if (isUpdateETagNeeded(newVersion, existingVersion))
		{
	...
	protected boolean isUpdateETagNeeded(final String newVersion, final String existingVersion)
	{
		if (newVersion == null || existingVersion == null)
		{
			return false;
		}
		try
		{
			final long newVersionNumeric = Long.parseLong(newVersion.replace("\"", "").replace(WEAK_ETAG_PREFIX, ""));
			final long existingVersionNumeric = Long.parseLong(existingVersion.replace("\"", "").replace(WEAK_ETAG_PREFIX, ""));
			if (newVersionNumeric < existingVersionNumeric)
			{
				LOG.warn(String.format(
						"New eTag '%s' is smaller than existing one '%s'. This is probably due to parallel update requests. Version counter won't be changed",
						newVersion, existingVersion));
			}
			return newVersionNumeric > existingVersionNumeric;
		}
		catch (final NumberFormatException ex)
		{
			LOG.warn(String.format("Either '%s' or '%s' is not of numeric format, update of version will happen if not equal",
					newVersion, existingVersion));
			return !newVersion.equals(existingVersion);
		}
	...
	{
		final ConfigModel configModel = getConfigurationProvider().retrieveConfigurationModel(configId, groupId, overviewOnly,
				options);
		updateETagFromModel(configModel);
		return configModel;
	}
