...
public class TrackingRecorderImpl extends AbstractTrackingRecorderImpl implements TrackingRecorder
{
			...
			final EventType event)
	{
		...
		item.getParameters().put(RecorderParameters.CART_ITEM_PK.toString(), DigestUtils.sha3_256Hex(entry.getPk().toString()));
		item.getParameters().put(RecorderParameters.QUANTITY.toString(), entry.getQuantity().toString());
		item.getParameters().put(RecorderParameters.CART.toString(), DigestUtils.sha3_256Hex(parameters.getCart().getGuid()));
		return item;
	}
