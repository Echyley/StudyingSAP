...
public class PersistenceConfigurationCopyStrategyImpl implements ConfigurationCopyStrategy
{
	...
	public void finalizeClone(final AbstractOrderModel source, final AbstractOrderModel target)
	{
		...
		if (entries != null)
		{
			for (int i = 0; i < entries.size(); i++)
			{
				...
				final AbstractOrderEntryModel cloneEntry = entries.get(i);
				replaceProductConfiguration(sourceEntry, cloneEntry,
						configModel -> replaceWithDeepCopy(cloneEntry, configModel, oldConfigId2newConfigIdMap));
			}
		}
	...
	protected void prepareEnvironment(final AbstractOrderModel source)
	{
		if (source == null)
		{
		...
		if (entries != null)
		{
			for (int i = 0; i < entries.size(); i++)
			{
				final AbstractOrderEntryModel sourceEntry = source.getEntries().get(i);
				 final AbstractOrderEntryModel cloneEntry = entries.get(i);
				replaceProductConfiguration(sourceEntry, cloneEntry,
						configModel -> replaceWithShallowCopy(cloneEntry, configModel));
			}
		}
	}
	...
	protected void replaceProductConfiguration(final AbstractOrderEntryModel sourceEntry,
			final AbstractOrderEntryModel cloneEntry,
			final Consumer<ProductConfigurationModel> replacementStrategy)
	{
		final ProductConfigurationModel oldConfigModel = sourceEntry.getProductConfiguration();
		if (cloneEntry.getProductConfiguration() != null)
		{
			throw new IllegalStateException("Clone should exclude ProductConfiguration");
		}
		if (oldConfigModel != null)
		{
			...
			{
				replacementStrategy.accept(oldConfigModel);
			}
		}
	}

	protected void replaceWithDeepCopy(final AbstractOrderEntryModel cloneEntry, final ProductConfigurationModel oldConfiguration,
			final Map<String, String> oldConfigId2newConfigIdMap)
	{
		final String newConfigId = oldConfigId2newConfigIdMap.get(oldConfiguration.getConfigurationId());
	...
	protected void replaceWithShallowCopy(final AbstractOrderEntryModel cloneEntry,
			final ProductConfigurationModel oldConfiguration)
	{
		cloneEntry.setProductConfiguration(oldConfiguration);
	}
