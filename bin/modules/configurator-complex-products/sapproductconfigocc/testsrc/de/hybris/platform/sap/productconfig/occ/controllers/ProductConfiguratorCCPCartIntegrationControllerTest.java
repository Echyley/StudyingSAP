...
public class ProductConfiguratorCCPCartIntegrationControllerTest
{
	...
	@Mock
	private ConfigurationExpertModeFacade expModefacade;
	...
	@Test
	public void testConfigureCartEntryInternal() throws CommerceCartModificationException
	{
	...
	@Test
	public void testConfigureCartEntryWithoutOptionalParameter() throws CommerceCartModificationException
	{
		final ProductWsDTO productWs = getProductWsDTO(PRODUCT_CODE);
		final ProductConfigOrderEntryWsDTO orderEntry = getProductConfigOrderEntryWsDTO(productWs);
 		final ConfigurationData configuration = getConfigurationData(PRODUCT_CODE);
 		Mockito.when(cartFacade.getSessionCart()).thenReturn(sessionCart);
 		Mockito.when(configCartFacade.configureCartItem(itemPK)).thenReturn(configuration);
 		final ConfigurationWsDTO configurationWsDto = classUnderTest.configureCartEntry("", orderEntry.getEntryNumber());
 		Mockito.verify(ccpControllerHelper).mapDTOData(Mockito.any(ConfigurationData.class));
	}
	...
	public void testConfigureCartEntry() throws CommerceCartModificationException
	{
		...
		Mockito.when(cartFacade.getSessionCart()).thenReturn(sessionCart);
		Mockito.when(configCartFacade.configureCartItem(itemPK)).thenReturn(configuration);
		final ConfigurationWsDTO configurationWsDto = classUnderTest.configureCartEntry("", orderEntry.getEntryNumber(),false);
		Mockito.verify(ccpControllerHelper).mapDTOData(Mockito.any(ConfigurationData.class));
		Mockito.verifyNoInteractions(expModefacade);

	}

	@Test
	public void testConfigureCartEntryExpModeEnabled() throws CommerceCartModificationException
	{
		final ProductWsDTO productWs = getProductWsDTO(PRODUCT_CODE);
		final ProductConfigOrderEntryWsDTO orderEntry = getProductConfigOrderEntryWsDTO(productWs);
		final ConfigurationData configuration = getConfigurationData(PRODUCT_CODE);
		Mockito.when(cartFacade.getSessionCart()).thenReturn(sessionCart);
		Mockito.when(configCartFacade.configureCartItem(itemPK)).thenReturn(configuration);
		final ConfigurationWsDTO configurationWsDto = classUnderTest.configureCartEntry("", orderEntry.getEntryNumber(),true);
		Mockito.verify(ccpControllerHelper).mapDTOData(Mockito.any(ConfigurationData.class));
		Mockito.verify(expModefacade).enableExpertMode();
