...
public class ModelCloneFactory
{
	...
	public static ConfigModel cloneConfigModel(final ConfigModel model)
	{
		...
		clonedConfigModel.setId(model.getId());
		clonedConfigModel.setKbBuildNumber(model.getKbBuildNumber());
		clonedConfigModel.setImmediateConflictResolution(model.hasImmediateConflictResolution());
		if (model.getMessages() != null)
		{
			// ProductConfigMessage is immutable - no need to clone
			final Set<ProductConfigMessage> clonedMessages = new HashSet<>(model.getMessages());
			clonedConfigModel.setMessages(clonedMessages);
		}
		...
		if (model.getCurrentTotalSavings() != null)
		{
			clonedConfigModel.setCurrentTotalSavings(clonePriceModel(model.getCurrentTotalSavings()));
		}
	...
	protected static PriceModel clonePriceModel(final PriceModel priceModel)
	{
		...
		clonedPriceModel.setObsoletePriceValue(priceModel.getObsoletePriceValue());
	...
	protected static CsticModel cloneCsticModel(final CsticModel csticModel)
	{
		...
		clonedCstic.setAssignedValuesWithoutCheckForChange(clonedAssignedValues);
		...
		clonedCstic.setChangedByFrontend(csticModel.isChangedByFrontend());
