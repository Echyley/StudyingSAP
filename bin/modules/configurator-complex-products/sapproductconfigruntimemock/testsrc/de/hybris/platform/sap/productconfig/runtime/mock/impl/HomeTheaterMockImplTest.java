...
/**
 * Unit test for {@link HomeTheaterMockImpl}
 */
@UnitTest
public class HomeTheaterMockImplTest
{
	...
	public void setUp()
	{
		...
		model.setKbKey(new KBKeyImpl("CONF_HOME_THEATER_ML"));
		...
		extras = instance.getSubInstance(HomeTheaterMockImpl.EXTRAS_SUBINSTANCE_ID);
	}
	...
	public void testMotorizedVisible()
	{
		...
		assertTrue(motorized.isVisible());
	...
	public void testMotorizedInvisible()
	{
		...
		final List<CsticValueModel> assignedValuesAfterChange = motorized.getAssignedValues();
		assertFalse( motorized.isVisible());
	...
	public void testProjectionScreenVisible()
	{
		...
		assertTrue(projectionScreen.isVisible());
	...
	public void testProjectionScreenInVisible()
	{
		...
		assertFalse(projectionScreen.isVisible());
	...
	public void testSoundbarInvisibleForSmallRoom()
	{
		...
		assertFalse(soundbar.isVisible());
	...
	public void testCompactSpeakerInvisibleForDifferentType()
	{
		...
		assertFalse(compactSpeaker.isVisible());
	}
	...
	public void testTowerSpeakerInVisibleForDifferentType()
	{
		...
		assertFalse(towerSpeaker.isVisible());
	...
	public void testTowerSpeakerVisibleForType()
	{
		...
		final CsticModel towerSpeaker = audioSystemSubInstance.getCstic(HomeTheaterMockImpl.TOWER_SPEAKER);
		classUnderTest.checkCstic(model, audioSystemSubInstance, towerSpeaker);
		assertTrue(towerSpeaker.isVisible());
	...
	public void testCompactSpeakerVisibleForDifferentTypeAndSmallRoom()
	{
		...
		final CsticModel compactSpeaker = audioSystemSubInstance.getCstic(HomeTheaterMockImpl.SPEAKER_COMPACT);
		classUnderTest.checkCstic(model, audioSystemSubInstance, compactSpeaker);
		assertTrue(compactSpeaker.isVisible());
	...
	public void testSubInstancesCreated()
	{
		...
		assertEquals("CONF_HT_EXTRAS", instance.getName());
		assertEquals(1, instance.getCsticGroups().size());
	...
	@Test
	public void testCheckCsticBlurayCabinetWood()
	{
		final CsticModel cstic = extras.getCstic(HomeTheaterMockImpl.BLURAY_CABINET_WOOD);
		classUnderTest.checkCstic(model, extras, cstic);
		assertEquals(HomeTheaterMockImpl.BLURAY_CABINET_WOOD, cstic.getName());
		assertEquals(2, cstic.getAssignableValues().size());
		assertFalse(cstic.isMultivalued());
		assertTrue(cstic.isAllowsAdditionalValues());
		assertEquals(CsticModel.TYPE_STRING, cstic.getValueType());
	}

	@Test
	public void testCheckCsticExtraSpeakerWire()
	{
		final CsticModel cstic = extras.getCstic(HomeTheaterMockImpl.EXTRA_SPEAKER_WIRE);
		classUnderTest.checkCstic(model, extras, cstic);
		assertEquals(HomeTheaterMockImpl.EXTRA_SPEAKER_WIRE, cstic.getName());
		assertEquals(2, cstic.getAssignableValues().size());
		assertFalse(cstic.isMultivalued());
		assertTrue(cstic.isAllowsAdditionalValues());
		assertEquals(CsticModel.TYPE_INTEGER, cstic.getValueType());
		assertEquals("__", cstic.getEntryFieldMask());
	}
