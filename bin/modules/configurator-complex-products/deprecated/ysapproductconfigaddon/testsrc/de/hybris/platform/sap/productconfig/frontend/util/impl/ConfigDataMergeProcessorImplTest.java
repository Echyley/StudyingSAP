...
{
	private static final String GROUP_ID_A = "A";
	private static final String GROUP_ID_B = "B";
	@InjectMocks
	...
	private final List<UiGroupData> groups = new ArrayList<>();
	private final List<Integer> outdatedConflictGroupIndices = new ArrayList<>();
	...
	{
		prepareGroupList();
		outdatedConflictGroupIndices.add(Integer.valueOf(0));
		outdatedConflictGroupIndices.add(Integer.valueOf(1));
		classUnderTest.removeOutdatedConflictGroups(groups, outdatedConflictGroupIndices);
	...
	{
		checkRemoveOutdatedConflictGroups(0, GROUP_ID_B);
	}

	@Test
	public void testRemoveOutdatedConflictGroupsRemoveSecond()
	{
		checkRemoveOutdatedConflictGroups(1, GROUP_ID_A);
	}

	protected void checkRemoveOutdatedConflictGroups(final int index, final String groupIdToCheck)
	{
		prepareGroupList();
		outdatedConflictGroupIndices.add(Integer.valueOf(index));
		classUnderTest.removeOutdatedConflictGroups(groups, outdatedConflictGroupIndices);
		assertEquals(1, groups.size());
		assertEquals(groupIdToCheck, groups.get(0).getId());
	}

	protected void prepareGroupList()
	{
		final UiGroupData uiGroupData = new UiGroupData();
		uiGroupData.setId(GROUP_ID_A);
		final UiGroupData uiGroupData2 = new UiGroupData();
		uiGroupData2.setId(GROUP_ID_B);
		groups.add(uiGroupData);
		groups.add(uiGroupData2);
	}
	...
	private void calculateCurrentIndices(final String groupPath)
	{
		createTestDataForUpdateTargetIncomplete();

		classUnderTest.setTokenizerPath(new StringTokenizer(groupPath));
		classUnderTest.setTokenizerGroupId(new StringTokenizer(idMain + "," + idSub2, ","));
		classUnderTest.calculateCurrentIndicesForPathToDisplayGroup();
	}
	...
	{
		calculateCurrentIndices("groups(5).subGroups(0)");
	}

	@Test(expected = IllegalArgumentException.class)
	public void testUpdateGroupListCorruptPathNoNumber()
	{
		calculateCurrentIndices("groups[].subGroups[0].");
	}

	@Test(expected = IllegalArgumentException.class)
	public void testUpdateGroupListCorruptPath()
	{
		calculateCurrentIndices("groups]2[.subGroups]0[.");
	}
