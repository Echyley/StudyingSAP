...
public class ConfigDataMergeProcessorImpl implements ConfigDataMergeProcessor
{
	...
	public void completeInput(final ConfigurationData targetConfigData)
	{
		if (!targetConfigData.isInputMerged())
		{
			...
			configData.setGroupIdToDisplay(targetConfigData.getGroupIdToDisplay());
	...
	protected void populateChangedValue(final ConfigurationData target)
	{
		...
		fieldPath = fieldPath.replace("key", "value");
	...
	{
		final Set<String> outdatedGroupIds = outdatedConflictGroupIndices.stream().map(index -> groups.get(index).getId())
				.collect(Collectors.toSet());
		groups.removeIf(group -> outdatedGroupIds.contains(group.getId()));
	}
