...
/**
 * Unit test for {@link AnalyticsItemPopulator}
 */
@UnitTest
public class AnalyticsItemPopulatorTest
{
	...
	public void testPopulateCharacteristicsMaxThreshold()
	{
		...
		{
			cstic = createCstic(CSTIC_ID + ii);
			source.addCstic(cstic);
		}

		classUnderTest.populateCharacteristics(source, target);
		assertNotNull(target.getCharacteristics());
		assertEquals(3, target.getCharacteristics().size());
	}
