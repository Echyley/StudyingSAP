...
/**
 * Unit test for {@link CommerceExternalConfigurationStrategyImpl}
 */
@UnitTest
public class CommerceExternalConfigurationStrategyImplTest
{
	...
	@Test
	public void testCollectOnItemLevelClassNode()
	{
		//class nodes don't carry units
		subItem.getQuantity().setUnit(null);
		collectOnItemLevel();
	}

	@Test
	public void testCollectOnItemLevelClassNodeEmptyUnit()
	{
		//class nodes don't carry units, unit code is sent as blank from CPS
		subItem.getQuantity().setUnit("");
		collectOnItemLevel();
	}

	protected void collectOnItemLevel()
	{
		final Map<String, String> unitCodesAsMap = new HashMap<>();
		classUnderTest.collectOnItemLevel(subItem, unitCodesAsMap);
		assertTrue(unitCodesAsMap.isEmpty());
	}
