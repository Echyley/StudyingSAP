...
/**
 * Unit test for {@link RequestErrorHandlerImpl}
 */
@UnitTest
public class RequestErrorHandlerImplTest
{
	...
	private final List<String> groupList = Arrays.asList("GROUP1", "GROUP2");
	...
	public void testProcessUpdateConfigurationErrorEtag() throws ConfigurationEngineException
	{
		...
		when(clientEx.getCode()).thenReturn(Integer.valueOf(HttpStatus.PRECONDITION_FAILED.value()));
		try
		{
	...
	{
		final HttpException ex = new HttpException(Integer.valueOf(HttpStatus.NOT_FOUND.value()), MESSAGE_TEXT);
	...
	public void testCheckNotFound() throws ConfigurationEngineException
	{
		...
		final HttpException ex = new NotFoundException(Integer.valueOf(HttpStatus.NOT_FOUND.value()), MESSAGE_TEXT);
	...
	@Test
	public void testProcessGetItemWithGroupDetailsError() throws ConfigurationEngineException
	{
		expected.expect(ConfigurationEngineException.class);
		classUnderTest.processGetItemWithGroupDetailsError(forbiddenEx, CONFIG_ID, ITEM_ID, groupList);
		checkCacheCleanUp();
	}

	@Test
	public void testProcessGetItemWithGroupDetailsErrorNotFound() throws ConfigurationEngineException
	{
		expected.expect(ConfigurationNotFoundException.class);
		classUnderTest.processGetItemWithGroupDetailsError(notFoundEx, CONFIG_ID, ITEM_ID, groupList);
		checkCacheCleanUp();
	}
