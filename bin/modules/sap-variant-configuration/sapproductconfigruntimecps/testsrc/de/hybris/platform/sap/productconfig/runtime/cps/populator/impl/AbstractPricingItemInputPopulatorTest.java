...
/**
 * Unit test for {@link AbstractPricingItemInputPopulator}
 */
@UnitTest
public class AbstractPricingItemInputPopulatorTest
{
	...
	public void setup()
	{
		...
		when(pricingConfigurationParameter.getSalesOrganization()).thenReturn(SALES_ORG);
		when(pricingConfigurationParameter.getDistributionChannelForConditions()).thenReturn(DIST_CHAN);
		when(configurationParameterB2B.getCustomerNumber()).thenReturn(CUSTOMER_NUMBER);
		when(Boolean.valueOf(configurationParameterB2B.isSupported())).thenReturn(Boolean.TRUE);
		final UnitModel unitModel = new UnitModel();
		unitModel.setSapCode(UOM_ST);
		when(pricingConfigurationParameter.retrieveUnitIsoCode(unitModel)).thenReturn(UOM_ST);
		...
		when(pricingConfigurationParameter.retrieveCurrencyIsoCode(currencyModel)).thenReturn(CURRENCY_USD);
	}
	...
	public void testFillAttributes()
	{
		...
		assertTrue(
				isAttributePresent(SapproductconfigruntimecpsConstants.PRICING_ATTRIBUTE_SALES_ORG, target.getAttributes(), null));
		assertTrue(
				isAttributePresent(SapproductconfigruntimecpsConstants.PRICING_ATTRIBUTE_DIST_CHANNEL, target.getAttributes(), null));
		assertTrue(
				isAttributePresent(SapproductconfigruntimecpsConstants.PRICING_ATTRIBUTE_DIVISION, target.getAttributes(), null));
		assertTrue(isAttributePresent(SapproductconfigruntimecpsConstants.PRICING_ATTRIBUTE_DIVISION_ITEM, target.getAttributes(),
				null));
		assertTrue(isAttributePresent(SapproductconfigruntimecpsConstants.PRICING_ATTRIBUTE_CUSTOMER_NUMBER, target.getAttributes(),
				null));
		assertTrue(isAttributePresent(SapproductconfigruntimecpsConstants.PRICING_ATTRIBUTE_COUNTRY, target.getAttributes(), null));
		...
		assertTrue(isAttributePresent(SapproductconfigruntimecpsConstants.PRICING_ATTRIBUTE_CURRENCY, target.getAttributes(),
				CURRENCY_USD));
		assertTrue(isAttributePresent(SapproductconfigruntimecpsConstants.PRICING_ATTRIBUTE_MATERIAL_NUMBER, target.getAttributes(),
				null));
		assertTrue(isAttributePresent(SapproductconfigruntimecpsConstants.PRICING_ATTRIBUTE_PRSFD, target.getAttributes(), null));
	}

	protected boolean isAttributePresent(final String name, final List<Attribute> attributes, final String valueToCheck)
	{
		for (final Attribute att : attributes)
		{
			if (name.equals(att.getName()))
			{
				...
				{
					return valueToCheck.equals(att.getValues().get(0));
				}
				return true;
			}
		}
	...
	public void testGetCurrentDateAsStringLength()
	{
		...
		assertEquals("Length must be " + AbstractPricingItemInputPopulator.FORMATTED_DATE_LENGTH,
				AbstractPricingItemInputPopulator.FORMATTED_DATE_LENGTH, dateAsString.length());
	}
