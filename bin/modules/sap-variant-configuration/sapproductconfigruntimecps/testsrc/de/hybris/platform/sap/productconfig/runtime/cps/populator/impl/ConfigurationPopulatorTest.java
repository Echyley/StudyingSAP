...
import de.hybris.platform.sap.productconfig.runtime.cps.model.runtime.CPSEngine;
...
/**
 * Unit test for {@link ConfigurationPopulator}
 */
@UnitTest
public class ConfigurationPopulatorTest
{
	...
	@Mock
	private MasterDataContext mockedCtxt;
	...
	public void initialize()
	{
		...
		given(mockedInstanceConverter.convertWithContext(Mockito.same(rootItem),
				(MasterDataContext) argThat(matchContextContainingSameKbContainer()))).willReturn(new InstanceModelImpl());


	}

	protected ArgumentMatcherMDC<MasterDataContext> matchContextContainingSameKbContainer()
	{
		return new ArgumentMatcherMDC<MasterDataContext>();
	}
	...
	{
		source.setKbBuild(Integer.valueOf(123));
		classUnderTest.populateCoreAttributes(source, target, mockedCtxt);
		assertEquals("123", target.getKbBuildNumber());
	}

	@Test
	public void testPopulateCoreAttributesResolutionTypeAVC()
	{
		source.setEngine(CPSEngine.AVC);
		classUnderTest.populateCoreAttributes(source, target, mockedCtxt);
		assertTrue(target.hasImmediateConflictResolution());
	}

	@Test
	public void testPopulateCoreAttributesResolutionTypeCPS()
	{
		source.setEngine(CPSEngine.CPS);
		classUnderTest.populateCoreAttributes(source, target, mockedCtxt);
		assertFalse(target.hasImmediateConflictResolution());
	}

	@Test
	public void testPopulateCoreAttributesResolutionTypeNoEngine()
	{
		source.setEngine(null);
		classUnderTest.populateCoreAttributes(source, target, mockedCtxt);
		assertFalse(target.hasImmediateConflictResolution());
	}
	...
	public class ArgumentMatcherMDC<MasterDataContext> implements ArgumentMatcher
	{

		@Override
		public boolean matches(final Object argument)
		{
			return ((de.hybris.platform.sap.productconfig.runtime.cps.populator.impl.MasterDataContext) argument)
					.getKbCacheContainer() == kbContainer;
		}
	}
}
