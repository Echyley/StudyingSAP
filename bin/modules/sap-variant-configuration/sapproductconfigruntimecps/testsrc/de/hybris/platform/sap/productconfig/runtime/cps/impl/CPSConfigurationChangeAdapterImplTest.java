...
/**
 * Unit test for {@link CPSConfigurationChangeAdapterImpl}
 */
@UnitTest
public class CPSConfigurationChangeAdapterImplTest
{
	...
	private static final String ANOTHER_CSTIC_NAME = "anotherCsticName";
	...
	private List<SolvableConflictModel> prepareConflicts(final String csticName, final String csticValue, final String instanceId)
	{
		...
		final List<ConflictingAssumptionModel> assumptions = new ArrayList<>();
	...
	public void testCollectRetractValueWithEmptyConflicts()
	{
		...
		assertEquals("Result list should be empty: ", 0, result.size());
	}

	@Test
	public void testCollectRetractValueWithAnotherCsticName()
	{
		final CsticModel csticModel = createCsticString(ANOTHER_CSTIC_NAME);
		csticModel.setInstanceId(INSTANCE_ID);
	...
	{
		final CsticModel csticModel = createCsticString(CSTIC_NAME);
		csticModel.setInstanceId("anotherInstanceId");
		final List<CPSValue> result = classUnderTest.collectRetractValue(csticModel);
		assertNotNull("Result should not be null: ", result);
		assertEquals("Result list should be empty: ", 0, result.size());
	}
	...
	public void testProcessCsticWithoutConflictAndRetractFromConfiguration()
	{
		...
		csticModel.setRetractTriggered(true);
		csticModel.setMultivalued(false);
		csticModel.setSingleValue(ANOTHER_CSTIC_NAME);
		final List<SolvableConflictModel> conflicts = new ArrayList<>();
		classUnderTest.processCstic(changedCstics, csticModel, conflicts);
		assertNotNull(changedCstics);
		assertEquals(1, changedCstics.size());
		assertEquals(ANOTHER_CSTIC_NAME, changedCstics.get(0).getValues().get(0).getValue());
	}

	@Test
	public void testAddCloudEngineValueWithNullValue()
	{
		final List<CPSValue> valuesList = new ArrayList<CPSValue>();
		classUnderTest.addCloudEngineValue(null, false, valuesList);
		assertTrue("Values list should be empty: ", CollectionUtils.isEmpty(valuesList));
	}
	...
	public void testAddCloudEngineValueWithDefinedValue()
	{
		...
		final List<CPSValue> valuesList = new ArrayList<CPSValue>();
		classUnderTest.addCloudEngineValue(value, false, valuesList);
		assertTrue("Values list should not be empty: ", CollectionUtils.isNotEmpty(valuesList));
		assertTrue("Values list should contain 1 entry: ", valuesList.size() > 0);
		assertEquals("Vaule should be " + value + ": ", value, valuesList.get(0).getValue());
	}
	...
	public void testCollectRetractValueForMultivaluedCstic()
	{
		...
		csticModel.setRetractTriggered(true);
		csticModel.setMultivalued(true);
		csticModel.setAssignedValues(createValueList(2));
		csticModel.setAssignableValues(createValueList(5));
		classUnderTest.collectRetractValueTriggeredInConfiguration(csticModel, changedValues);
		assertEquals("List of changed values should contain 2 entries", 2, changedValues.size());
	}

	private void collectRetractValueAndVerify(final String singleValue)
	{
		final List<CPSValue> changedValues = new ArrayList<>();
		final CsticModel csticModel = createCsticString(CSTIC_NAME);
		csticModel.setInstanceId(INSTANCE_ID);
		csticModel.setRetractTriggered(true);
		csticModel.setMultivalued(false);
		csticModel.setSingleValue(singleValue);
		classUnderTest.collectRetractValueTriggeredInConfiguration(csticModel, changedValues);
		assertEquals("List of changed values should be empty", 0, changedValues.size());
	}

	@Test
	public void testCollectRetractValueForSingleValuedCsticWithNullValue()
	{
		collectRetractValueAndVerify(null);
	}

	@Test
	public void testCollectRetractValueForSingleValuedCsticWithEmptyValue()
	{
		collectRetractValueAndVerify("");
	}
