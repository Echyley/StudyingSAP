...
public class AbstractPricingItemInputPopulator
{
	...
	public static final int FORMATTED_DATE_LENGTH = 10;
	...
	protected void fillPricingAttributes(final String id, final PricingItemInput target)
	{
		...
		final String currency = pricingConfigurationParameter.retrieveCurrencyIsoCode(currencyModel);
	...
	protected String getPricingDateAsString(final ConfigurationRetrievalOptions context)
	{
		...
		{
			return context.getPricingDate().toInstant().toString().substring(0, FORMATTED_DATE_LENGTH);
		}
		return Instant.now().toString().substring(0, FORMATTED_DATE_LENGTH);
	}
	...
	protected ProductInfo createProductInfo(final String uom)
	{
		...
		productInfo.setAlternateProductUnits(new ArrayList<>());
		return productInfo;
	}
