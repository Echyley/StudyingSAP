...
public class RequestErrorHandlerImpl implements RequestErrorHandler
{
	...
	{
		if (ex instanceof ClientException && ex.getCode() == HttpStatus.PRECONDITION_FAILED.value())
		{
	...
	{
		if (ex instanceof ClientException && ex.getCode() == HttpStatus.PRECONDITION_REQUIRED.value())
		{
	...
	@Override
	public CPSItem processGetItemWithGroupDetailsError(final HttpException ex, final String configId, final String itemId,
			...
			throws ConfigurationEngineException
	{
		...
		throw new ConfigurationEngineException(
				"Get item with group details failed for item " + itemId + " with group list " + groupList.toString()
