...
public class CPSConfigurationOrderEntryMapper implements ConfigurationOrderEntryMapper
{
	...
	{
		return entry.getExternalConfiguration() != null
				&& !getMappingDecisionStrategy().isA2AServiceMappingActive(entry.getOrder());
	}
			...
			final String entryNumber)
	{
		...
		for (final CPSExternalItem sourceItem : sourceItems)
		{
			...
			targetItem.setObjectKey(sourceItem.getBomPositionObjectKey().getId());
			targetItem.setObjectType(sourceItem.getBomPositionObjectKey().getType());
	...
	protected MappingDecisionStrategy getMappingDecisionStrategy()
	{
