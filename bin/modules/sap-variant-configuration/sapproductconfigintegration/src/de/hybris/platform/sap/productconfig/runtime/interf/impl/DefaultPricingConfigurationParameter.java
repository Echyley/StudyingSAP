...
public class DefaultPricingConfigurationParameter implements PricingConfigurationParameter
{
	...
	{
		final Set<CurrencyModel> currencies = getBaseStoreService().getCurrentBaseStore().getCurrencies();
		final CurrencyModel currencyModel = currencies.stream().filter(c -> sapCode.equals(c.getSapCode())).findFirst()
				.orElse(null);
