...
public class DefaultPricingConfigurationParameterTest
{
	...
	private final Set<CurrencyModel> currencies = new HashSet();
	...
	public void setUp()
	{
		...
		when(sapConfiguration.getSapproductconfig_enable_pricing()).thenReturn(Boolean.TRUE);
		when(sapConfiguration.getSapproductconfig_show_deltaprices()).thenReturn(Boolean.TRUE);
		when(sapConfiguration.getSapproductconfig_show_baseprice_and_options()).thenReturn(Boolean.TRUE);
		currencies.add(currencyModel);
		when(baseStore.getCurrencies()).thenReturn(currencies);
	...
	public void testConvertSapToIsoCode()
	{
		when(currencyModel.getSapCode()).thenReturn(CURRENCY_SAP_CODE);
		when(currencyModel.getIsocode()).thenReturn(CURRENCY_ISO_CODE);
		final String result = classUnderTest.convertSapToIsoCode(CURRENCY_SAP_CODE);
		assertNotNull(result);
		assertEquals(CURRENCY_ISO_CODE, result);
	}

	@Test
	public void testConvertSapToIsoCodeNoMatchingCurrency()
	{
		final String result = classUnderTest.convertSapToIsoCode(CURRENCY_SAP_CODE);
		assertNull(result);
	}
