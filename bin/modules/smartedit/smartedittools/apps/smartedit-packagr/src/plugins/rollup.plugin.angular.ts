...
const componentRegex =
    /@Component\(\s?{([\s\S]*)}\s?\)$|type:\s?Component,\s?args:\s?\[\s?{([\s\S]*)},\s?\]/gm;
...
async function transformHelper(
    filter: (id: string | any) => boolean,
    map: string,
    source: string,
    options: any
    ...
    const replaceTemplateUrl = async (subString: string, url: string) => {
        ...
        if (fileExt === 'js' && options.replace === true) {
            ...
            console.info(
                `templateUrl in file ${map} has been changed from ${chalk.green(
                    subString
                )}  to ${chalk.green(toReplace)}`
            );
        }
        return toReplace;
    };
    ...
    const replaceStyleUrls = async (subString: string, urls: string) => {
        ...
        if (fileExt === 'js' && options.replace === true) {
            ...
            console.info(
                `styleUrls in file ${map} has been changed from ${chalk.green(
                    subString
                )} to ${chalk.green(toReplace)}`
            );
        }
    ...
    const replaceModuleId = (_subString: string, _moduleId: string) => {
        hasReplacements = true;
        return '';
    };
    ...
    while ((match = componentRegex.exec(source)) !== null) {
        ...
        replacement = temp
            ...
            .replace(moduleIdRegex, replaceModuleId);
    ...
    let sourceMap: any;
    const result: TransformResult = { code: magicString.toString(), map: sourceMap };
...
export const angularPlugin = (options: any) => {
    ...
    return {
        ...
        async transform(source: string, map: string) {
            return transformHelper(filter, map, source, options);
        }
    };
};
