...
export class RenderService extends IRenderService {
    constructor(
        ...
        private readonly jQueryUtilsService: JQueryUtilsService,
        private readonly experienceService: IExperienceService,
        private readonly seNamespaceService: SeNamespaceService,
        ...
        this.crossFrameEventService.subscribe(EVENT_THEME_SAVED, () => {
    ...
    refreshOverlayDimensions(element: JQuery = null): void {
        ...
        const children: any =
            this.componentHandlerService.getFirstSmartEditComponentChildren(element);
    ...
    updateComponentSizeAndPosition(element: HTMLElement, componentOverlayElem?: HTMLElement): void {
        ...
        const cloneId: string = this._buildShallowCloneId(
            componentElem.attr(ID_ATTRIBUTE),
            componentElem.attr(TYPE_ATTRIBUTE),
            componentElem.attr(CONTAINER_ID_ATTRIBUTE)
        );
    ...
    private _cloneComponent(el: HTMLElement): void {
        ...
        const smarteditComponentId: string = element.attr(ID_ATTRIBUTE);
        const smarteditComponentType: string = element.attr(TYPE_ATTRIBUTE);
        const smarteditContainerId: string = element.attr(CONTAINER_ID_ATTRIBUTE);
    ...
    private _markSmartEditBootstrap(): void {
