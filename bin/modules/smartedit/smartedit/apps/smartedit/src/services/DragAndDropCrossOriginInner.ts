...
export class DragAndDropCrossOrigin extends IDragAndDropCrossOrigin {
    ...
    private onDnDCrossOriginStart = (eventId: string): void => {
        this.dragAndDropScrollingService.toggleThrottling(
            this.polyfillService.isEligibleForThrottledScrolling()
        );
    };

    private onTrackMouseInner = (eventId: string, eventData: IMousePosition): void => {
    ...
    private onDropElementInner = (eventId: string, mousePosition: IMousePosition): void => {
        if (this.currentElementHovered.length) {
    ...
    private dispatchDragEvent(
        ...
        (evt as any).dataTransfer = {
            ...
            setData(_type: string, val: object): void {
                this.data[_type] = val;
            },
            getData(_type: string): any {
