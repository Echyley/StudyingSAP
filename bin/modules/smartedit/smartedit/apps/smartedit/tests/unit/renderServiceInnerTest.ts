...
import { RenderService, SeNamespaceService } from 'smartedit/services';
import {
    annotationService,
...
import { jQueryHelper, ElementForJQuery } from 'testhelpers';
...
describe('renderServiceInner', () => {
    ...
    const ATTR_DATA_SMARTEDIT_COMPONENT_ID = 'data-smartedit-component-id';
    ...
    beforeEach(() => {
        ...
        experienceService.buildRefreshedPreviewUrl.and.callFake(() => {
    ...
    describe('renderComponent', () => {
        ...
        it('extracts the slot ID for the given component ID and type', () => {
            ...
            expect(slotElement.attr).toHaveBeenCalledWith(ATTR_DATA_SMARTEDIT_COMPONENT_ID);
        });
    ...
    describe('renderSlots', () => {
        ...
        describe('when no slot ids are provided', () => {
            ...
            it('returns a rejected promise with an error message', async () => {
                await expectAsync(actual).toBeRejectedWith(EXPECTED_EXCEPTION_NO_SLOT_IDS);
            });
        ...
        describe('when multiple slot IDs are provided and http request succeeds', () => {
            ...
            beforeEach((done) => {
                ...
                jQueryUtilsService.extractFromElement
                    .withArgs(root, EXPECTED_SLOT_1_SELECTOR)
                    .and.returnValue(fetchedSlotToRender1)
                    .withArgs(root, EXPECTED_SLOT_2_SELECTOR)
                    .and.returnValue(fetchedSlotToRender2);
                const slot1: jasmine.SpyObj<JQuery<ElementForJQuery>> =
                    jQueryHelper.wrap(EXPECTED_SLOT_1_SELECTOR);
                slot1.html.and.returnValue(originalSlotToReplace1);

                const slot2: jasmine.SpyObj<JQuery<ElementForJQuery>> =
                    jQueryHelper.wrap(EXPECTED_SLOT_2_SELECTOR);
                ...
                const jQuerySpy = jasmine.createSpy();
                jQuerySpy
                    .withArgs(EXPECTED_SLOT_1_SELECTOR)
                    .and.returnValue(originalSlotToReplace1)
                    .withArgs(EXPECTED_SLOT_2_SELECTOR)
                    .and.returnValue(originalSlotToReplace2);
                jQuery = jQueryHelper.jQuery(jQuerySpy);
                initService(jQuery);
            ...
            it('should call the httpClient.get with the current page URL', () => {
                expect(httpClient.get as any).toHaveBeenCalledWith(
    ...
    describe('toggleOverlay', () => {
        ...
        it('should make the SmartEdit overlay visible when passed true', () => {
            ...
            expect(overlay.css as any).toHaveBeenCalledWith('visibility', 'visible');
        });

        it('should make the SmartEdit overlay hidden when passed false', () => {
            renderService.toggleOverlay(false);
            expect(overlay.css as any).toHaveBeenCalledWith('visibility', 'hidden');
        });
    });
    ...
    describe('refreshOverlayDimensions', () => {
        ...
        beforeEach(() => {
            ...
            jQuerySpy
                ...
                .withArgs(element)
                .and.returnValue(wrappedElement);
            jQuery = jQueryHelper.jQuery(jQuerySpy);
    ...
    describe('updateComponentSizeAndPosition', () => {
        ...
        beforeEach(() => {
            ...
            spyOn(renderService as any, '_getParentInOverlay')
                .withArgs(originalElement)
                .and.returnValue(parentOverlay);
        });
        ...
        beforeEach(() => {
            ...
            originalElement.attr.and.callFake(((attribute: any) => {
                return componentAttrFake(attribute);
            }) as any);
    ...
    describe('_cloneComponent', () => {
        ...
        beforeEach(() => {
            ...
            element.get.and.returnValue({
                attributes: [
                    ...
                    {
                        nodeName: ATTR_DATA_SMARTEDIT_COMPONENT_ID,
            ...
            componentHandlerService.getParent.withArgs(element).and.returnValue(parentOverlay);
        ...
        it('should copy attributes with "data-smartedit" prefix from the element to the wrapped SmartEdit decorator wrapper', () => {
            ...
            expect(componentDecorator.attr.calls.argsFor(0)).toEqual([
                ATTR_DATA_SMARTEDIT_COMPONENT_ID,
                MOCK_COMPONENT_ID
            ]);
    ...
    describe('_getParentInOverlay', () => {
        ...
        describe('when called with a SmartEdit component', () => {
            ...
            beforeEach(() => {
                expectedOverlay = {};
                componentHandlerService.getOverlayComponent.and.returnValue(
                    expectedOverlay as JQuery
                );
                componentHandlerService.getOverlay.and.returnValue(overlay as JQuery);
                initService();
            });
            ...
            it('should fetch the overlay', () => {
                componentHandlerService.getParent.and.returnValue({} as JQuery);
    ...
    describe('renderPage', () => {
        ...
        beforeEach(() => {
            ...
            componentHandlerService.getOverlay.and.returnValue(overlay);
            initService();
            spyOn(renderService as any, '_resizeSlots');
            spyOn(renderService as any, '_markSmartEditAsReady');
            spyOn(renderService, 'isRenderingBlocked').and.returnValue(Promise.resolve(true));
        });

        it('will publish overlay rerendered event', async () => {
            await renderService.renderPage(false);
            expect(crossFrameEventService.publish).toHaveBeenCalledWith('overlayRerendered');
        });

        it('call with isRerender:false will publish restartProcess', async () => {
            await renderService.renderPage(false);
        ...
        it('call with isRerender:true will publish restartProcess', async () => {
            await renderService.renderPage(true);
            expect(systemEventService.publish).toHaveBeenCalledWith(
                CONTRACT_CHANGE_LISTENER_PROCESS_EVENTS.RESTART_PROCESS
            );
        });
        ...
        it('when current perspective is "Preview" should not mark SmartEdit as ready', async () => {
            await renderService.renderPage(false);
            expect((renderService as any)._markSmartEditAsReady).toHaveBeenCalled();
        });

        // render page should only resize the slots with 'isRerender' true,otherwise '_resizeSlots' will be executed in preview mode which is not expected
        it('should resize the slots', () => {
            renderService.renderPage(true);
            expect((renderService as any)._resizeSlots).toHaveBeenCalled();
        });
    });
    ...
    describe('destroyComponent', () => {
        ...
        beforeEach(() => {
            ...
            component.attr.and.callFake(componentAttrFake);
    ...
    function componentAttrFake(element: string): string {
        switch (element) {
            case ID_ATTRIBUTE:
