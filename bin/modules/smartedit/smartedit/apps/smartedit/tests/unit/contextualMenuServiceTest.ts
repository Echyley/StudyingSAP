...
describe('test contextualMenuService', () => {
    ...
    const nameI18nKey = 'some.key';
    const componentAttributes = {} as ComponentAttributes;
    const element: JQuery<HTMLElement> = {} as JQuery<HTMLElement>;
    const slotId = 'slotId';
    const slotUuid = 'slotUuid';
    const isComponentHidden = false;

    @Component({ selector: 'dummy-component' })
    ...
    it('getContextualMenuItems will provide the dom element', () => {
        ...
        const promise: Promise<ContextualMenu> =
            contextualMenuService.getContextualMenuItems(config);
