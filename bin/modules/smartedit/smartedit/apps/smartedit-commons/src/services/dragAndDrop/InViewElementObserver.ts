...
import { nodeUtils, stringUtils, MUTATION_TYPES } from '../../utils';
...
export class InViewElementObserver {
    ...
    elementFromPoint(mousePosition: IMousePosition): Element {
        ...
        return (
            ...
            this.getInViewElements().find((component) =>
                nodeUtils.isPointOverElement(mousePosition, component as HTMLElement)
            )
        );
    }
    ...
    private _aggregateAddedOrRemovedNodes(
        ...
        return lodash.uniqWith(
            entries,
            (entry1: HTMLElement, entry2: HTMLElement) =>
                entry1.contains(entry2) || entry2.contains(entry1)
        );
    }
     ...
     */
    private _aggregateMutationsOnClass(mutations: MutationRecord[], addedOnes: boolean): Node[] {
     ...
     */
    private _mutationObserverCallback(mutations: MutationRecord[]): void {
     ...
     */
    private _updateQueue(entry: IntersectionObserverEntry): void {
     ...
     */
    private _newMutationObserver(callback: MutationCallback): MutationObserver {
     ...
     * wrapping for test purposes
     */
    private _newIntersectionObserver(callback: IntersectionObserverCallback): IntersectionObserver {
    ...
    private _isEligibleComponent(component: Element): boolean {
        return this.yjQuery(component).is(this._getJQuerySelector());
    }

    private _getEligibleElements(): HTMLElement[] {
    ...
    private _getComponentIndexInQueue(component: Element): number {
        return this.componentsQueue.findIndex(function (obj) {
            return component === obj.component;
        });
    }

    private _isInDOM(component: Element): boolean {
