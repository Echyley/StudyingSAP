...
export class SelectComponent<T extends SelectItem> implements OnInit, OnChanges {
    @HostBinding('class.se-select--single') get isSingleCss(): boolean {
        return !this.multiSelect;
    }

    @HostBinding('class.se-select--multi') get isMultiCss(): boolean {
        return this.multiSelect;
    }

    @HostBinding('class.se-select--paged') get isPagedCss(): boolean {
     ...
     */
    @Input() keepModelOnReset = false;

    /** Whether component is disabled  */
    @Input() @HostBinding('class.is-disabled') isReadOnly = false;
    ...
    /**
     * additionalBodyClass: Whether the popover container needs an extra class for styling.
     */
    @Input() additionalBodyClass?: string;
    ...
    /**
     * used for additional body class of popover
     *
     * @internal
     */
    public fullAdditionalBodyClass: string;
     ...
     * @internal
     */
    private readonly api: SelectApi;
    ...
    public fetchEntity(modelId: string): Promise<T> {
        return this.fetchStrategy.fetchEntity(modelId).then((item: T) => {
            if (!item) {
     ...
     * It will only notify a parent component that the model has changed.
    ...
    /** @internal */
    private internalFetchAll(mask = ''): Promise<T[]> {
        ...
        if (!this.multiSelect) {
            ...
            if (this.fetchStrategy.fetchEntities) {
                ...
                promise = this.fetchStrategy
                    .fetchEntities(this.model as string[])
                    .then((items: T[]) => {
     ...
     * @internal
     */
    private setSelected(items: T | T[], updateModel = true): void {
        this.selected = items;
