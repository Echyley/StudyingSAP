...
describe('genericEditorMappingService - Common Mapping -', () => {
    ...
    it('GIVEN a mapping is defined WHEN getMapping is called THEN the mapping will be returned if there is a partial match', () => {
        ...
        expect(EXACT_MATCH_PREDICATE as any).not.toHaveBeenCalledWith(field.smarteditComponentType);
        expect(EXACT_MATCH_PREDICATE).toHaveBeenCalledWith(DISCRIMINATOR, field.qualifier);
    });
...
describe('genericEditorMappingService - Mapping Predicates - ', () => {
    beforeEach(() => {
        buildCommonSpyObjects();
    });
...
describe('genericEditorMappingService - Field Mapping - ', () => {
    beforeEach(() => {
        buildCommonSpyObjects();
    });
    ...
    it('WHEN _registerDefaultFieldMappings is called THEN all default mappings are added', () => {
        ...
        expect(
            editorFieldMappingService.getEditorFieldMapping(field, componentTypeStructure).component
        ).toEqual(ShortStringComponent);
        expect(
            editorFieldMappingService.getEditorFieldMapping(field, componentTypeStructure)
                .validators.notBlank
        ).toBeDefined();
...
describe('genericEditorMappingService - Tab Mapping - ', () => {
    beforeEach(() => {
        buildCommonSpyObjects();
    });
    ...
    it('WHEN getFieldTabMapping is called THEN it is delegated to _getMapping', () => {
        ...
        // WHEN
        const result = editorFieldMappingService.getFieldTabMapping(field, componentTypeStructure);
