...
let schemaComplierService: jasmine.SpyObj<SchemaCompilerService>;
...
describe('SchemaBuilderService - basic check', () => {
    beforeEach(() => {
...
describe('SchemaBuilderService - compileGroup', () => {
    beforeEach(() => {
        buildCommonSpyObjects();
    });

    it('will distribute the fields in fieldsMap according to their tab mapping and will sort the tabs', () => {
        const schema: GenericEditorSchema = {
            languages,
            uriContext,
            id: 'id',
        ...
        editorFieldMappingService.getEditorFieldMapping.and.callFake(
            (field: GenericEditorField, structure: GenericEditorStructure) => {
                switch (field.qualifier) {
                    ...
                    default:
                        throw new Error(
                            `unexpected qualifier ${field.qualifier} passed to editorFieldMappingService.getEditorFieldMapping`
                        );
                }
            }
        );
        ...
        editorFieldMappingService.getFieldTabMapping.and.callFake(
            (field: GenericEditorField, structure: GenericEditorStructure) => {
                switch (field.qualifier) {
                    ...
                    default:
                        throw new Error(
                            `unexpected qualifier ${field.qualifier} passed to editorFieldMappingService.getFieldTabMapping`
                        );
                }
            }
        );
...
function buildCommonSpyObjects() {
    ...
    schemaComplierService = jasmine.createSpyObj('schameComplierService', ['compileGroup']);

    schemaBuilderService = new GenericEditorStateBuilderService(
        editorFieldMappingService,
        genericEditorTabService,
        translateService,
        schemaComplierService
    );
