...
const structureApi = '/cmswebservices/types/:smarteditComponentType';
const contentApi = `/cmswebservices/catalogs/${CONTEXT_CATALOG}/versions/${CONTEXT_CATALOG_VERSION}/items`;
...
let fetchEnumDataHandler: jasmine.SpyObj<FetchEnumDataHandler>;
...
const calledTimes2 = 2;
describe('test GenericEditor class - initializing', () => {
    beforeEach(() => {
    ...
    it('successful init will call load, pushEditorToStack with given editorStackId, and publish loaded event', async () => {
        ...
        expect(systemEventService.publishAsync).toHaveBeenCalledTimes(calledTimes2);
...
describe('test GenericEditor class - data loading', () => {
    beforeEach(() => {
        buildCommonSpyObjects();
    });
    ...
    it('GIVEN generic editor with modified component WHEN onSuccess is called and the backend returns empty response THEN it returns original payload and calls updateCallback', () => {
        // GIVEN
        const editor = new GenericEditor({
            smarteditComponentType,
            smarteditComponentId,
...
describe('test GenericEditor class - validation and submit ', () => {
    beforeEach(() => {
        buildCommonSpyObjects();
    });

    it(
        'GIVEN there are errors in one or more fields in the current editor detected externally - ' +
            'WHEN GENERIC_EDITOR_UNRELATED_VALIDATION_MESSAGES_EVENT handler is called - ' +
            'THEN the editor must display those validation errors',
        () => {
            // Arrange
            const failure: BackendErrorResponse<BackendValidationErrors> = {
                error: {
...
describe('test GenericEditor class - Some Check', () => {
    beforeEach(() => {
        buildCommonSpyObjects();
    });
    ...
    it('GIVEN generic editor When I make changes to the content The save button is enabled', () => {
        const editor = new GenericEditor({
            smarteditComponentType,
            smarteditComponentId,
            element,
        ...
        expect(editor.isSubmitDisabled()).toEqual(false);
    });
});
...
function buildCommonSpyObjects() {
    ...
    fetchEnumDataHandler.findByMask.and.callFake((): any => {
