...
describe('outer perspectiveService', () => {
    const mockRouteChangeSuccess = jasmine.createSpy();
    ...
    const SMARTEDIT_PERSPECTIVE = 'smartedit-perspectives';
    ...
    describe('initialization', () => {
        ...
        it('_registerEventHandlers will register event handlers', () => {
            ...
            systemEventService.subscribe.calls.reset();
            crossFrameEventService.subscribe.calls.reset();
            routingService.routeChangeSuccess.calls.reset();
            ...
            expect(routingService.routeChangeSuccess.calls.count()).toBe(1);
            expect(mockRouteChangeSuccess).toHaveBeenCalledWith(jasmine.any(Function));
            ...
            const routeChangeCallback = mockRouteChangeSuccess.calls.argsFor(0)[0];
            const mockAJSRouterEvent = { current: { originalPath: STORE_FRONT_CONTEXT } };
            routeChangeCallback(mockAJSRouterEvent);

            expect((perspectiveService as any)._clearPerspectiveFeatures).toHaveBeenCalled();
        });

        it('GIVEN perspectives are initialized', async () => {
            // WHEN
    ...
    describe('register', () => {
        ...
        it('GIVEN that perspective configuration has features, THEN register pushes to the features list a Perspective instantiated from configuration and sends a notification', async () => {
            // GIVEN
        ...
        it('should send EVENT_PERSPECTIVE_ADDED event if register is called the first time and EVENT_PERSPECTIVE_UPDATED is register is called the second time with the same key', async () => {
            // WHEN
            await perspectiveService.register({
                key: 'somekey',
                nameI18nKey: 'somenameI18nKey',
        ...
        it('does not override existing perspectives but merges features and nested perspectives', async () => {
            // GIVEN
            await perspectiveService.register({
                key: 'somekey',
                nameI18nKey: 'somenameI18nKey',
    ...
    describe('_fetchAllFeatures', () => {
        it('collects all features of nested perspectives, returns a set of unique features, and log message if a nested perspective does not exist', async () => {
            // GIVEN
            const perspective1 = {
                key: 'persp1',
                nameI18nKey: 'persp1',
    ...
    describe('refreshPerspective', () => {
        it('should select the default perspective if there is no active perspective', async () => {
            // GIVEN
            ...
            expect(storageService.getValueFromLocalStorage).toHaveBeenCalledWith(
                SMARTEDIT_PERSPECTIVE,
                true
            );
        });

        it('will publish a perspective refreshed event after a successful refreshPerspective', async () => {
            // GIVEN
            const perspective0 = {
                key: 'persp0',
                nameI18nKey: 'persp0',
    ...
    describe('refreshPerspective - not permitted perspective', () => {
        it('WILL select NONE_PERSPECTIVE if the active perspective is not permitted', async () => {
    ...
    describe('switchTo', () => {
        const SE_FAKE_PERMISSION = 'se.fake.permission';
        beforeEach(() => {
        ...
        it('WILL silently do nothing if trying to switch to the same perspecitve as the activated one', async () => {
            // GIVEN
            await perspectiveService.register({
                key: 'aperspective',
        ...
        it('NONE_PERSPECTIVE WILL publish a rerender/false and hide the wait modal', async () => {
            // WHEN
        ...
        it('WILL activate all (nested) features of a perspective and notify to rerender', async () => {
            // GIVEN
            const perspective0 = {
                key: 'persp0',
                nameI18nKey: 'persp0',
        ...
        it('WILL disable features of previous perspective not present in new one and activate features of new perspective not present in previous one', async () => {
            // GIVEN
            const perspective0 = {
                key: 'persp0',
                nameI18nKey: 'persp0',
        ...
        it('will publish a perspective changed event after a successful switch', async () => {
            // GIVEN
            const perspective0 = {
                key: 'persp0',
                nameI18nKey: 'persp0',
        ...
        it('enable function called when feature has permission', async () => {
            // GIVEN
            featureService.getFeatureProperty.and.returnValue(
                Promise.resolve([SE_FAKE_PERMISSION])
            );
            permissionService.isPermitted.and.returnValue(Promise.resolve(true));
            ...
            expect(permissionService.isPermitted).toHaveBeenCalledWith([
                {
                    names: [SE_FAKE_PERMISSION]
                }
            ]);
            expect(featureService.enable).toHaveBeenCalled();
        });

        it('when getFeatureProperty for permissions returns undefined, an empty array is used for permission.names', async () => {
            // GIVEN
        ...
        it('enable function is not called when feature does not have permission', async () => {
            // GIVEN
            featureService.getFeatureProperty.and.returnValue(
                Promise.resolve([SE_FAKE_PERMISSION])
            );
        ...
        it('getPerspectives should return all perspectives after permissions are removed', async () => {
            // GIVEN
    ...
    describe('selectDefault', () => {
        it('WILL select NONE_PERSPECTIVE if none is found in smartedit-perspectives cookie', async () => {
            // GIVEN
            spyOn(perspectiveService, 'switchTo').and.returnValue(Promise.resolve());
        ...
        it('WILL select the perspective by disabling all features first if a perspective already exists', async () => {
            // GIVEN
            spyOn(perspectiveService, 'switchTo').and.returnValue(Promise.resolve());
            spyOn(perspectiveService as any, '_disableAllFeaturesForPerspective');
        ...
        it('WILL select NONE_PERSPECTIVE if perspective stored in cookie has no permission', async () => {
            // GIVEN
            const PERMISSION1 = 'permission1';
        ...
        it('WILL select perspective stored in cookie if perspective has permission', async () => {
            // GIVEN
            const PERMISSION1 = 'permission1';
            const PERSPECTIVE_WITHOUT_PERMISSIONS = {
                key: 'persp0',
    ...
    describe('getActivePerspective', () => {
        ...
        it('returns active perspective', async () => {
            // GIVEN
    ...
    describe('isEmptyPerspectiveActive', () => {
        it('returns true if active perspective is NONE_PERSPECTIVE', async () => {
            // GIVEN
        ...
        it('returns false if active perspective is not NONE_PERSPECTIVE', async () => {
            expect(await perspectiveService.isEmptyPerspectiveActive()).toBe(false);
        });
    });

    describe('getPerspectives', () => {
        it('returns perspectives for which user is granted permission', async () => {
            // GIVEN
            const PERMISSION1 = 'permission1';
    ...
    describe('getActivePerspectiveKey', function () {
        it(' will return the key of the perspective loaded in the storefront', async () => {
            // GIVEN
        ...
        it('returns null if no perspective is loaded', async () => {
            const result = await perspectiveService.getActivePerspectiveKey();
