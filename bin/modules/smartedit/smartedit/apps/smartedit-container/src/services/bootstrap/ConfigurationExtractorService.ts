...
import { Cloneable, LogService, Payload } from 'smarteditcommons';
...
export class ConfigurationExtractorService {
            ...
            .filter((extendingApp) => {
                ...
                if (index === -1) {
                    this.logService.error(
                        `Application ${extendingApp.name} located at ${extendingApp.location} is ignored because it extends an unknown application '${extendingApp.extends}'; SmartEdit functionality may be compromised.`
                    );
                }
                return index > -1;
            });
    ...
    private _addExtendingAppsInOrder(
        ...
        extendingApps.forEach((extendingApp) => {
            const index = simpleApps.findIndex(
                (item: Module) => item.name === extendingApp.extends
            );
            if (index > -1) {
    ...
    private _getAppsAndLocations(
        ...
        const appsAndLocations = lo
            .map(configurations, (value: Cloneable, prop: string) => ({ key: prop, value }))
            .reduce(
