...
export class PerspectiveService extends IPerspectiveService {
    ...
    getPerspectives(): Promise<IPerspective[]> {
        ...
        return Promise.all(promises).then((results: boolean[]) =>
            this.immutablePerspectives.filter(
    ...
    switchTo(key: string): Promise<void> {
        ...
        featuresFromPreviousPerspective
            .filter(
                (featureKey: string) =>
                    !featuresFromNewPerspective.some((f: string) => featureKey === f)
            )
        ...
        featuresFromNewPerspective
            .filter(
                (feature: string) =>
                    !featuresFromPreviousPerspective.some((f: string) => feature === f)
            )
            .forEach((featureKey: string) => {
    ...
    private _addPerspectiveSelectorWidget(configuration: IPerspective): void {
    ...
    private _addDefaultPerspectives(): void {
        this.register({
    ...
    private _registerEventHandlers(): void {
        ...
        this.routingService.routeChangeSuccess().subscribe((event: RouteNavigationEnd) => {
    ...
    private _validate(configuration: IPerspective): void {
    ...
    private _fetchAllFeatures(perspective: IPerspective, holder: string[]): void {
        if (!holder) {
            holder = [];
        }
     ...
     */
    private _handleUnloadEvent(nextPerspectiveKey: string): void {
        if (
    ...
    private _changeActivePerspective(newPerspectiveKey: string): IPerspective {
    ...
    private _disableAllFeaturesForPerspective(perspective: IPerspective): void {
        const features: string[] = [];
    ...
    private _clearPerspectiveFeatures(): Promise<void> {
