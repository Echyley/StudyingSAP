...
export class IframeManagerService {
    ...
    loadPreview(homePageOrPageFromPageList: string, previewTicket: string): Promise<void> {
        ...
        return promiseToResolve.then((previewURL: string) => {
            ...
            if (this._mustLoadAsSuch(homePageOrPageFromPageList)) {
                 ...
                 */
                return (
                    isCrossOrigin ? Promise.resolve({}) : this._getPageAsync(pageInPreviewMode)
                ).then(
                    () =>
    ...
    applyDefault(): void {
        const defaultDeviceSupport = DEVICE_SUPPORTS.find(
            (deviceSupport: DeviceSupport) => deviceSupport.default
        );
        const defaultDeviceOrientation = DEVICE_ORIENTATIONS.find(
            (deviceOrientation: DeviceOrientation) => deviceOrientation.default
        );
    ...
    private _getPageAsync(url: string): Promise<any> {
