...
export class StorageService extends IStorageService {
    ...
    private _removeAllAuthTokens(): void {
        const sessions: ISessionAuth = this.getAuthTokens();
    ...
    private _setSmarteditSessions(sessions: ISessionAuth): void {
        const sessionsJSONString = Buffer.from(
            decodeURIComponent(encodeURIComponent(JSON.stringify(sessions)))
        ).toString('base64');
    ...
    private _setValueInLocalStorage(cookieName: string, value: any, encode: boolean): void {
