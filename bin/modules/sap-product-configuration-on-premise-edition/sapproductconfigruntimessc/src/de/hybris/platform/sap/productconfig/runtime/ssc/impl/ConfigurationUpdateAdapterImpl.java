...
public class ConfigurationUpdateAdapterImpl implements ConfigurationUpdateAdapter
{
	...
	protected static final boolean WITHOUT_DESCRIPTION = true;
	...
	{
		final List<CsticValueDelta> csticValueDeltas;
		if (trackingEnabled)
		{
	...
	{
		final ICsticValueData[] csticValues = icd.getCsticValues();
		if (csticValues != null && csticValues.length > 0)
		{
			final List<ICsticValueData> icvdList = Arrays.asList(csticValues);
			...
			final IConfigSessionClient session, final String configId) throws IpcCommandException
	{
		...
		{
			final String assumptionId = conflictAdapter.getAssumptionId(csticModel, configModel);
			if (StringUtils.isNotBlank(assumptionId))
			{
	...
	protected void setObjectMapper(final ObjectMapper mapper)
	{
		this.mapper = mapper;
	}
}
