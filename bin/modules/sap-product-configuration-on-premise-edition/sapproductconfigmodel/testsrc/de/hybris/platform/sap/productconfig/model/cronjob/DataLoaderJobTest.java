...
public class DataLoaderJobTest
{
	...
	public void setUp()
	{
		...
		willReturn(mockedDataloaderManager).given(classUnderTest)
				.createDataloaderManager(Mockito.any(IDataloaderConfiguration.class));
	...
	{
		// should be invoke-able
		classUnderTest.getMessageListener()
				.messageReported(new Message(DataLoaderJobTest.class.getSimpleName(), new Date().toString(), null, "a test Message"));
	}
	...
	protected void givenAbortNeeded()
	{
		...
		Mockito.when(mockedCronJobService.getRunningOrRestartedCronJobs())
				.thenReturn(Collections.singletonList(anotherDataLoaderCronJob));
	}
	...
	public void testPerformStartDeltaWrongMode()
	{
		...
		dataLoaderCronJobModel.setTriggerMode(DataLoadTriggerMode.STARTINITIAL);
		classUnderTest.startDeltaLoad(DataLoadTriggerMode.STARTINITIAL, sapConfigurationModel,
				classUnderTest.getDataLoaderManagerContainer().getDataLoaderManager());
	}
