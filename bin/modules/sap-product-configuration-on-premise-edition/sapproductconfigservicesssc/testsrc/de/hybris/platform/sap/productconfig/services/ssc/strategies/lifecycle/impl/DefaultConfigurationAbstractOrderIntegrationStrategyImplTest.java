...
public class DefaultConfigurationAbstractOrderIntegrationStrategyImplTest
{
	...
	public void initialize()
	{
		...
		classUnderTest.setConfigurationLifecycleStrategy(configurationLifecycleStrategy);
		...
		when(configurationService.retrieveConfigurationOverview(CONFIG_ID)).thenReturn(configModelFromSession);
	...
	{
		Mockito.when(configurationService.retrieveConfigurationOverview(CONFIG_ID)).thenReturn(configModelFromExternal);
	...
	{
		final String oldExternalConfiguration = "Old External Configuration";
		final String newExternalConfiguration = "New External Configuration";
		final AbstractOrderEntryModel entry = Mockito.spy(new AbstractOrderEntryModel());
		Mockito.when(entry.getPk()).thenReturn(entryKey);
		Mockito.when(configurationVariantUtil.isCPQNotChangeableVariantProduct(Mockito.any(ProductModel.class))).thenReturn(false);
		Mockito.when(entry.getProduct()).thenReturn(productModel);
		entry.setExternalConfiguration(oldExternalConfiguration);
		given(productModel.getCode()).willReturn(PRODUCT_ID);
		given(configurationService.retrieveExternalConfiguration(CONFIG_ID_NEW)).willReturn(newExternalConfiguration);
		classUnderTest.refreshCartEntryConfiguration(entry);
		Mockito.verify(sessionAccessService).setConfigIdForCartEntry(entryKey.toString(), CONFIG_ID_NEW);
		Mockito.verify(configurationLifecycleStrategy).releaseSession(CONFIG_ID);
		assertEquals(newExternalConfiguration, entry.getExternalConfiguration());
	}
