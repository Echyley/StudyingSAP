...
import static de.hybris.platform.assistedserviceservices.constants.AssistedserviceservicesConstants.DEFAULT_SUGGESTED_CUSTOMERS_LIMIT_VAL;
...
public class DefaultAssistedServiceFacade implements AssistedServiceFacade
{
	...
	public static final String ERROR_NO_CUSTOMER_OR_CART_ID_PROVIDED = "asm.emulate.error.no_customer_or_cart_id_provided";
	protected static final int DEFAULT_SESSION_TIMER = 600;
	protected static final String SESSION_CART_KEY = "cart";
	protected static final String AGENT_TIMER_KEY = "agentTimer";
	protected static final String CREATE_DISABLED_KEY = "createDisabled";
	protected static final String EMULATE_BY_ORDER_KEY = "emulateByOrder";
	protected static final String ASM_ERROR_MESSAGE_KEY = "asm_message";
	protected static final String ASM_ERROR_MESSAGE_ARGS_KEY = "asm_message_args";
	protected static final String ASM_AGENT_STORE = "asm_agent_store";
	protected static final String CURRENT_USER = "current";
	// Abbreviation 'AS' in this class means Assisted Service
	private static final Logger LOG = Logger.getLogger(DefaultAssistedServiceFacade.class);
	...
	private Converter<CustomerModel, AutoSuggestionCustomerData> suggestionCustomerConverter;
	...
	public void emulateCustomer(final String customerId, final String cartId, final String orderId) throws AssistedServiceException
	{
		...
		if (StringUtils.isBlank(customerId) && StringUtils.isBlank(cartId))
		{
			throw new AssistedServiceAgentNoCustomerAndCartIdException(
					Localization.getLocalizedString(ERROR_NO_CUSTOMER_OR_CART_ID_PROVIDED));
		}
	...
	{
		return (getUserService().isAnonymousUser(customer) || !customer.isLoginDisabled())
				&& (customer.getDeactivationDate() == null || customer.getDeactivationDate().after(new Date()));
	}
	...
	@Override
	public void loginAssistedServiceAgentSAML(final String username, final LoginToken token) throws AssistedServiceException
	{
		try
		{
			...
			if (!UserManager.getInstance().checkPassword(getModelService().getSource(agent), token))
			{
		...
		{
			LOG.debug(e.getMessage(), e);
			throw new AssistedServiceAgentBadCredentialsException("Could not find token encoder.");
		}
	}
	...
	protected void loginAssistedServiceAgent(final UserModel agent) throws AssistedServiceException
	{
		...
		getSessionService().setAttribute(AssistedservicefacadesConstants.ACTING_USER_UID, agent.getUid());
	...
	public void logoutAssistedServiceAgent() throws AssistedServiceException
	{
		...
		if (Config.getBoolean(AssistedservicefacadesConstants.AS_CUSTOMER_LOGOUT_WITH_AGENT_LOGOUT, false))
		{
			stopEmulateCustomer();
		}
		...
		getSessionService().removeAttribute(AssistedservicefacadesConstants.ACTING_USER_UID);
	...
	public CustomerData getCustomerByOrder(final String orderId)
	{
		...
		if (orderModel == null)
		{
			throw new BadRequestException("Wrong orderId:" + orderId);
		}
	...
	public List<CustomerData> getSuggestedCustomerList(final String username) throws AssistedServiceException
	{
		...
		pageableData.setPageSize(Config.getInt(AssistedserviceservicesConstants.DEFAULT_SUGGESTED_CUSTOMERS_LIMIT,
				DEFAULT_SUGGESTED_CUSTOMERS_LIMIT_VAL));
		return this.getCustomers(username, pageableData);
	}

	@Override
	public List<AutoSuggestionCustomerData> getSuggestedCustomerData(String username)
	{
		try
		{
			validateSession();
			return getCustomerSuggestions(username);
		}
		catch (final AssistedServiceException e)
		{
			LOG.debug(e.getMessage(), e);
	...
	@Override
	public List<AutoSuggestionCustomerData> getCustomerSuggestions(final String searchTerm)
	{
		final PageableData pageableData = new PageableData();
		pageableData.setCurrentPage(0);
		pageableData.setPageSize(Config.getInt(DEFAULT_SUGGESTED_CUSTOMERS_LIMIT, DEFAULT_SUGGESTED_CUSTOMERS_LIMIT_VAL));
		final var customerSuggestions = getAssistedServiceService().getCustomers(searchTerm, pageableData).getResults();
		if(CollectionUtils.isEmpty(customerSuggestions))
		{
			return Collections.singletonList(new AutoSuggestionCustomerData());
		}
		return getSuggestionCustomerConverter().convertAll(customerSuggestions);
	}
	...
	{
		getAssistedServiceService().bindCustomerToCartWithoutEmulating(customerId, cartId);
	}
	 ...
	 */
	protected void verifyAgentCanLogin(final UserModel asmAgent) throws AssistedServiceException
	{
		if (!assistedServiceService.isAgentCanLogin(asmAgent))
		{
			throw new AssistedServiceException("User cannot login");
		}
	}
	/**
	 * Verify that provided user participate in a parent AS agent group.
	...
	@Override
	public List<PointOfServiceData> getStoresByAgentId(final String agentId)
	{
		final List<PointOfServiceModel> pointOfServiceModels = assistedServiceService.getAllStoresForAgent(getAccurateAgentId(agentId));

		if (CollectionUtils.isNotEmpty(pointOfServiceModels))
		{
			return getPointOfServiceConverter().convertAll(pointOfServiceModels);
		}
	...
	protected String getAccurateAgentId(final String agentId)
	{
		if (agentId.equals(CURRENT_USER))
		{
			return getAsmSession().getAgent().getUid();
		}
		return agentId;
	}
	...
	protected Converter<CustomerModel, AutoSuggestionCustomerData> getSuggestionCustomerConverter()
	{
		return suggestionCustomerConverter;
	}

	@Required
	public void setSuggestionCustomerConverter(
			final Converter<CustomerModel, AutoSuggestionCustomerData> suggestionCustomerConverter)
	{
		this.suggestionCustomerConverter = suggestionCustomerConverter;
	}
