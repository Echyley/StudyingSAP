...
public class CustomerProfileDataPopulator implements Populator<CustomerModel, CustomerProfileData>
{
	...
	private void populateBillingAddress(final CustomerModel userModel, final CustomerProfileData customerProfileData)
	{
		...
		{
			populateBillingAddressFromDefaultPaymentInfo(userModel, customerProfileData);
		}
		...
		{
			populateBillingAddressFromOrderPaymentInfo(userModel, customerProfileData);
		}
	}

	private void populateBillingAddressFromOrderPaymentInfo(final CustomerModel userModel, final CustomerProfileData customerProfileData)
	{
		...
		if (optionalPayment.isPresent())
		{
			...
			final AddressModel billingAddressModel = paymentInfoModel.getBillingAddress();
			if (billingAddressModel != null)
			{
				customerProfileData.setBillingAddress(getAddressConverter().convert(billingAddressModel));
				if (billingAddressModel.getPhone1() != null)
				{
					customerProfileData.setPhone1(billingAddressModel.getPhone1());
				}
			}
		}
	}

	private void populateBillingAddressFromDefaultPaymentInfo(final CustomerModel userModel, final CustomerProfileData customerProfileData)
	{
		customerProfileData
	...
	protected List<CCPaymentInfoData> getPaymentInfoList(final CustomerModel userModel)
	{
		...
		List<CreditCardPaymentInfoModel> paymentInfoModels = new ArrayList<>();
