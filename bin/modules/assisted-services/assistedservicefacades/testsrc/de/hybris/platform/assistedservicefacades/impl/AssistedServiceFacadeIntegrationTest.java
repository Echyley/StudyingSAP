...
public class AssistedServiceFacadeIntegrationTest extends ServicelayerTransactionalTest
{
	...
	private static final String ASAGENT_1_UID = "asagent_1";
	...
	{
		final List<PointOfServiceData> result = assistedServiceFacade.getStoresByAgentId(ASAGENT_1_UID);
		assertThat(result).hasSize(2).extracting("name", "displayName").contains(tuple("Nakano", "Nakano"), tuple("ws_warehouse_n", "Warehouse North"));
	}
	...
	public void testAssistedServiceAgentStore()
	{
		...
		assertEquals("asm.emulate.no_stores", agentStore);
	}
