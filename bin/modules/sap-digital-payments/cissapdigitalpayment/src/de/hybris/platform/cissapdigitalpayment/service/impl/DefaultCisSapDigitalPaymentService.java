...
public class DefaultCisSapDigitalPaymentService implements CisSapDigitalPaymentService
{
	...
	public Observable<CisSapDigitalPaymentRegistrationUrlResult> getRegistrationUrl() throws TimeoutException
	{
		...
		Map<String, String> configMap = this.createDigitalPaymentConfigurationMap(sapDigitalPaymentConfig);

		//Prepare routing parameter
		DigitalPaymentsRoutingParameters routingParameters = new DigitalPaymentsRoutingParameters();
		routingParameters.setCompanyCode(configMap.get(SAP_DIGITAL_PAYMENT_COMPANY_CODE_KEY));
		routingParameters.setCustomerCountry(configMap.get(SAP_DIGITAL_PAYMENT_CUSTOMER_COUNTRY_KEY));
		routingParameters.setCustomParameterValue(configMap.get(SAP_DIGITAL_PAYMENT_CUSTOM_PARAM_KEY));
		routingParameters.setPaymentMethod(configMap.get(SAP_DIGITAL_PAYMENT_PAYMENT_METHOD_KEY));
		...
		//registrationRequestModel.setConsumedRedirectUrl("https://dummyUrl");
		registrationRequest.setRoutingParameters(routingParameters);
		registrationRequest.setSessionType("ONLINE");

		CisSapDigitalPaymentRegistrationUrlResult registrationUrlResult = getCisSapDigitalPaymentClient(sapDigitalPaymentConfig).getRegistrationUrl(registrationRequest);
		return Observable.just(registrationUrlResult);
	}
	 ...
	 *
	 * @param refundRequests - List of CisSapDigitalPaymentRefundRequest
	 * @param sapDigitalPaymentConfig - SAP Digital payment configuration
