...
public class SapRevenueCloudProductInboudHelperTest
{
	...
	public void setUp()
	{
		...
		lenient().when(sapMarketToCatalogMappingModelGenericDao.find()).thenReturn(getDummyMarketToCatalogList());
	}
	...
	public void checkForDefaultCatalogForNonExistingMarketMapping()
	{
		...
		lenient().when(catalogService.getCatalogForId(defaultCatalogId)).thenReturn(defCatalog);
	...
	public void checkForSubscriptionPricePlanEndDate()
	{
		...
		lenient().when(catalogVersionService.getCatalogVersion(any(String.class), any(String.class))).thenReturn(cv);
		lenient().when(sapRevenueCloudProductService.getSubscriptionPricePlanForId(any(String.class), eq(cv))).thenReturn(sp1);
		sapRevenueCloudProductInboudHelper.processSubscriptionPricePlanEndDate("sp1Id", new CatalogVersion());
		lenient().doNothing().when(modelService).save(any(SubscriptionPricePlanModel.class));
