...
<%@ taglib prefix="orderaddon" tagdir="/WEB-INF/tags/addons/sapsubscriptionaddon/responsive/order" %>
<%@ taglib prefix="common" tagdir="/WEB-INF/tags/responsive/common" %>
<spring:htmlEscape defaultHtmlEscape="true" />
<c:set var="quoteState" value="${quoteData.state}"/>
<ul class="item__list item__list__cart">
   <li class="hidden-xs hidden-sm">
      <ul class="item__list--header">
         ...
         <c:if test="${quoteData.enableDiscount eq true}">
            <li class="item__price">
               <spring:theme code="basket.page.discountPercent"/>
            </li>
         </c:if>
   ...
   <c:forEach items="${quoteData.entries}" var="orderEntry" varStatus="loop">
      <c:choose>
         <c:when test="${not empty orderEntry.product.subscriptionTerm}">
      ...
      <li class="item__list--item">
         ...
         <div class="item__price">
            ...
            <c:choose>
               <c:when test="${subscriptionProduct}">
                  <c:choose>
                     <c:when test="${quoteState eq 'BUYER_SUBMITTED'}">
                        <c:forEach items="${orderEntry.product.price.oneTimeChargeEntries}" var="oneTimeChargeEntry">
                     ...
                     <c:otherwise>
                        ...
                        <div>
                           <spring:theme code="text.product.payoncheckout"/>
                        </div>
                     </c:otherwise>
                  </c:choose>
                  <c:if test="${quoteData.enableDiscount eq true}">
                     <button id="viewCharges" class="btn btn-small btn-link" style="font: bold 10.5px Arial;" data-entry-number="${orderEntry.entryNumber}">
                        <spring:theme code="text.account.subscription.viewCharges" text="View Charges"/>
                     </button>
                     <c:choose>
                        <c:when test="${quoteData.state eq 'EXPIRED' or quoteData.state eq 'BUYER_OFFER' or quoteData.state eq 'CANCELLED' or quoteData.state eq'BUYER_ACCEPTED' or quoteData.state eq 'ORDERED' or quoteData.state eq 'BUYER_ORDERED'}">
                           <div style="display:none" id="${orderEntry.entryNumber}">
                              <orderaddon:cpqDiscountedRatePlanPanel orderEntry="${orderEntry}" />
                           </div>
                        </c:when>
                        <c:otherwise>
                           <div style="display:none" id="${orderEntry.entryNumber}">
                              <orderaddon:cpqRatePlanPanel orderEntry="${orderEntry}" />
                           </div>
                        </c:otherwise>
                     </c:choose>
                  </c:if>
         ...
         <c:if test="${quoteData.enableDiscount eq true}">
            <c:choose>
               <c:when test="${subscriptionProduct}">
                  <c:if test="${empty orderEntry.discountPercent}">
                     <div class="item__price">
                        <span class="visible-xs visible-sm">
                           <spring:theme code="basket.page.itemDiscount"/>
                           :
                        </span>
                        -
                     </div>
                  </c:if>
                  <c:if test="${orderEntry.discountPercent == 'null'}">
                     <div class="item__price">
                        <span class="visible-xs visible-sm">
                           <spring:theme code="basket.page.itemDiscount"/>
                           :
                  ...
                  <c:if test="${not empty orderEntry.discountPercent and (orderEntry.discountPercent != 'null')}">
                     <div class="item__price">
                        <span class="visible-xs visible-sm">
                           <spring:theme code="basket.page.itemDiscount"/>
                           :
                        </span>
                        ${fn:escapeXml(orderEntry.discountPercent)}&nbsp;%
                     </div>
                  </c:if>
               </c:when>
               <c:otherwise>
         ...
         <div class="item__price">
            <span class="visible-xs visible-sm">
               <spring:theme code="basket.page.itemTotal"/>
               :
            </span>
