            ...
            <table class="orderhistory-list-table responsive-table">
               ...
               <tr class="responsive-table-item">
                  ...
                  <td class="responsive-table-cell" style="width:1px;white-space:no-wrap;">
                     <div id="table">
                        <c:set var="pricingPrametersCount" value="${fn:length(subscriptionData.pricing.pricingParameters)}"/>
                        <c:choose>
                           <c:when test="${pricingPrametersCount eq 0}">
                              <button id="detailedCharges" class="btn btn-link" style="font: bold 9px Arial;" data-rate-plan-id="${fn:escapeXml(subscriptionData.ratePlanId)}">
                                 <spring:theme code="text.account.subscription.viewCharges" text="View Charges"/>
                              </button>
                              <div style="display:none" id="pricepanel_${fn:escapeXml(subscriptionData.ratePlanId)}">
                                 <subscription:subscriptionRatePlanPanel subscription="${subscriptionData}" />
                              </div>
                           </c:when>
                           <c:otherwise>
                              <button id="detailedChargesWithDiscounts" type="button"
                                 class="btn btn-link"
                                 style="font: bold 9px Arial;"
                                 data-view-charges-url="${fn:escapeXml(viewChargesUrl)}" >
                                 <spring:theme code="text.account.subscription.viewCharges" text="View Charges"/>
                              </button>
                           </c:otherwise>
                        </c:choose>
         ...
         <c:if test="${(subscriptionData.status eq 'ACTIVE') and (withdrawalPeriodEndDate gt currentDate)}">
            ...
            <td>
               <form:form id="subscriptionWithdrawalForm"
                  ...
                  <button id="withdrawRCSubscription" type="submit"
...
<br><br>
<form:form id="subscriptionRatePlanForm" name="subscriptionRatePlanForm" action="${fn:escapeXml(viewChargesUrl)}" method="post" modelAttribute="subscriptionRatePlanForm">
   <%-- List of External Object References --%>
   <c:forEach items="${subscriptionData.externalObjectReferences}" var="externalObjectReference" varStatus="status">
      <input type="hidden" name="externalObjectReferences[${fn:escapeXml(status.index)}].externalIdTypeCode" value="${fn:escapeXml(externalObjectReference.externalIdTypeCode)}"/>
      <input type="hidden" name="externalObjectReferences[${fn:escapeXml(status.index)}].externalId" value="${fn:escapeXml(externalObjectReference.externalId)}"/>
      <input type="hidden" name="externalObjectReferences[${fn:escapeXml(status.index)}].externalSystemId" value="${fn:escapeXml(externalObjectReference.externalSystemId)}"/>
   </c:forEach>
   <%-- Pricing --%>
   <c:set var="pricing" value="${subscriptionData.pricing}"/>
   <input type="hidden" name="pricing.pricingDate" value="${pricing.pricingDate}" />
   <%-- List of Pricing Parameters --%>
   <c:forEach items="${pricing.pricingParameters}" var="pricingParameter" varStatus="status">
      <input type="hidden" name="pricing.pricingParameters[${fn:escapeXml(status.index)}].code" value="${pricingParameter.code}"/>
      <input type="hidden" name="pricing.pricingParameters[${fn:escapeXml(status.index)}].value" value="${pricingParameter.value}"/>
   </c:forEach>
</form:form>
