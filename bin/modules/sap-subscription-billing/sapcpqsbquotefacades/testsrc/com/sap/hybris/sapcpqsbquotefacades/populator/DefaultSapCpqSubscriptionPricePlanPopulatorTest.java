...
public class DefaultSapCpqSubscriptionPricePlanPopulatorTest {
	...
	@Mock
	Converter<CpqPricingParameterModel, CpqPricingParameterData> cpqPricingParameterConverter;
	...
	public void setUp(){
		...
		//mock
		Mockito.lenient().when(sapSubscriptionBillingEffectivePriceService.getSubscriptionEffectivePricePlan(any(ProductModel.class),any(Date.class))).thenReturn(subscriptionPricePlanModel);
		Mockito.lenient().when( recurringChargeEntryConverter.convert(any())).thenReturn(recurringChargeEntryData);
		Mockito.lenient().when( perUnitUsageChargeConverter.convert(any())).thenReturn(perUnitUsageChargeData);
		Mockito.lenient().when( volumeUsageChargeConverter.convert(any())).thenReturn(volumeUsageChargeData);
		Mockito.lenient().when( oneTimeChargeEntryConverter.convert(any())).thenReturn(oneTimeChargeEntryData);
		Mockito.lenient().when( usageChargeConverter.convert(any())).thenReturn(usageChargeData);
		Mockito.lenient().when( cpqPricingPrameterConverter.convert(any())).thenReturn(cpqPricingParameterData);
		Mockito.lenient().when( sapCpqSbFetchQuoteDiscounts.getCurrentQuoteVendorDiscounts(any())).thenReturn(java.util.Optional.ofNullable(quoteModel));
	}
