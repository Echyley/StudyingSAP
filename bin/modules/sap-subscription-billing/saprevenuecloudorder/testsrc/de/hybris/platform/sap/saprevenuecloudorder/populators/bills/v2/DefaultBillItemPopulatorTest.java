...
public class DefaultBillItemPopulatorTest {
    ...
    public void setup(){
        ...
        //Charge
        charge.setMetricId("SomeMetricId");
    ...
    public void populate_success_CQ0() {
        ...
        List<UsageChargeData> usageChargeDataList = subscriptionBillingData.getCharges();
        Assert.assertEquals(charges.size(), usageChargeDataList.size());
        for(UsageChargeData usageChargeDataValue: usageChargeDataList){
            Assert.assertEquals(charge.getConsumedQuantity().getValue(),usageChargeDataValue.getUsage());
            Assert.assertEquals(usageUnitModel.getName(),usageChargeDataValue.getName());
            Assert.assertEquals(usageUnitModel.getNamePlural(),usageChargeDataValue.getUsageUnit().getId());
            Assert.assertEquals(planElementModel.getName(),usageChargeDataValue.getName());
            Assert.assertEquals(charge.getRatingPeriod().getEnd(),usageChargeDataValue.getToDate());
            Assert.assertEquals(charge.getRatingPeriod().getStart(),usageChargeDataValue.getFromDate());
        }
    }
