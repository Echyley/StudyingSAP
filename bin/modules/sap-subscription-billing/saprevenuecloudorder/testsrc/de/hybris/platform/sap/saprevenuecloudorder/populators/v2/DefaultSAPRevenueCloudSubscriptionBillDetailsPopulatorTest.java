...
public class DefaultSAPRevenueCloudSubscriptionBillDetailsPopulatorTest {
    ...
    public void setup(){
        ...
        //Mocks
        lenient().doReturn(currencyModel).when(mockCommonI18NService).getCurrentCurrency();
        lenient().doReturn(priceData).when(mockPriceDataFactory).create(eq(PriceDataType.BUY), any(BigDecimal.class), any(String.class));
        lenient().doReturn(catalogVersionModel).when(mockCmsSiteService).getCurrentCatalogVersion();
        lenient().doReturn(subscriptionPricePlanModel).when(mockSapRevenueCloudProductService).getSubscriptionPricePlanForId(any(String.class),any(CatalogVersionModel.class));
        lenient().doNothing().when(mockProductModelProductDataPopulator).populate(eq(productModel),any(ProductData.class));
        lenient().doReturn(usageUnitModel).when(mockSapRevenueCloudProductService).getUsageUnitfromId(any(String.class));
        lenient().doReturn(currentCurrency).when(mockCommonI18NService).getCurrentCurrency();
        lenient().doReturn(priceData).when(mockPriceDataFactory).create(eq(PriceDataType.BUY), any(BigDecimal.class), eq("USD"));
        lenient().doReturn(planElementModel).when(mockSapRevenueCloudProductService).getRatePlanElementfromId(any(String.class));
    }
    ...
    public void populate_success_CQ1() {
        ...
        for(UsageChargeData usageChargeData: usageChargeDataList){
            ...
            Assert.assertEquals(charge.getConsumedQuantity().getValue(),Math.round(usageChargeData.getUsage()));
