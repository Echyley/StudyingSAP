...
public class DefaultSAPRevenueCloudPerUnitUsageChargePopulatorTest {
    ...
    public void populate_success(){
        ...
        //Mocks
        lenient().doNothing().when(spyDefaultSAPRevenueCloudPerUnitUsageChargePopulator).populate(eq(perUnitUsageChargeModel),eq(perUnitUsageChargeData));
        lenient().doReturn(usageChargeEntryData).when(mockChargeEntryModelPerUnitUsageChargeEntryDataConverter).convert(any(PerUnitUsageChargeEntryModel.class));
