...
public class DefaultSapRevenueCloudSubscriptionFacadeTest {
    ...
    public void setUp() {
        ...
        Mockito.lenient().when(mockCustomerFacade.getCurrentCustomer()).thenReturn(customerData);
        Mockito.lenient().when(mockUserService.getCurrentUser()).thenReturn(this.customerModel);
        defaultSapRevenueCloudSubscriptionFacade = Mockito.spy(defaultSapRevenueCloudSubscriptionFacade);
        Mockito.lenient().doReturn(configurationModel).when(defaultSapRevenueCloudSubscriptionFacade).getRevenueCloudConfiguration();

        try {
        	Mockito.lenient().when(mockSubscriptionService.getSubscription(anyString())).thenReturn(subscription);
    ...
    public void changePaymentDetailsAsCard_success() throws SubscriptionFacadeException, SubscriptionServiceException {
    	...
    	Mockito.lenient().when(mockSapRevenueCloudConfigurationModel.getPaymentMethod()).thenReturn(paymentCard);
    	Mockito.lenient().when(mockSubscriptionService.updatePayment(eq(subscriptionData.getId()), any(PaymentRequest.class)))
        ...
        Mockito.lenient().when(mockCustomerAccountService.getCreditCardPaymentInfoForCode(any(CustomerModel.class), any(String.class))).thenReturn(ccPaymentInfoModel);

        //Execute
        defaultSapRevenueCloudSubscriptionFacade.changePaymentDetailsAsCard(subscriptionData, paymentCardToken);
        Mockito.verify(defaultSapRevenueCloudSubscriptionFacade).changePaymentDetailsAsCard(subscriptionData, paymentCardToken);
    }

    @Test
    public void changePaymentDetailsAsInvoice_success() throws SubscriptionFacadeException, SubscriptionServiceException {
    	Mockito.lenient().when(mockSapRevenueCloudConfigurationModel.getInvoiceMethod()).thenReturn(paymentInvoice);
    	Mockito.lenient().when(mockSubscriptionService.updatePayment(eq(subscriptionData.getId()), any(PaymentRequest.class)))
            .thenReturn(new PaymentResponse());

        defaultSapRevenueCloudSubscriptionFacade.changePaymentDetailsAsInvoice(subscriptionData);
        Mockito.verify(defaultSapRevenueCloudSubscriptionFacade).changePaymentDetailsAsInvoice(subscriptionData);
    }

    @Test
    public void reverseCancellation_success() throws SubscriptionFacadeException, SubscriptionServiceException {
        //Setup
    	Mockito.lenient().when(mockSubscriptionService
        ...
        Mockito.verify(defaultSapRevenueCloudSubscriptionFacade).reverseCancellation(subscriptionData);
    }
