...
public abstract class AbstractAutomatedWorkflowTemplateJob implements AutomatedWorkflowTemplateJob
{
	...
	{
		final List<WorkflowDecisionModel> workflowDecisionList = (List<WorkflowDecisionModel>) workflowActionModel.getDecisions();
	...
	{
		if (b2bRegistrationModel.getCustomer() != null)
		{
			return b2bRegistrationModel.getCustomer();
		}
		else
		{
			return getUserService().getUserForUID(b2bRegistrationModel.getEmail().toLowerCase(LocaleHelper.getPersistenceLocale()),
					CustomerModel.class);
		}
	}
