...
public class DefaultSynchronizationFacade implements SynchronizationFacade
{
	...
	public SyncJobData createCatalogSynchronization(final SyncRequestData syncJobRequest)
	{
		...
		try
		{
			...
			final SyncItemJobModel lastSyncJob = latestSyncItemJobModel.get();
			final UserModel user = lastSyncJob.getSessionUser();

			// When the synchronization cron job is configured with a session user the system is forced to stop running as an admin (which enables
			// search restrictions). However, the system will execute as the current user in the UserService, not the session user in the cron job.
			// Thus, it is necessary to manually change the current user to match the one in the cron job's configuration.
			if (Objects.nonNull(user))
			{
				getUserService().setCurrentUser(user);
			}

			newSyncCronJob = createSyncCronJob(lastSyncJob);
			...
			final String targetCatalogVersionName)
	{
		...
		if (!synchronizations.isEmpty())
		{
			syncItemJobModel = synchronizations.stream()
					.filter(syncItemJob -> syncItemJob.getTargetVersion().getVersion().equals(targetCatalogVersionName))
