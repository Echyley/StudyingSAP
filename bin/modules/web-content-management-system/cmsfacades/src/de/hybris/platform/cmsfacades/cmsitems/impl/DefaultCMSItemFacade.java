...
public class DefaultCMSItemFacade implements CMSItemFacade
{
	...
	public static final String NAVIGATIONNODE_TYPE_CODE = "CMSNavigationNode";

	private static final Logger LOG = Logger.getLogger(DefaultCMSItemFacade.class);
	private static final Pattern MODEL_SAVING_EXCEPTION_REGEX = Pattern.compile("\\[([\\w,\\s]+)\\](?!\\.)*");
	private static final Pattern EXCEPTION_REGEX_START = Pattern.compile("\\[w*");
	private static final Pattern EXCEPTION_REGEX_END = Pattern.compile("\\]w*");
	...
	@Override
	public SearchResult<Map<String, Object>> findCMSItems(final CMSItemSearchData cmsItemSearchData, final PageableData pageableData)
	{
		getCmsItemAttributeFilterEnablerService().setMode(cmsItemSearchData.getFields());
	...
	{
		getCmsItemAttributeFilterEnablerService().setMode(mode);
	...
	{
		return getSessionSearchRestrictionsDisabler().execute(() -> buildCMSItemConvertAndPopulate(uuid, throwException));
	}
	...
	public Map<String, Object> createItem(final Map<String, Object> itemMap) throws CMSItemNotFoundException
	{
		...
		//ECP-5889 made children a partOf attribute, ignore partof constraint when creating CMSNavigationNode
		JaloSession.getCurrentSession().getSessionContext().setAttribute(IGNORE_PART_OF_CONSTRAINT_FOR_TYPE_CODE, NAVIGATIONNODE_TYPE_CODE);
	...
	protected Map<String, Object> buildCMSItemConvertAndPopulate(final String uuid, final boolean throwException)
	{
		try
		{
	...
	protected void initialUpdateValidation(final String uuid, final Map<String, Object> itemMap) throws CMSItemNotFoundException
	{
		...
		if (!getUniqueItemIdentifierService().getItemModel(uuid, CMSItemModel.class).isPresent()) {
			throw createCMSItemNotFoundException(uuid);
		}
			...
			final String permissionName)
	{
		...
		return getSessionSearchRestrictionsDisabler().execute(() -> {
			try
			{
				return buildContextForItem(itemMap);
			}
	...
	protected Map<String, Object> buildContextForItem(final Map<String, Object> itemMap)
	{
		setCloneContext(itemMap);
		setRestoreContext(itemMap);
		setOriginalItemContext(itemMap);
	...
	protected void transformValidationException(final ModelSavingException error)
	{
		...
		{
			final Matcher matcher = MODEL_SAVING_EXCEPTION_REGEX.matcher(error.getMessage());

			if (matcher.find())
			{
				final String match = matcher.group().replaceAll(EXCEPTION_REGEX_START.pattern(), "").replaceAll(EXCEPTION_REGEX_END.pattern(), "");
			...
			else if (error.getCause() instanceof ValidationViolationException)
			{
				((ValidationViolationException) error.getCause())
						.getHybrisConstraintViolations()
						.stream()
						.forEach(violation -> getValidationErrorsProvider().getCurrentValidationErrors()
	...
	protected Map<String, Object> validateItem(final Map<String, Object> itemMap)
	{
		...
		return getSessionSearchRestrictionsDisabler().execute(() -> {
			buildContextForValidItem(itemMap, uuidProvided, uidProvided);
	...
	protected void buildContextForValidItem(final Map<String, Object> itemMap, final Boolean uuidProvided, final Boolean uidProvided)
	{
		try
		{
			setCatalogInSession(itemMap);
	...
	/**
	 * Sets the original item context in the current session. The original item context is used to detect that the item
	 * is being modified by this facade class and that the {@link RelatedPageRejectionService} should not save this item
	 * part of its processing.
	 *
	 * @param source
	 */
	protected void setOriginalItemContext(final Map<String, Object> source)
	{
		final Map<String, Object> originalItemContext = new HashMap<>();

		getOriginalItemUuid(source)
				.flatMap(uuid -> getUniqueItemIdentifierService().getItemModel(uuid, CMSItemModel.class))
				.ifPresent((final CMSItemModel originalItem) -> {
					originalItemContext.put(CMSItemModel.UID, originalItem.getUid());
					originalItemContext.put(CMSItemModel.CATALOGVERSION, originalItem.getCatalogVersion());
				});

		getCmsAdminSiteService().setOriginalItemContext(originalItemContext);
	}

	/**
	 * Method returns original uuid for an item in the payload.
	 * Important: even if the UUID exists in the payload it's not always the uuid of the original item.
	 * For example: while cloning component the payload contains FIELD_PAGE_UUID but it can not be considered as original item
	 * @param source the source object.
	 * @return the optional UUID of the item, empty if there is no original uuid.
	 */
	protected Optional<String> getOriginalItemUuid(final Map<String, Object> source)
	{
		final Optional<String> uuidOpt = Optional.ofNullable(source.get(FIELD_UUID)).or(() -> Optional.ofNullable(source.get(FIELD_PAGE_UUID)))
				.map(Object::toString);

		if (Objects.isNull(source.get(FIELD_IS_SLOT_CUSTOM)))
		{
			return uuidOpt;
		}
		else
