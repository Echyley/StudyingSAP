...
public class ClonePagePopulator implements Populator<Map<String, Object>, ItemModel>
{
		...
		if (isNotBlank(sourcePageUUID) && !Objects.isNull(cloneComponents))
		{
			try
			{
				final Optional<Map<String, Object>> context = Optional
						.of(Maps.newHashMap(Cms2Constants.CLONE_ACTION, cloneComponents));
				...
				if ((Boolean)cloneComponents)
				{
					getRelationBetweenComponentsService().maintainRelationBetweenComponentsOnPage((AbstractPageModel) itemModel);
				}
			}
	...
	public void setRelationBetweenComponentsService(final RelationBetweenComponentsService relationBetweenComponentsService)
	{
