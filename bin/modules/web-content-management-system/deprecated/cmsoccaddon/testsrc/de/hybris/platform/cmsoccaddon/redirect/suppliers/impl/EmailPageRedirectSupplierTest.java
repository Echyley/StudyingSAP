...
{
	public static final String EMAIL_PAGE_UID = "EmailPageUid";
	...
	public void setup()
	{
		lenient().when(typeCodePredicate.test("EmailPage")).thenReturn(true);
		when(typeCodePredicate.negate()).thenReturn(typeCodeNegatedPredicate);
		lenient().when(typeCodeNegatedPredicate.test("EmailPage")).thenReturn(false);
	...
	public void shouldPopulateAllParametersForRedirectUrl()
	{
		...
		assertEquals(EMAIL_PAGE_UID, pageId);
	}
