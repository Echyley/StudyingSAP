...
public class DefaultCMSActivateBaseSiteInSessionStrategyTest
{
	...
	public void testThrowAnExceptionWhenNotEmptyContentCatalogs() throws BusinessException
	{
		...
		final Throwable expected = new IllegalStateException("blah");
		Mockito.doThrow(expected).when(catalogVersionService).setSessionCatalogVersions(argThat(new CollectionArgumentMatcher<CatalogVersionModel>(catalogVersion)));
		try
		{
			strategy.activate(siteModel);
	...
	public void testShouldThrowAnExceptionForFewStoresForSite() throws BusinessException
	{
		...
		Mockito
				...
				.setSessionCatalogVersions(
						argThat(new CollectionArgumentMatcher<CatalogVersionModel>(catalogVersionOne, catalogVersionTwo)));

		Mockito.doThrow(expected).when(catalogVersionService).setSessionCatalogVersions(
				argThat(new CollectionArgumentMatcher<CatalogVersionModel>(catalogVersionOne, catalogVersionTwo)));
	 ...
	 * in the test
	 */
	protected void throwExceptionIfInteraction() throws CMSItemNotFoundException
	{
...
private class CollectionArgumentMatcher<T> implements ArgumentMatcher<Collection>
{
