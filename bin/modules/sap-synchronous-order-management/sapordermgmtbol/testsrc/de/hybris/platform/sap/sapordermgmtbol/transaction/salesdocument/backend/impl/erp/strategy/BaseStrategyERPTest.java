...
public class BaseStrategyERPTest extends JCORecTestBase
{
	...
	public void init()
	{
		...
		classUnderTest.setMessageMapper(
				(BackendMessageMapper) genericFactory.getBean(SapordermgmtbolConstants.ALIAS_BEAN_BACKEND_MESSAGE_MAPPER));
		classUnderTest
				.setCustExit((ERPLO_APICustomerExits) genericFactory.getBean(SapordermgmtbolConstants.ALIAS_BEAN_BE_CUST_EXIT));
	...
	public void testCreateBufferForLogging()
	{
		...
		BaseStrategyERP.createBufferForLogging(functionName, tableForLogging, direction, builder);
		assertTrue(builder.indexOf(functionName) > 0);
		assertTrue(builder.indexOf("T_PROBCLSS") > 0);
