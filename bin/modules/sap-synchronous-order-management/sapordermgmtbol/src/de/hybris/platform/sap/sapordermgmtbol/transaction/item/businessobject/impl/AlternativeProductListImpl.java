...
public class AlternativeProductListImpl implements AlternativeProductList
{
	...
	{
		return (AlternativeProduct) genericFactory.getBean(SapordermgmtbolConstants.ALIAS_BEAN_ALTERNATIVE_PRODUCT);
	}
	 ...
	 */
	@Override
	public AlternativeProduct createAlternativProduct(final String systemProductId, final TechKey systemProductGUID,
			final String description, final String enteredProductIdType, final String substitutionReasonId)
	{
	 ...
	 * @return AlternativProduct the alternative product for the given index, or null if index is out of bounds
	 */
	 ...
	 */
	@Override
	public void addAlternativProduct(final int i, final AlternativeProduct altProd)
	{
	 ...
	 *           description of the system product
	 * @param enteredProductIdType
	 *           if the system product was found through determination, this specifies, as what the entered product id was
	 *           interpreted
	 * @param substitutionReasonId
	 *           if the system product is a substitute product, this is the id for the substitution reason
	 */
	@Override
	public void addAlternativProduct(final String systemProductId, final TechKey systemProductGUID, final String description,
	...
	{
		alternativProductList.clear();
		alternativProductList.addAll(alternativProductListData);
	}
	...
	public Object clone()
	{
		...
		// (not-deep) copy.
		myClone.alternativProductList = new ArrayList<AlternativeProduct>(alternativProductList.size());
		for (final AlternativeProduct altProd : alternativProductList)
		{
			myClone.alternativProductList.add((AlternativeProduct) altProd.clone());
		}
	...
	{
		return alternativProductList != null && !alternativProductList.isEmpty()
				&& ((AlternativeProductImpl) alternativProductList.get(0)).isSubstituteProduct();
	}
	...
	{
		return alternativProductList != null && !alternativProductList.isEmpty()
				&& ((AlternativeProductImpl) alternativProductList.get(0)).isDeterminationProduct();
	}
