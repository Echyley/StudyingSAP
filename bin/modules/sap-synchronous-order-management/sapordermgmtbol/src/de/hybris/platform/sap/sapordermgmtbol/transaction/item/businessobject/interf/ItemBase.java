...
public interface ItemBase extends SimpleItem
{
	 ...
	 * identical with a deep copy. The only difference is that the property <code>itemDelivery</code> (a list) is backed by
	 * the same data.
	 *
	 *
	 ...
	 * Whereas NetPrice gives the price per base unit, the NetValue is the price for the whole quantity. This includes the
	 ...
	 * Whereas NetPrice gives the price per base unit, the NetValue is the price for the whole quantity. This is excluding
	 ...
	 * If the quantity is changed for a item, this quantity shows the value before. This can e.g. be used for some business
	 * events.
	 *
	...
	/**
	 * Sets the total discount, which is used for strike through prices. The number must be positive (e.g. 10 and not -10,
	 * this is how it comes from IPC).
	 *
	 * @param totalDiscount
	 *           value to set
	 */
