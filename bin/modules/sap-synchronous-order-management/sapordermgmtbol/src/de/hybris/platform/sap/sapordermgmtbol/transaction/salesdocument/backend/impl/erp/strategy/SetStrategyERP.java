...
public class SetStrategyERP extends BaseStrategyERP implements SetStrategy, ConstantsR3Lrd
{
			...
			final List<String> itemNewShipTos, final boolean onlyUpdateHeader) throws BackendException
	{
		...
		try
		{
			...
			salesDocR3Lrd.setErroneous(("X").equals(esError.getString("ERRKZ")));
	...
	protected Set<String> findItemsToChange(
			final Map<String, //
					Item> itemsERPStatus, //
			...
			final TransactionConfiguration transConf)
	{
		...
		final StringBuilder output = new StringBuilder(LF);
		String traceString = null;
		if (tracing)
		{
			output.append("findItemsToChange, Results:");
		}
		...
		for (int i = 0; i < size; i++)
		{
			...
			if (existingItem != null)
			{
				...
				if (tracing)
				{
					...
					output.append(traceString).append("item attributes equal: ").append(doNotAddToChangedItems);
				}
				notChangeItem(itemsToBeChanged, tracing, output, traceString, currentItem, existingItem, doNotAddToChangedItems);
			}
			else
			{
				...
				if (tracing)
				{
					output.append(traceString).append("new or not known yet or customizing tells to send all items");
				}
			}
		}
		if (tracing)
	 ...
	 * @param currentItem
	 * @param existingItem
	 * @param doNotAddToChangedItems
	 */
	private void notChangeItem(final Set<String> itemsToBeChanged, final boolean tracing, final StringBuilder output,
			final String traceString, final Item currentItem, final Item existingItem, final boolean doNotAddToChangedItems)
	{
		if (doNotAddToChangedItems)
		{
			// now take configuration into account

			isCurrentItemConfigure(itemsToBeChanged, tracing, output, traceString, currentItem, existingItem);
		}
		else
		{
			itemsToBeChanged.add(currentItem.getHandle());
			if (tracing)
			{
				output.append(traceString).append("changed");
			}
		}
	}
	 ...
	 * @param existingItem
	 */
	private void isCurrentItemConfigure(final Set<String> itemsToBeChanged, final boolean tracing, final StringBuilder output,
			final String traceString, final Item currentItem, final Item existingItem)
	{
		if (currentItem.isConfigurable())
		{
			...
			if (tracing)
			{
				output.append(traceString).append("is configurable");
			}
		}
		else
		{
			...
			if (!custExit.customerExitIsItemChanged(currentItem, existingItem))
			{
				if (tracing)
				{
					output.append(traceString).append("does not need to be changed in ERP");
				}
			}
			else
			{
				...
				if (tracing)
				{
					output.append(traceString).append("extensions changed / cust exit");
				}
			}
		}
	}
	...
	protected boolean isItemToBeSent(final Item currentItem, final Item existingItem)
	{
		...
		final boolean isCancelStatusIdentical = currentItem.getOverallStatus().isCancelled() == existingItem.getOverallStatus()
				.isCancelled();
		final boolean quantityIdentical = isQuantityIdentical && currentItem.getFreeQuantity().compareTo(BigDecimal.ZERO) == 0;
		final boolean isEqualIsunitIsCancelStatus = isEqualTexts(currentItem, existingItem) && isUnitIdentical
				&& isCancelStatusIdentical;

		isEqual = isProductIdIdentical && quantityIdentical && equalDeliverDate && isEqualIsunitIsCancelStatus;
		return isEqual;
	}
	 ...
	 */
	protected int determineItemPosnr(final int lastNumber)
	{
		return lastNumber + TEN;
	}
