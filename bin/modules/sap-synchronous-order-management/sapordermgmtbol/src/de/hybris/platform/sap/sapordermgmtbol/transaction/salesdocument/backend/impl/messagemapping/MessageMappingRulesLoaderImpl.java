...
public class MessageMappingRulesLoaderImpl implements MessageMappingRulesLoader
{
	...
	private static final Log4JWrapper SAP_LOGGER = Log4JWrapper.getInstance(MessageMappingRulesLoaderImpl.class.getName());
	...
	{
		SAP_LOGGER.entering("loadRules(InputStream is)");
		...
		//NOSONAR  parser configured securely in DocumentBuilderFactoryUtil.setSecurityFeatures
		parser.parse(new InputSource(messageMappingFileInputStream));

		SAP_LOGGER.log(LogSeverity.DEBUG, LogCategories.APPS_BUSINESS_LOGIC,
		...
		for (final MessageMappingRule rule : tmpRules)
		{
			...
			for (final MessageMappingRule dublicated : rulesList)
			{
				...
				{
					final String msgPattern = "Rule {0} has the same pattern as rule {1}, it will be ignored";
					final Object[] args = new Object[]
					{ rule, dublicated };

					SAP_LOGGER.trace(LogSeverity.WARNING, msgPattern, args);

					final String errMsg = MessageFormat.format(msgPattern, args);
