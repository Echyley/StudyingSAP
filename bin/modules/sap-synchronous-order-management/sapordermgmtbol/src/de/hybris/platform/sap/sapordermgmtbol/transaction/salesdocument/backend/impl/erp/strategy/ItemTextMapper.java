...
public class ItemTextMapper extends TextMapper
{
			...
			final JCoTable textX, final JCoTable objInst)
	{
		...
		if (salesDoc.getItemList() != null)
		{
			...
			while (itemIT.hasNext())
			{
				...
				checkSendText(itemsToBeChanged, textV, textX, objInst, item, itemHandle);
			}
		}
	}
	 ...
	 * @param itemHandle
	 */
	private void checkSendText(final Set<String> itemsToBeChanged, final JCoTable textV, final JCoTable textX,
			final JCoTable objInst, final Item item, final String itemHandle)
	{
		if ((itemsToBeChanged == null) || itemsToBeChanged.contains(itemHandle))
		{
			...
			/*
			 * Now check whether we need to send this item text If no text has been maintained for a new text, we won't
			 * send anything.
			 *
			 * But note that this check only applies to a new text, if we already have a handle from ERP, we need to update
			 * the text anyhow!
			 */
			...
			setValues(textV, textX, objInst, item, itemHandle, sendKeyFields, handle);
		}
	}
	 ...
	 * @param handle
	 */
	protected void setValues(final JCoTable textV, final JCoTable textX, final JCoTable objInst, final Item item,
			final String itemHandle, boolean sendKeyFields, String handle)
	{
		textV.appendRow();
		if (sapLogger.isDebugEnabled())
		{
			traceText("Item text will be sent to ERP", configTextId, configLangIso, item.getText(), handle, itemHandle, sapLogger);
		}
