...
public class ItemHierarchy
{
	...
	private static class Node
	{
		...
		final public boolean isRootConfigurable()
		{
	...
	public ItemHierarchy(final ItemList itemList)
	{
		...
		while (it.hasNext())
		{
			...
			{ // sub position
				...
				lastNode = createSubNode(lastNode, item);
			}
		}
	}
	 ...
	 * @return
	 */
	protected final Node createSubNode(final Node lastNode, final Item item)
	{
		Node subNode;
		Node lNode = lastNode;
		if (lNode != null)
		{
			subNode = new Node(lNode, item);
			lNode.addNode(subNode);
			lNode = subNode;
		}
		else
		{
	 ...
	 * @return
	 */
	protected Node findParentId(final Item item)
	{
		Node lastNode;
		Node subNode;
	 ...
	 * @param item
	 *           The item to answer the question for
	 * @return <code>true</code> if the parent item of a given item is conf'able; otherwise <code>false</code> is returned.
	 */
