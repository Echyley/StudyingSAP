...
public class ItemSalesDoc extends ItemBaseImpl implements Item
{
	 ...
	 * from an order template If so, this flag might be used, to suppress things like campaign determination, etc. for the
	 * copied item.
	 *
	...
	/**
	 * Overwrites the super implementation. Merge is not allowed if a gift card or a configurable product is involved. Sub
	 * items are also not subject to merge. <br>
	 *
	...
	/**
	 * This method sets a flag, that indicates if the item is copied from another item, e.g. when an order is created from
	 * an order template If so, this flag might be used, to suppress things like campaign determination, etc. for the copied
	 * item.
	 *
