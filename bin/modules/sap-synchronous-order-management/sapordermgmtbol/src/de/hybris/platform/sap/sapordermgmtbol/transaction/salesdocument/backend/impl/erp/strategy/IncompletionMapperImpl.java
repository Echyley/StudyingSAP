...
public class IncompletionMapperImpl implements IncompletionMapper
{
	...
	@Override
	public void mapLogToMessage(final JCoTable incompLog, final JCoTable messages, final ItemList items)
	{
	 ...
	 *
	 * @param incompLog
	 *           JCO table of incompletion log, data will be read from current record
	 * @param messages
	 *           JCO table of messages, data will be written into current record
	 * @param items
	 *           Sales document items
	 */
	protected void mapItemMessage(final JCoTable incompLog, final JCoTable messages, final ItemList items)
	{
		final Item item = backendUtil.findItem(items, incompLog.getString("POSNR"));
	...
	/**
	 * Maps attributes between incompletion log and messages. The data written to the message table must match the
	 * message mapping customizing in messages.xml
	 *
	 * @param incompLog
	 *           JCO table of incompletion log, data will be read from current record
	 * @param messages
	 *           JCO table of messages, data will be written into current record
	 */
	protected void mapAttributes(final JCoTable incompLog, final JCoTable messages)
	{
		messages.setValue("MSGTY", "I");
		messages.setValue("MSGID", MSG_CLASS_INCOMPLETION + incompLog.getString("TBNAM"));
	...
	/**
	 * Do we consider this entry in the incompletion log? This depends on entry in FDNAM, the standard implementation
	 * only takes care of issues related to CUOBJ
	 *
	 * @param fieldName
