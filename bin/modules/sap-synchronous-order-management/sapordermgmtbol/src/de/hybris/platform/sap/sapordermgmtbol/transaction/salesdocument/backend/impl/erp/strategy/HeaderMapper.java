...
public class HeaderMapper extends BaseMapper
{
			...
			final Header header)
	{
		...
		readEsHeadComV(esHeadComV, header, GetAllReadParameters.setIpcPriceAttributes,GetAllReadParameters.shippingConditionsAsText, salesDocument,
				backendState, esHeadComR);

		// changeability
		header.setChangeable(backendState.getLoadState().isChangeable());

		readEsHeadComR(esHeadComR, header, headerPropMap, headerPriceAttribs, itemsPropMap, shop);
		readTtHeadCondV(ttHeadCondV, esHeadComR, header, GetAllReadParameters.headerCondTypeFreight);
			...
			final JCoStructure esHeadComR)
	{
		if (sapLogger.isDebugEnabled())
		{
			...
			debugOutput.append("\n handle         :      ").append(JCoHelper.getString(esHeadComV, ConstantsR3Lrd.FIELD_HANDLE));
			debugOutput.append("\n external ID    : ").append(JCoHelper.getString(esHeadComV, "BSTKD"));
			debugOutput.append("\n shipCond       :    ").append(JCoHelper.getString(esHeadComV, "VSBED"));
			debugOutput.append("\n req. deliv date:    ")
					.append(ConversionHelper.convertDateToLocalizedString(esHeadComV.getDate("VDATU")));
			sapLogger.debug(debugOutput);
		}
		header.setHandle(JCoHelper.getString(esHeadComV, ConstantsR3Lrd.FIELD_HANDLE));
		...
		final DocumentType docType = DocumentTypeMapping
				.getDocumentTypeByTransactionGroup(JCoHelper.getString(esHeadComR, "TRVOG_R"));
		...
		final MessageList msgList = baseR3Lrd.getMessageList(header.getTechKey());
		if ((msgList != null) && !(msgList.isEmpty()))
		{
			for (int j = 0; j < msgList.size(); j++)
			{
				...
				// for the Header!)
				((BusinessObject) businessObjectInterface).addMessage(msgList.get(j));
	 ...
	 *           the document Type
	 */
	protected void setValidToDateForQuotations(final JCoStructure esHeadComV, final Header header, final DocumentType docType)
	{
		if (DocumentType.QUOTATION.equals(docType))
		{
			...
			{
				sapLogger.debug("No valid to date returned, set default valid to date");
	 ...
	 */
	@SuppressWarnings("squid:S1172")
	protected void readEsHeadComR(final JCoStructure esHeadComR, final Header header,
			final Map<String, String> ipcHeadPropMap, final Map<String, String> ipcHeadAttribMap,
			final Map<String, String> ipcItemPropMap, final TransactionConfiguration shop)
	{
		...
		else
		{
			...
			sapLogger.debug(
					"No Document type could be determined for transaction group: " + JCoHelper.getString(esHeadComR, "TRVOG_R")); //docType
		}
		...
		if (GetAllReadParameters.shippingConditionsAsText)
		{
	 ...
	 *           the document Type
	 */
	protected void setValidFromDateForQuotations(final JCoStructure esHeadComR, final Header header, final DocumentType docType)
	{
		if (DocumentType.QUOTATION.equals(docType))
		{
	 ...
	 */
	@SuppressWarnings("squid:S1172")
	protected void readTtHeadCondV(final JCoTable ttHeadCondV, final JCoStructure esHeadComR, final Header header,
			final String headerCondTypeFreight)
	{
		...
		{
			sapLogger.debug("No condition type had been defined to determine the freight value");
		...
		while (numRows < ttHeadCondV.getNumRows())
		{
			...
			{
				BigDecimal freightValue = ttHeadCondV.getBigDecimal("KWERT_INT");
				final String currency = JCoHelper.getString(ttHeadCondV, "WAERK");
				final int numberOfDecimals = CustomizingHelper.getNumberOfDecimals(converter, currency);
		...
		sapLogger.debug("No freight value found for condition \"" + headerCondTypeFreight + "\"");
		header.setFreightValue(BigDecimal.ZERO);
	}
			...
			final JCoStructure headComX, final TransactionConfiguration config)
	{
		...
		if (sapLogger.isDebugEnabled())
		{
			...
			debugOutput.append("\n ID         :         ").append(salesDocHeader.getTechKey());
			debugOutput.append("\n handle     :         ").append(salesDocHeader.getHandle());
			debugOutput.append("\n external ID:         ").append(salesDocHeader.getPurchaseOrderExt());
			debugOutput.append("\n shipping conditions: ").append(shipCond);
			debugOutput.append("\n req. delivery date : ").append(reqDeliveryDate);
			debugOutput.append("\n inco1:               ").append(salesDocHeader.getIncoTerms1());
			debugOutput.append("\n inco2:               ").append(salesDocHeader.getIncoTerms2());
		...
		if (salesDocR3Lrd.getLoadState().getLoadOperation().equals(LoadOperation.create) && (config.getDeliveryBlock() != null))
		{
