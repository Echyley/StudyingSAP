...
public abstract class GetAllStrategyERP extends BaseStrategyERP implements GetAllStrategy, ConstantsR3Lrd
{
	 ...
	 */
	public static final Log4JWrapper SAP_LOGGER = Log4JWrapper.getInstance(GetAllStrategyERP.class.getName());
	...
	public BackendCallResult execute(final BackendState backendState, final SalesDocument salesDocument,
			final ItemBuffer itemBuffer, final JCoConnection cn) throws BackendException
	{

		final String methodName = "execute()";
		SAP_LOGGER.entering(methodName);
		...
		if (SAP_LOGGER.isDebugEnabled())
		{
			...
			debugOutput.append("\n VBELN    : ").append(vbeln);
			debugOutput.append("\n is changeable: ").append(backendState.getLoadState().isChangeable());
			SAP_LOGGER.debug(debugOutput.toString());
		}
		...
		if (custExit != null)
		{
			custExit.customerExitBeforeGetAll(salesDocument, function, cn, SAP_LOGGER);
		}
		executeRfc(cn, function);
		if (custExit != null)
		{
			custExit.customerExitAfterGetAllJCOCall(salesDocument, function, cn, SAP_LOGGER);
		}
		// First error check
		...
		if (hasMessage("I", "V2", "167", etMessages, esError))
		{
			dispatchMessages(salesDocument, etMessages, esError);
		}
		...
		if (SAP_LOGGER.isDebugEnabled())
		{
			SAP_LOGGER.debug("Tracing requested items (IT_ITEM_HANDLE) and returned items (TT_ITEM_COMV):\n");

			JCoHelper.logCall(function.getName(), function.getImportParameterList().getTable("IT_ITEM_HANDLE"), function
					.getTableParameterList().getTable("TT_ITEM_COMV"), SAP_LOGGER);
		}
		...
		if (GetAllReadParameters.setIpcPriceAttributes)
		{
		...
		// header tables
		headMapper.read(esHeadComV, esHeadComR, ttHeadCondV, backendState, shop, headerPriceAttribs, itemsPropMap, headerPropMap,
		...
		itemMapper.read(ttItemComV, ttItemComR, ttItemVstatComV, ttItemSlineComV, esError, shop, objInstMap, itemKey, itemMap,
				itemsPriceAttribMap, backendState, salesDocument);
		...
		if (custExit != null)
		{
			custExit.customerExitAfterGetAll(salesDocument, function, cn, SAP_LOGGER);
		}
		SAP_LOGGER.exiting();
		return retVal;
	}
	...
	{
		if (!backendState.getLoadState().isChangeable() && (vbeln.length() <= TEN))
		{
	 ...
	 */
	@SuppressWarnings("squid:MethodCyclomaticComplexity")
	protected void handleTtIDFlow(final Map<String, String> itemKey, final Map<String, Item> itemMap, final String thisDocNumber,
			final DocumentType documentType, final JCoTable ttIDFlow)
	{
		String docNumber = thisDocNumber;
		if (!(ttIDFlow.getNumRows() > 0))
		{
			return;
		}
		docNumber = ConversionTools.cutOffZeros(docNumber);
		...
		for (int i = 0; i < numItems; i++)
		{
			...
			// docNumber (
			if (documentType.equals(docType) && docNumber.equals(docNum))
			{
