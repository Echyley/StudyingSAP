...
import static de.hybris.platform.sap.sapordermgmtbol.constants.SapordermgmtbolConstants.EXCEPTION_DURING_ACCESS_TO_CUSTOMIZING;
...
public class CheckoutConfigurationBackendTools
{
	 ...
	 * Logging instance
	 */
	protected static final Log4JWrapper sapLogger = Log4JWrapper.getInstance(CheckoutConfigurationBackendTools.class.getName());

	/**
	 * generic method to call a searchvaluehelp in the backend and format result can be used for customizing tables, but
	 * not the complex valuehelps e.g. BP search <br>
	 * background: <br>
	 ...
	 *           = true means, if no description is found, the value is shown as description. This is done in order to
	 *           avoid lines with space in the DropDownListboxes, which actually refer to values
	 * @return a HashMap with pairs ( content of the valuefield, content of the descriptionfield)
	 * @throws BackendException
	 *            in case of issues with the JCO-Connection or ABAP-errors in the backend
	 */
	public Map<String, String> executeSearchhelp(final JCoConnection jConn, final String searchhelp, final String valueField,
			final String descriptionField, final Boolean withFallbackDesc) throws BackendException
	{
		...
		searchResult = convertSearchResult(jParamList, valueField, descriptionField, withFallbackDesc.booleanValue());
	...
	/**
	 * @param jConn
	 *           JC0-Connection to the backend, normally type JCoConnectionStateless
	 * @param searchhelp
	 *           name of the searchhelp in the backen
	 * @return the exportParameterlist with the tables containing the resultl
	 * @throws BackendException
	 */
	protected JCoParameterList executeBackendAccess(final JCoConnection jConn, final String searchhelp) throws BackendException
	{
		...
		jImportParams.getStructure("EXPLICIT_SHLP").setValue("SHLPNAME", searchhelp);
		...
		final JCoStructure returnStructure = jExportParams.getStructure("RETURN");
		...
		if ("S&013".equals(returnStructure.getString("TYPE")))
		{
			...
			final String logMessage = "searchhelp" + searchhelp + returnStructure.getString("MESSAGE")
					+ "check the situation in the Backend";
			sapLogger.log(LogSeverity.WARNING, LogCategories.APPS_COMMON_CONFIGURATION, "Searchhelp doesn't exist", new Object[]
			{ logMessage });
		}
	 ...
	 *
	 * @param jTableParams
	 *           = contains all table parameters from the call of BAPI_HELPVALUES_GET
	 * @param valueField
	 *           = Name of the Field, which contains the value
	 * @param descriptionField
	 *           = Name of the field, which contains the description background
	 ...
	 * @return a HashMap with pairs ( content of the valuefield, content of the descriptionfield)
	 */
	protected Map<String, String> convertSearchResult(final JCoParameterList jTableParams, final String valueField,
			final String descriptionField, final boolean withFallbackDesc)
	{
		...
		final Map<String, String> searchResult = new HashMap<String, String>();

		processSearchResult(jTableParams, valueField, descriptionField, withFallbackDesc, searchResult);
	 ...
	 * @param searchResult
	 */
	protected void processSearchResult(final JCoParameterList jTableParams, final String valueField,
			final String descriptionField, final boolean withFallbackDesc, final Map<String, String> searchResult)
	{
		JCoTable jFieldList;
		...
		for (int i = 0; i < jFieldList.getNumRows(); i++)
		{
			...
			try
			{
				...
				{
					intValueFieldOffset = Integer.parseInt(jFieldList.getString("OFFSET"));
					intValueFieldEnd = intValueFieldOffset + Integer.parseInt(jFieldList.getString("LENG"));
				}

				if (jFieldList.getString("FIELDNAME").equals(descriptionField))
				{
					intDescriptionFieldOffset = Integer.parseInt(jFieldList.getString("OFFSET"));
					intDescriptionFieldEnd = intDescriptionFieldOffset + Integer.parseInt(jFieldList.getString("LENG"));
				}
			}
			catch (final NumberFormatException ex)
			{
				sapLogger.log(LogSeverity.ERROR, LogCategories.APPS_BUSINESS_LOGIC, EXCEPTION_DURING_ACCESS_TO_CUSTOMIZING,
						new Object[]
						{ ex.toString() });
				sapLogger.traceThrowable(LogSeverity.DEBUG, EXCEPTION_DURING_ACCESS_TO_CUSTOMIZING, ex);
				sapLogger.exiting();
		...
		{
			final BORuntimeException ex = new BORuntimeException("Exception during access to customizing in backend");
			sapLogger.log(LogSeverity.ERROR, LogCategories.APPS_BUSINESS_LOGIC, EXCEPTION_DURING_ACCESS_TO_CUSTOMIZING, new Object[]
			{ ex.toString() });
			sapLogger.traceThrowable(LogSeverity.DEBUG, EXCEPTION_DURING_ACCESS_TO_CUSTOMIZING, ex);
			sapLogger.exiting();
		...
		processHelpValuesTableParams(jTableParams, withFallbackDesc, searchResult, intValueFieldOffset, intValueFieldEnd,
				intDescriptionFieldOffset, intDescriptionFieldEnd);
	}
	 ...
	 * @param withFallbackDesc
	 * @param searchResult
	 * @param intValueFieldOffset
	 * @param intValueFieldEnd
	 * @param intDescriptionFieldOffset
	 * @param intDescriptionFieldEnd
	 */
	protected void processHelpValuesTableParams(final JCoParameterList jTableParams, final boolean withFallbackDesc,
			final Map<String, String> searchResult, int intValueFieldOffset, int intValueFieldEnd, int intDescriptionFieldOffset,
			int intDescriptionFieldEnd)
	{
		JCoTable jFieldList;
		...
		for (int i = 0; i < jFieldList.getNumRows(); i++)
		{
			...
			if (intValueFieldEnd > 0)
			{
				...
				strlenFieldStart = Math.min(strlenHelpvalues, intValueFieldOffset);
				strlenFieldEnd = Math.min(strlenHelpvalues, intValueFieldEnd);
				final String work = stringHelpvalues.substring(strlenFieldStart, strlenFieldEnd);
				stringValue = deleteTrailingSpaces(work);

			} // if valueField found

			// if the descriptionField is found, evaluate it
			stringDescription = isDescriptionField(withFallbackDesc, intDescriptionFieldOffset, intDescriptionFieldEnd,
					stringHelpvalues, stringValue, stringDescription, strlenHelpvalues);
	 ...
	 * @param intDescriptionFieldOffset
	 * @param intDescriptionFieldEnd
	 * @param stringHelpvalues
	 * @param stringValue
	 * @param stringDescription
	 * @param strlenHelpvalues
	 * @return
	 */
	private String isDescriptionField(final boolean withFallbackDesc, final int intDescriptionFieldOffset,
			final int intDescriptionFieldEnd, final String stringHelpvalues, final String stringValue,
			final String stringDescription, final int strlenHelpvalues)
	{
		int strlenFieldEnd;
		String des = stringDescription;
		if (intDescriptionFieldEnd > 0)
		{

			strlenFieldEnd = Math.min(strlenHelpvalues, intDescriptionFieldEnd);
			String workstr = null;
			if (strlenHelpvalues > intDescriptionFieldOffset)
			{
				des = stringHelpvalues.substring(intDescriptionFieldOffset, strlenFieldEnd);
			}
			if (withFallbackDesc)
			{
	 ...
	 * @param inString
	 *           input string
	 * @return a copy of the inString, with all space-characters removed from the end
	 */
	public static String deleteTrailingSpaces(final String inString)
	{
		...
		if (work != null)
		{
			...
			while (work.length() > 0 && work.lastIndexOf(' ') == work.length() - 1)
			{
