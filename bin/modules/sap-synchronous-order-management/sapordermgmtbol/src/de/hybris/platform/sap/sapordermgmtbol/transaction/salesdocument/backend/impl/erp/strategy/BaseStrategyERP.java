...
public class BaseStrategyERP
{
			...
			final Log4JWrapper log)
	{
		...
		createBufferForLogging(functionName, record, direction, builder);
			...
			final StringBuilder builder)
	{
		...
		if ((record != null) && recordOK)
		{
			...
			builder.append("::").append(functionName).append("::").append(direction).append(record.getMetaData().getName())
					.append(" * ");
			final JCoFieldIterator iterator = record.getFieldIterator();
			while (iterator.hasNextField())
			{
				final JCoField field = iterator.nextField();
				builder.append(field.getName()).append("='").append(field.getString()).append("' ");
			}
	...
	@SuppressWarnings("squid:S00107")
	public static boolean hasMessage(final String msgType, final String msgId, final String msgNo, final JCoTable msgTable,
			final JCoStructure singleMsg)
	{
		boolean retVal = false;

		retVal = MessageUtil.hasMessage(msgType, msgId, msgNo, singleMsg);

		if (!retVal)
		{
			retVal = MessageUtil.hasMessage(msgType, msgId, msgNo, msgTable);
		}
	...
	void logErrorMessage(final JCoStructure esError)
	{
		...
		sapLogger.log(LogSeverity.ERROR, LogCategories.APPLICATIONS, message + ": {0},{1},{2}", msgArgs);
	}
