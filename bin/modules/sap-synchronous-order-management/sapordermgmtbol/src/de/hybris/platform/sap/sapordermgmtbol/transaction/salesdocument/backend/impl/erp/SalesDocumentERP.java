...
public abstract class SalesDocumentERP extends IsaBackendBusinessObjectBaseSalesERP implements SalesDocumentBackend
{
	...
	private static final Log4JWrapper sapLogger = Log4JWrapper.getInstance(SalesDocumentERP.class.getName());
	...
	private final Map<TechKey, Item> erroneousItems = new HashMap<>();
	...
	protected Map<String, String> itemVariantMap = new HashMap<>(0);
			...
			final TransactionConfiguration transConf) throws BackendException
	{
		...
		if (!(salesDocument.getMessageList().isEmpty()) && !(salesDocument.getItemList().isEmpty()))
		{
	...
	protected boolean isInBackend(final SalesDocument salesDocument)
	{
		...
		return (id != null) && (!id.isEmpty()) && (id.length() <= TEN);
	}
