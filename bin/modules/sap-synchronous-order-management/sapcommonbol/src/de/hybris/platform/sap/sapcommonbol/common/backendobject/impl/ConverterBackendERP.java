...
public class ConverterBackendERP extends ConverterBackendERPCRM
{
			...
			throws BackendException
	{
		...
		for (int i = 0; i < f4Help.getNumRows(); i++)
		{
			...
			numberOfDecimalsForUOMs.put(internalName, Integer.valueOf(decimals));
			...
			throws BackendException
	{
		...
		importsErpWecCustF4Currency.setValue(ErpWecCustF4Currency.ivLanguage, language.toUpperCase(Locale.ENGLISH));

		getDefaultJCoConnection().execute(function);


		final JCoParameterList exportParameterList = function.getExportParameterList();
		final JCoTable f4Help = exportParameterList.getTable(ErpWecCustF4Currency.etCurrencyUnit);
