...
public class DefaultSAPPartnerService implements SapPartnerService
{
	...
	public String getCurrentSapCustomerId()
	{
		...
		if (rootB2BUnit != null && rootB2BUnit.getUid() != null)
		{
			//added logic to append leading zeros in case of S4HANA
			return ConversionTools.addLeadingZerosToNumericID(rootB2BUnit.getUid(), 10);
		}
		else
		{
			return null;
	...
	public AddressModel getHybrisAddressForSAPCustomerId(final String sapCustomerId)
	{
		...
		if (parent != null)
		{
			...
			{
				String currentSAPCustomerID = address.getSapCustomerID();
				if (currentSAPCustomerID != null)
				 {
					 //added logic to append leading zeros in case of S4HANA
					 currentSAPCustomerID = ConversionTools.addLeadingZerosToNumericID(address.getSapCustomerID(), 10);
				}
				if (LOG.isDebugEnabled())
				{
	...
	{
		final B2BCustomerModel b2bCustomer = (B2BCustomerModel) b2bCustomerService.getCurrentB2BCustomer();
		return (B2BUnitModel) b2bUnitService.getParent(b2bCustomer);
	}
