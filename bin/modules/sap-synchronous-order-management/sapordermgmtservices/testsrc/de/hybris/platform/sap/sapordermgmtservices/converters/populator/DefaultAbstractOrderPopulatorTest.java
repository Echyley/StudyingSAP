...
public class DefaultAbstractOrderPopulatorTest
{
	 ...
	 *
	 */
	private static final BigDecimal NET_VALUE_EXAMPLE = new BigDecimal(520);
	/**
	 * 
	 */
	...
	public static void injectMockedBeans(final DefaultAbstractOrderPopulator<Basket, CartData> populator)
	{
		...
		final BaseStoreModel baseStoreModel = EasyMock.createMock(BaseStoreModel.class);
		EasyMock.expect(Boolean.valueOf(baseStoreModel.isNet())).andReturn(Boolean.TRUE);
		EasyMock.replay(baseStoreModel);
		final BaseStoreService baseStoreService = EasyMock.createMock(BaseStoreService.class);
		EasyMock.expect(baseStoreService.getCurrentBaseStore()).andReturn(baseStoreModel);
	...
	public void testPopulateHeaderPrices()
	{
		...
		Assert.assertEquals(NET_VALUE_EXAMPLE, target.getTotalPrice().getValue());
		Assert.assertEquals(GROSS_VALUE_EXAMPLE, target.getTotalPriceWithTax().getValue());
	...
	public static void setupCart(final SalesDocument basket)
	{
		...
		header.setNetValue(NET_VALUE_EXAMPLE);
