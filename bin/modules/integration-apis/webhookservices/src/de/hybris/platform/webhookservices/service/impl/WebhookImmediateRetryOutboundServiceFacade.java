...
public class WebhookImmediateRetryOutboundServiceFacade implements OutboundServiceFacade
{
	...
	private Observable<ResponseEntity<Map>> sendIntegrationObject(final SyncParameters params)
	{
		...
		LOGGER.debug("Passing {} to {} within the retry template", params, outboundServiceFacade);
	...
	{
		LOGGER.debug("Failed attempt to send {}", params);

		if (error instanceof HttpServerErrorException ex)
		{
			checkForServerError(ex.getStatusCode());
		}
		else if (error instanceof WebhookRetryableException ex)
		{
			throw ex;
		}
	}
