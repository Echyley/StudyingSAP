...
public class ReadService
{
	...
	public ReadService(@NotNull final FlexibleSearchService flexibleSearchService, @NotNull final TypeService typeService,
	                   @NotNull final SchemaGenerator generator, @NotNull final ReadOnlyAttributesConfiguration configuration)
	{
		Preconditions.checkArgument(flexibleSearchService != null, "flexibleSearchService cannot be null");
		Preconditions.checkArgument(typeService != null, "typeService cannot be null");
		Preconditions.checkArgument(generator != null, "oDataDefaultSchemaGenerator cannot be null");
		Preconditions.checkArgument(configuration != null, "readOnlyAttributesConfiguration cannot be null");
		this.flexibleSearchService = flexibleSearchService;
		this.typeService = typeService;
		this.oDataDefaultSchemaGenerator = generator;
		this.readOnlyAttributesConfiguration = configuration;
	}
	...
	public List<IntegrationObjectVirtualAttributeDescriptorModel> getVirtualAttributeDescriptorModelsByCode(String code)
	{
		return flexibleSearchService.<IntegrationObjectVirtualAttributeDescriptorModel>search(
				String.format("SELECT PK FROM {IntegrationObjectVirtualAttributeDescriptor} WHERE p_code = '%s'",
						code)).getResult();
	}
	...
	{
		return flexibleSearchService.<IntegrationObjectVirtualAttributeDescriptorModel>search(
				"SELECT PK FROM {IntegrationObjectVirtualAttributeDescriptor}").getResult();
	}
	...
	public InputStream getEDMX(final IntegrationObjectModel integrationObject)
	{
		...
		catch (final NullPointerException | IOException | XMLStreamException | EntityProviderException e)
		{
