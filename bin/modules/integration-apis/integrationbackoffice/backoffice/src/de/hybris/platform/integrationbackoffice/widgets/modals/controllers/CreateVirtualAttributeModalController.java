...
public class CreateVirtualAttributeModalController extends DefaultWidgetController
{
	...
	transient List<AbstractListItemDTO> currentListboxAttributes = new ArrayList<>();
	...
	{
		setCurrentListboxAttributes(attributes);
		loadScriptLocation();
		loadDescriptorType();
	...
	private void setCurrentListboxAttributes(final List<AbstractListItemDTO> listboxAttributes)
	{
		if (listboxAttributes != null)
		{
			currentListboxAttributes = new ArrayList<>(listboxAttributes);
		}
		else
		{
			currentListboxAttributes.clear();
		}
	}
	...
	private boolean isAliasAndCodeValid(final String descriptorCode, final String alias, final boolean isCodeUsedAsAlias)
	{
		...
		else if (!ModalUtils.isAliasUnique(alias, descriptorCode, Collections.unmodifiableList(currentListboxAttributes)))
		{
