...
public class DefaultMapToIntegrationKeyValueGeneratorUnitTest
{
	...
	{
		final TypeDescriptor typeDesc = mock(TypeDescriptor.class);
		doReturn(type).when(typeDesc).getItemCode();
		final TypeAttributeDescriptor attrDesc = mock(TypeAttributeDescriptor.class);
		doReturn(typeDesc).when(attrDesc).getTypeDescriptor();
		doReturn(property).when(attrDesc).getAttributeName();
		return new KeyAttribute(attrDesc);
	}
