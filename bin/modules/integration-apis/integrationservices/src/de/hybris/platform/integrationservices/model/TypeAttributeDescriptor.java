 ...
 * of the payloads to/from the {@link ItemModel}.
 */
public interface TypeAttributeDescriptor
{
	 ...
	 * is interested in cars only and does not care about engines outside the car model, then the attribute should be
	 * defined with {@code partOf == true}. If the engine has independent existence, e.g. engine can be sold without a car,
	 * then the attribute should be defined with {@code partOf == false}.
	...
	/**
	 * Determines whether the attribute is referring to a map of values. Typically, used to determine in conjunction with
