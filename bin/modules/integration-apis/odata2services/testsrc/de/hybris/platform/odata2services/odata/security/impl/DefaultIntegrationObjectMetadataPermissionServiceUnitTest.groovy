...
@UnitTest
class DefaultIntegrationObjectMetadataPermissionServiceUnitTest extends JUnitPlatformSpecification {
    ...
    @Test
    def "metadata access is bypassed when the integration object contains classes"() {
        given: 'access right check is enabled'
        integrationServicesConfiguration.isAccessRightsEnabled() >> true
        and: 'the integration object contains classes'
        contextIntegrationObject.getClasses() >> [ioClass('path.to.class.ClassName')].toSet()
    ...
    @Test
    def "no exception is thrown when the model is not found for the IO code"() {
        given: 'the integration object model is not found'
    ...
    private IntegrationObjectClassModel ioClass(final String className) {
