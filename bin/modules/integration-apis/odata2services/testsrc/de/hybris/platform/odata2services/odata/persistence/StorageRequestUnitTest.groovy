...
@UnitTest
class StorageRequestUnitTest extends JUnitPlatformSpecification {
    ...
    @Shared
    private EdmEntitySet entitySet = entitySet(entityType)
    ...
    def "IllegalArgumentException is thrown when request is built with null #condition"() {
        ...
        where:
        condition                 | set             | content        | accept        | intItem
        'entity set'              | null            | CONTENT_LOCALE | ACCEPT_LOCALE | integrationItem()
        'entity type'             | entitySet(null) | CONTENT_LOCALE | ACCEPT_LOCALE | integrationItem()
        'Accept-Language locale'  | entitySet       | CONTENT_LOCALE | null          | integrationItem()
        'Content-Language locale' | entitySet       | null           | ACCEPT_LOCALE | integrationItem()
        'integration item'        | entitySet       | CONTENT_LOCALE | ACCEPT_LOCALE | null
    }
    ...
    def "request can be built without persistence hooks specification"() {
        ...
        expect:
        request != null
        request.postPersistHook == null
        request.prePersistHook == null
    }
    ...
    EdmEntitySet entitySet(EdmEntityType type) {
        Stub(EdmEntitySet) {
            getEntityType() >> type
        }
    }
