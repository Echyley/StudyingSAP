...
public class DefaultPayloadBuildingRequestDecorator implements OutboundRequestDecorator
{
	...
	{
		return !HttpMethod.DELETE.equals(decoratorContext.getHttpMethod());
	}
	...
	private ItemToMapConversionContext toConversionContext(final DecoratorContext ctx)
	{
		return ctx.getIntegrationObjectItem()
		          .map(type -> new ItemToMapConversionContext(ctx.getPayloadObject(), type))
		          .orElseThrow(
				          () -> new IntegrationObjectAndItemMismatchException(ctx.getPayloadObject(), ctx.getIntegrationObject()));
	}
