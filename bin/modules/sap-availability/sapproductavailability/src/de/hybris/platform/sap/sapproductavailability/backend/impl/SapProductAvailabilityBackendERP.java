...
public class SapProductAvailabilityBackendERP extends BackendBusinessObjectBaseJCo implements SapProductAvailabilityBackend
{
	...
	private static final String MATERIAL_LONG = "MATERIAL_LONG";
			...
			final Long requestedQuantity) throws BackendException
	{
		...
		SapProductAvailability availability = getCachedBackend().readCachedProductAvailability(product, customerId, plant,
				requestedQuantity);
		...
		try
		{
			...
			//check material code > 18 chars for SAP HANA backened.
			if(material.length()>PRODUCT_CODE_LENGTH)
			{
				importParameterList.setValue(MATERIAL_LONG, material);
			}
			else
			{
			...
			if (!resultTable.isEmpty())
			{
				...
				availability = processResultTable(product, plant, bapiMatAvail, material, stockLevelDecimal, resultTable);
			}

			getCachedBackend().cacheProductAvailability(availability, product, customerId, plant, requestedQuantity);
			return availability;
	...
	protected SapProductAvailability processResultTable(final ProductModel product, final String plant,
			final JCoFunction bapiMatAvail, final String material, final Long stockLevelDecimal,
			final JCoTable resultTable) {
		SapProductAvailability availability;
	...
	public String readPlant(final ProductModel product, final String customerId) throws BackendException
	{
		...
		String plant = getCachedBackend().readCachedPlant(product.getCode(), customerId,
		...
		getCachedBackend().cachePlant(plant, product.getCode(), customerId, getProperty(SapproductavailabilityConstants.SALES_ORG),
	...
	public String readPlantForCustomerMaterial(final String material, final String customerId) throws BackendException
	{
		...
		String plant = getCachedBackend().readCachedPlantCustomer(material, customerId,
				getProperty(SapproductavailabilityConstants.SALES_ORG), getProperty(SapproductavailabilityConstants.DIS_CHANNEL));
		...
		try
		{
			...
			if(material.length()>PRODUCT_CODE_LENGTH)
			{
				matTable.setValue(MATERIAL_LONG, material);
			}
			else{
				...
				sapLogger
						.debug(String
								.format(
										...
										customerId, getProperty(SapproductavailabilityConstants.SALES_ORG), getProperty(SapproductavailabilityConstants.DIS_CHANNEL), material));
			}
			...
			// Cache the result
			getCachedBackend().cachePlantCustomer(plant, material, customerId, getProperty(SapproductavailabilityConstants.SALES_ORG),
					getProperty(SapproductavailabilityConstants.DIS_CHANNEL));
	...
	{
		final Object propertyValue = getConfigAccess().getProperty(name);
