...
public class CloseCustomerAccountCronJobPerformableTest
{
	...
	public void setUp()
	{
		Mockito.lenient().when(s3StorageServiceFactory.getS3Service(Mockito.anyString(), Mockito.anyString(), Mockito.anyString()))
				.thenReturn(s3Service);
		Mockito.lenient().when(s3Service.getObject(Mockito.any())).thenReturn(s3Object);
		Mockito.lenient().when(configurationService.getConfiguration()).thenReturn(configuration);
		Mockito.lenient().when(configuration.getString("gigyauserdeleteservices.s3.config.objectnames.prefix")).thenReturn("test");

		Mockito.lenient().when(configuration.getString("gigyauserdeleteservices.s3.config.bucketId")).thenReturn("bucketId");
		Mockito.lenient().when(configuration.getString("gigyauserdeleteservices.s3.config.processed.folder.name")).thenReturn("processed");
	...
	{
		Mockito.lenient().when(s3Service.listObjects("bucketId", "test")).thenReturn(objectListing);
		Mockito.lenient().when(objectListing.getObjectSummaries()).thenReturn(Collections.singletonList(firstS3ObjectSummary));
		Mockito.lenient().when(firstS3ObjectSummary.getBucketName()).thenReturn("bucketId");
		Mockito.lenient().when(firstS3ObjectSummary.getKey()).thenReturn("test1.csv");
		...
		Mockito.lenient().when(gigyaLoginService.findCustomerByGigyaUid("123")).thenReturn(firstCustomer);
		Mockito.lenient().when(gigyaLoginService.findCustomerByGigyaUid("321")).thenReturn(secondCustomer);
		Mockito.lenient().when(gigyaConfigGenericDao.find(Collections.singletonMap(GigyaConfigModel.GIGYAAPIKEY, "")))
				.thenReturn(Collections.singletonList(gigyaConfig));
		Mockito.lenient().when(gigyaConfig.getSites()).thenReturn(Collections.singletonList(baseSite));
		Mockito.lenient().when(s3Object.getObjectContent()).thenReturn(s3ObjectInputStream);
		Mockito.lenient().when(s3Object.getKey()).thenReturn("test1.csv");
		Mockito.lenient().when(s3StorageServiceFactory.getS3Service(Mockito.any(), Mockito.any(), Mockito.any())).thenReturn(s3Service);
	...
	{
		Mockito.lenient().when(s3Service.listObjects("bucketId", "test")).thenReturn(objectListing);
		Mockito.lenient().when(objectListing.getObjectSummaries()).thenReturn(Arrays.asList(firstS3ObjectSummary, secondS3ObjectSummary));
		Mockito.lenient().when(firstS3ObjectSummary.getBucketName()).thenReturn("bucketId");
		Mockito.lenient().when(firstS3ObjectSummary.getKey()).thenReturn("test1.csv");

		Mockito.lenient().when(secondS3ObjectSummary.getBucketName()).thenReturn("bucketId");
		Mockito.lenient().when(secondS3ObjectSummary.getKey()).thenReturn("test2.csv");
		...
		Mockito.lenient().when(gigyaLoginService.findCustomerByGigyaUid("123")).thenReturn(firstCustomer);
		Mockito.lenient().when(gigyaLoginService.findCustomerByGigyaUid("321")).thenReturn(secondCustomer);
		Mockito.lenient().when(gigyaLoginService.findCustomerByGigyaUid("098")).thenReturn(thirdCustomer);
		Mockito.lenient().when(gigyaConfigGenericDao.find(Collections.singletonMap(GigyaConfigModel.GIGYAAPIKEY, "")))
				.thenReturn(Collections.singletonList(gigyaConfig));
		Mockito.lenient().when(gigyaConfig.getSites()).thenReturn(Collections.singletonList(baseSite));
		Mockito.lenient().when(s3Object.getObjectContent()).thenReturn(firstS3ObjectInputStream, secondS3ObjectInputStream);
		Mockito.lenient().when(s3Object.getKey()).thenReturn("test1.csv", "test2.csv");
		Mockito.lenient().when(s3StorageServiceFactory.getS3Service(Mockito.any(), Mockito.any(), Mockito.any())).thenReturn(s3Service);
	...
	{
		Mockito.lenient().when(s3Service.listObjects("bucketId", "test")).thenReturn(null);
		Mockito.lenient().when(s3StorageServiceFactory.getS3Service(Mockito.any(), Mockito.any(), Mockito.any())).thenReturn(s3Service);
		final PerformResult result = closeCustomerAccountCronJobPerformable.perform(cronJob);
		Assert.assertEquals(CronJobResult.SUCCESS, result.getResult());
		Assert.assertEquals(CronJobStatus.FINISHED, result.getStatus());
	...
	public void testCloseAccountJobWhenErrorOccurs()
	{
		...
		Mockito.lenient().when(firstS3ObjectSummary.getKey()).thenReturn("test1.csv");
		Mockito.lenient().when(s3StorageServiceFactory.getS3Service(Mockito.any(), Mockito.any(), Mockito.any())).thenReturn(s3Service);
		Mockito.lenient().when(s3Object.getObjectContent()).thenThrow(new RuntimeException());
