 ...
 * organization
 * @deprecated (SAP CDC has introduced accounts.b2b.auth.getAssets and B2B user roles have to be fetched using this API.)
 */
@Deprecated(since = "2211")
public class DefaultGigyaAuthService implements GigyaAuthService {

	private static final Logger LOG = LoggerFactory.getLogger(DefaultGigyaAuthService.class);
	...
	public void assignAuthorisationsToCustomer(final CustomerModel customer) {
		final ResponseEntity<GigyaAuthData> response = fetchAuthorizations(customer);

		// If there is an error in fetching authorisations, then stop.
		if (response == null) {
			return;
		}

		final GigyaAuthData authData = response.getBody();
		final GigyaAssetsData assetData = authData != null ? authData.getAssets() : null;

		if (validateAssetDataExists(authData, assetData) && validateFunctionalRolesAndActionsExist(assetData)) {
			final List<GigyaActionsAccessData> accessList = assetData.getAssetTemplates().getCommerceFunctionalRoles()
					.getActions().getAccessList();
	...
	private ResponseEntity<GigyaAuthData> fetchAuthorizations(final CustomerModel customer) {
		...
		final String token = getGigyaTokenGenerator().generate(gigyaConfig.getAuthRequestKey(),
				gigyaConfig.getAuthRequestSecret(), gigyaConfig.getAuthRequestTokenValidity());
		...
		try {
			return getRestTemplate().exchange(gigyaConfig.getAuthorizationUrl(), HttpMethod.POST, entity,
					GigyaAuthData.class);
		} catch (final RestClientException e) {
			// Fail gracefully in case of exception
			LOG.error("Exception in fetching authorizations from CDC for the user {} with Customer Data Cloud UID {} ",
					customer.getUid(), customer.getGyUID(), e);

			return null;
		}
	}

	protected HttpEntity<String> createHttpEntity(final CustomerModel customer, final HttpHeaders headers) {
	...
	private boolean validateFunctionalRolesAndActionsExist(final GigyaAssetsData assetData) {
		return assetData.getAssetTemplates().getCommerceFunctionalRoles() != null
				&& assetData.getAssetTemplates().getCommerceFunctionalRoles().getActions() != null
				&& CollectionUtils.isNotEmpty(
						assetData.getAssetTemplates().getCommerceFunctionalRoles().getActions().getAccessList());
	}
	...
	public void setGigyaAuthRequestConverter(
			final Converter<CustomerModel, GigyaAuthRequestData> gigyaAuthRequestConverter) {
