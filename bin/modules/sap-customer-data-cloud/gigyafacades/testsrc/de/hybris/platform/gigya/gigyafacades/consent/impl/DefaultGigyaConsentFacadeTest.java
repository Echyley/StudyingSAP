...
public class DefaultGigyaConsentFacadeTest {
	...
	public void testSynchronizeConsentsWhenBaseSiteDoesntExist() {
		...
		Mockito.lenient().when(gigyaConsentService.getConsentTemplates(null)).thenThrow(new IllegalArgumentException());
	...
	public void testSynchronizeConsentsWhenConsentTemplateForSitesDoNotExist() {
		Mockito.lenient().when(baseSiteService.getCurrentBaseSite()).thenReturn(baseSiteModel);

		Mockito.lenient().when(gigyaConsentService.getConsentTemplates(baseSiteModel)).thenReturn(null);
		...
		Mockito.verify(gigyaConsentService, Mockito.times(0)).withdrawConsent(Mockito.any(), Mockito.any(),
				Mockito.any());
		Mockito.verify(customerConsentDataStrategy).populateCustomerConsentDataInSession();
	}
	...
	public void testSynchronizeConsentsWhenConsentTemplateForSitesDoExistWithInvalidInfo() throws Exception {
		...
		Mockito.lenient().when(gigyaConsentService.getConsentTemplates(baseSiteModel)).thenReturn(consentTemplates);

		Mockito.lenient().when(firstTemplate.getId()).thenReturn("terms.generalTerms");
		Mockito.lenient().when(secondTemplate.getId()).thenReturn("terms.mandatory.electronics");
	...
	public void testSynchronizeConsentsWhenConsentTemplateForSitesDoExistWithValidInfoAndNoActiveConsent()
		...
		Mockito.lenient().when(firstTemplate.getId()).thenReturn("terms.generalTerms");
		Mockito.lenient().when(secondTemplate.getId()).thenReturn("terms.test");
		Mockito.lenient().when(thirdTemplate.getId()).thenReturn("terms.mandatory.electronics");

		Mockito.lenient().when(firstTemplate.getVersion()).thenReturn(0);
		Mockito.lenient().when(secondTemplate.getVersion()).thenReturn(1);
		Mockito.lenient().when(thirdTemplate.getVersion()).thenReturn(1);

		GSObject gsObject = new GSObject(SAMPLE_PREFERENCE_OBJ_VALID);

		Mockito.lenient().when(gigyaConsentService.getActiveConsent(customer, firstTemplate)).thenReturn(null);
		Mockito.lenient().when(gigyaConsentService.getActiveConsent(customer, secondTemplate)).thenReturn(secondConsent);
		Mockito.lenient().when(gigyaConsentService.getActiveConsent(customer, thirdTemplate)).thenReturn(thirdConsent);

		Mockito.lenient().when(thirdConsent.getConsentWithdrawnDate()).thenReturn(new Date());
	...
	/*
	 * Create a new entry for consent with an updated version Consent Templates with
	 * previous version exists in commerce
	 */
	@Test
	public void testSynchronizationConsentsWhenVersionOfConsentTemplateAndConsentDiffers() throws Exception {
		...
		consentTemplates.add(secondTemplate);
		Mockito.lenient().when(gigyaConsentService.getConsentTemplates(baseSiteModel)).thenReturn(consentTemplates);
		Mockito.lenient().when(firstTemplate.getId()).thenReturn("terms.mandatory.electronics");
		Mockito.lenient().when(secondTemplate.getId()).thenReturn("terms.test");
		Mockito.lenient().when(firstTemplate.getVersion()).thenReturn(1); // docVersion of terms.mandatory would be 1
		Mockito.lenient().when(secondTemplate.getVersion()).thenReturn(2); // docVersion of terms.test would be 1
