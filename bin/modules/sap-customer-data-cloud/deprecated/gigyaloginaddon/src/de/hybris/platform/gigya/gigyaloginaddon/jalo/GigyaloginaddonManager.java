...
/**
 * This is the extension manager of the Gigyaloginaddon extension.
 */
public class GigyaloginaddonManager extends GeneratedGigyaloginaddonManager
{
	/** Edit the local|project.properties to change logging behavior (properties 'log4j.*'). */
	private static final Logger LOG = LoggerFactory.getLogger(GigyaloginaddonManager.class.getName());
	 ...
	 */
	public static GigyaloginaddonManager getInstance()
	{
		return (GigyaloginaddonManager) Registry.getCurrentTenant().getJaloConnection().getExtensionManager().getExtension(
				GigyaloginaddonConstants.EXTENSIONNAME);
	}
	 ...
	 */
	public GigyaloginaddonManager() 
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("constructor of GigyaloginaddonManager called.");
		}
	}
	...
	public void init()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("init() of GigyaloginaddonManager called. " + getTenant().getTenantID());
		}
	}
	...
	public void destroy()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("destroy() of GigyaloginaddonManager called, current tenant: " + getTenant().getTenantID());
		}
	}
