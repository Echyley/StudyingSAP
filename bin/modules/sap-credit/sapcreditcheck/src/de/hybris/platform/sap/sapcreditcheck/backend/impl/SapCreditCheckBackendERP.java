...
public class SapCreditCheckBackendERP extends BackendBusinessObjectBaseJCo implements SapCreditCheckBackend
{
	...
	public boolean checkCreditLimitExceeded(final AbstractOrderData orderData, final String soldTo) throws BackendException
	{
		...
		if (("V1".equalsIgnoreCase(outputTable.getString("ID")) && "150".equalsIgnoreCase(outputTable.getString("NUMBER"))) ||
		    ("V1".equalsIgnoreCase(outputTable.getString("ID")) && "152".equalsIgnoreCase(outputTable.getString("NUMBER"))) )
		{
			/** 
			...
			**/
			creditExceeded = true;
			sapLogger.info("Credit limit check failed for order " + orderData.getCode() + "! " + outputTable.getString(SapcreditcheckConstants.ERROR_MESSAGE));
		}
		...
		{
			creditExceeded = true;
			sapLogger.error("Getting an error message while checking the credit limit for order " + orderData.getCode() + "! " + outputTable.getString(SapcreditcheckConstants.ERROR_MESSAGE));
		...
		{
			sapLogger.warn("Getting a warning message while checking the credit limit for order " + orderData.getCode() + "! " + outputTable.getString(SapcreditcheckConstants.ERROR_MESSAGE));
