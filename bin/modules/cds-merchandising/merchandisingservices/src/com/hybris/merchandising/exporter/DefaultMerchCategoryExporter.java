...
import com.hybris.merchandising.client.MerchCatalogServiceClient;
 ...
 *
 *@deprecated - see {@link DefaultMerchProductDirectoryCategoryExporter}.
 *
 */
...
public class DefaultMerchCategoryExporter extends AbstractJobPerformable<CronJobModel> implements MerchCategoryExporter {
	 ...
	 */
	@Override
	public void exportCategories(final MerchProductDirectoryConfigModel productDirectory) {
		//No op
	}
	...
	protected PerformResult perform(final InMemoryChangesCollector collector) {
		...
		{
			changes.forEach(itemChangeDto -> LOG.debug("Changed CategoryModel found - {} itemChangeDto info = {}", itemChangeDto.getItemPK(), itemChangeDto.getInfo()));
		}
		if(!changes.isEmpty()) {
