...
public interface MerchCatalogServiceProductDirectoryClient
{
	...
	@Control(retries = "${retries:1}", retriesInterval = "${retriesInterval:2000}", timeout = "${timeout:30000}")
	...
	@Control(retries = "${retries:1}", retriesInterval = "${retriesInterval:2000}", timeout = "${timeout:30000}")
	String handleProductsBatch(@PathParam("productDirectoryId") final String productDirectoryId,
	                           @PathParam("versionId") final String version, final List<Product> products);
	...
	@Control(retries = "${retries:2}", retriesInterval = "${retriesInterval:2000}", timeout = "${timeout:15000}")
	 ...
	 * @param productDirectoryId - the product directory ID we wish to associate the products with.
	 * @param version            the version to publish.
	 * @param context            additional info e.g. total number of products
	 * @return Void.
	 */
	@POST
	@Produces(MediaType.APPLICATION_JSON)
	...
	@Control(retries = "${retries:2}", retriesInterval = "${retriesInterval:2000}", timeout = "${timeout:15000}")
	Void publishProducts(@PathParam("productDirectoryId") final String productDirectoryId,
	                     @PathParam("versionId") final String version, final ProductPublishContext context);
	...
	@Control(retries = "${retries:2}", retriesInterval = "${retriesInterval:2000}", timeout = "${timeout:30000}")
	...
	@Path("/${tenant}/productdirectories/{productDirectoryId}")
	@Produces(MediaType.APPLICATION_JSON)
	@Control(retries = "${retries:2}", retriesInterval = "${retriesInterval:2000}", timeout = "${timeout:5000}")
