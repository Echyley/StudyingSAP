...
public class B2BCustomerExportServiceTest {
  ...
  @Mock
  private CustomerAddressReplicationUtilityService customerAddressReplicationUtilityService = mock(CustomerAddressReplicationUtilityService.class);
  ...
  public void testPrepareB2BContactData() throws InterceptorException {
    ...
    lenient().when(languageModel.getIsocode()).thenReturn(B2BCUSTOMER_SESSION_LANGUAGE);
    lenient().when(b2bCustomerModel.getSessionLanguage()).thenReturn(languageModel);
  ...
  protected void mockBaseCustomerData() {
    ...
    given(customerAddressReplicationUtilityService.findDefaultAddress(b2bCustomerModel, null)).willReturn(null);
  }
