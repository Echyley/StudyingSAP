...
{
	private static final Logger LOGGER = Logger.getLogger(com.sap.hybris.sapcustomerb2b.outbound.B2BCustomerExportService.class.getName());
	...
	public void prepareAndSend(final B2BCustomerModel changedB2bCustomerModel, final String language)
	{
		...
		if(!b2bCustomers.contains(changedB2bCustomerModel)){
			b2bCustomers.add(changedB2bCustomerModel);
		}
		...
		{
			final HashMap<String, Object> map = prepareB2BCustomerData(b2bCustomer, language);
			if(map != null){
				rawHybrisB2BCustomers.add(map);
			}
		}
	...
	protected void sendRawItemsToDataHub(final String rawItemType, final List<Map<String, Object>> rawData)
	{
		...
		{
			LOGGER.debug("No send to datahub occurred because target is empty");
		}
	}
	...
	protected HashMap<String, Object> prepareB2BCustomerData(final B2BCustomerModel b2bCustomerModel, final String sessionLanguage)
	{
		...
		if (parentB2BUnitUid != null)
		{
			...
			AddressModel defaultShipmentAddress = b2bCustomerModel.getDefaultShipmentAddress();

			if(defaultShipmentAddress == null){
				defaultShipmentAddress = getCustomerAddressReplicationUtilityService().findDefaultAddress(b2bCustomerModel, null);
			}
