...
public class DefaultSAPB2BUnitService extends DefaultB2BUnitService
{
	...
	public B2BUnitModel getParent(final B2BCustomerModel b2bCustomer)
	{
		...
		if (parentB2BUnit != null)
		{
			...
			if (salesAreaSuffix.isEmpty())
			{
				...
				{
					LOGGER.debug(
							"No SAP Base Store Configuration with sale area definition assigned to current Base Store! Returns sales area independent B2B Unit with UID: ["
									+ parentB2BUnitUID + "]");
				}
	...
	private B2BUnitModel getB2BUnitModel(B2BUnitModel parentB2BUnit, String parentB2BUnitUID, String salesAreaSuffix)
	{
		...
		if (Objects.nonNull(parentB2BUnitMembers))
		{
			final Optional<PrincipalModel> parentB2BUnitMember = parentB2BUnitMembers.stream().findFirst();
			if (parentB2BUnitMember.isPresent())
			{
				...
				return parentIsInstanceOfB2BUnitModel(parentB2BUnitMember.get(), salesAreaDependentB2BUnitUID);
			}
		}
		return null;
	}

	private B2BUnitModel parentIsInstanceOfB2BUnitModel(final PrincipalModel parentB2BUnitMember,
	                                                    final String salesAreaDependentB2BUnitUID)
	{
	...
	protected String getCommonDivision(final String salesOrganization, final String division)
	{
		...
		{
			LOGGER.error(String.format("Missing configuration for sales organization [%s] and division [%s]!", salesOrganization,
					division));
			LOGGER.error(
					"Check the item types SAPSalesOrganization, ReferenceDistributionChannelMapping, and ReferenceDivisionMapping for the missing sales organization and division!");
			return null;
		}
	}
