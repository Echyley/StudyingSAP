...
public class Y2C4CB2BTicketPopulatorTest
{
	...
	{
		Mockito.when(Boolean.valueOf(sitePropsHolder.isB2C())).thenReturn(Boolean.FALSE);
		...
		Assert.assertEquals(CUSTOMER_ID, serviceRequestData.getBuyerMainContactPartyID());
		Assert.assertEquals("less10", serviceRequestData.getBuyerPartyID());
	}
	...
	public void shouldPopulateWhenCustomerExternalIdEqualTo10Length()
	{
		...
		Mockito.verify(b2bUserFacade, Mockito.only()).getParentUnitForCustomer(Mockito.anyString());
		Assert.assertEquals(CUSTOMER_ID, serviceRequestData.getBuyerMainContactPartyID());
		Assert.assertEquals("0123456789", serviceRequestData.getBuyerPartyID());
	}
	...
	public void shouldPopulateWhenCustomerExternalIdMoreThan10Length()
	{
		...
		Mockito.verify(b2bUserFacade, Mockito.only()).getParentUnitForCustomer(Mockito.anyString());
		Assert.assertEquals(CUSTOMER_ID, serviceRequestData.getBuyerMainContactPartyID());
		Assert.assertEquals("MoreThan10", serviceRequestData.getBuyerPartyID());
	}
	...
	public void shouldNotPopulateB2C()
	{
		...
		Assert.assertNull(serviceRequestData.getBuyerPartyID());
	}
}
