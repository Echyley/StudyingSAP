...
/**
 * Default implementation of the CarOrderFacade interface to provide concrete business logic to access Order details and
 * history
 */
public class DefaultCarOrderFacade implements CarOrderFacade
{
	...
	public SearchPageData<CarOrderHistoryData> getPagedOrderHistoryForCustomer(final PageableData pageableData)
	{
		...
		final List<CarOrderHistoryData> orderList = getMultichannelOrderHistoryService()
				.readOrdersForCustomer(currentCustomer.getCustomerID(), paginationData);
	...
	public CarOrderHistoryData getOrderDetails(final String businessDayDate, final String storeId, final Integer transactionIndex)
	{
		...
		if (order == null)
		{
			throw new UnknownIdentifierException(
					"Order with orderGUID " + businessDayDate + "Order with orderGUID " + businessDayDate + "Order with orderGUID "
							+ businessDayDate + " not found for current user in current BaseStore");
		}
	...
	public CarMultichannelOrderHistoryData getSalesDocumentDetails(final String TransactionNumber)
	{
		...
		final CarMultichannelOrderHistoryData order = getMultichannelOrderHistoryService()
				.readSalesDocumentDetails(currentCustomer.getCustomerID(), TransactionNumber);

		if (order == null)
		{
			throw new UnknownIdentifierException(
					"Order with orderGUID " + TransactionNumber + " not found for current user in current BaseStore");
		}
