		...
		DefaultCarDataProviderServiceMockData implements MultichannelDataProviderService
{
	...
	public ODataFeed readMultiChannelTransactionsFeed(String customerNumber, PaginationData paginationData)
	{
		...
		try (final InputStream edmContent = getClass().getResourceAsStream("/test/services/serviceMetadata.xml"))
		{
			...
			try (final InputStream content = getClass().getResourceAsStream("/test/data/multichannelOrders.json"))
			{
		...
		{
			throw new RuntimeException("Error while reading MC sales Feed", e);
		}
	...
	public ODataFeed readSalesDocumentHeaderFeed(String customerNumber, String transactionNumber)
	{
		...
		try (final InputStream edmContent = getClass().getResourceAsStream("/test/services/serviceMetadata.xml"))
		{
			...
			try (final InputStream content = getClass().getResourceAsStream("/test/data/multichannelOrder.json"))
			{
				feed = EntityProvider.readFeed(SapcarintegrationConstants.APPLICATION_JSON,
						entityContainer.getEntitySet("MultiChannelPurchasesQueryResults"),
						content, EntityProviderReadProperties.init().build());
		...
		{
			throw new RuntimeException("Error while reading Multi Channel Sales Header Feed", e);
		}
	...
	public ODataFeed readSalesDocumentItemFeed(String customerNumber, String transactionNumber)
	{
		...
		try (final InputStream edmContent = getClass().getResourceAsStream("/test/services/serviceMetadata.xml"))
		{
			...
			try (final InputStream content = getClass().getResourceAsStream("/test/data/multichannelOrderItems.json"))
			{
				feed = EntityProvider.readFeed(SapcarintegrationConstants.APPLICATION_JSON,
						entityContainer.getEntitySet("MultiChannelPurchasesQueryResults"),
						content, EntityProviderReadProperties.init().build());
		...
		{
			throw new RuntimeException("Error while reading Multi Channel Sales Item Feed", e);
		}
