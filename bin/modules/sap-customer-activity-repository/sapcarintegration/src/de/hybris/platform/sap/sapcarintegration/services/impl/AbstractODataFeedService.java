...
public abstract class AbstractODataFeedService
{
	...
	{
		final HttpURLConnection connection = getCarConnectionService().createConnection(relativeUri, contentType, httpMethod);
	...
	protected ODataFeed readFeed(final String serviceUri, final String contentType, final String entitySetQueryName,
			final String entityResultSetName, final String select, final String filter, final String expand)
	{
		...
		try
		{
			...
			final String absoluteUri = createUri(serviceUri, entitySetQueryName, null, expand, select, filter);

			content = execute(absoluteUri, contentType, SapcarintegrationConstants.HTTP_METHOD_GET);

			ODataFeed oDataFeed = EntityProvider.readFeed(contentType, entityContainer.getEntitySet(entityResultSetName), content,
		...
		{
			throw new CarConnectionRuntimeException("error while reading feed url : " + serviceUri + e);
		}
		finally
		{
	...
	protected Edm readEdm(final String serviceUrl)
	{
		...
		{
			throw new CarConnectionRuntimeException("Error while reading service url : " + serviceUrl, e);
		}
		finally
		{
			if (content != null)
	...
	protected String createUri(final String serviceUri, final String entitySetName, final String id,
			final String expandRelationName, final String select, final String filter)
	{
		...
		{
			throw new CarConnectionRuntimeException("Error while building url : " + serviceUri, e);
		}
	...
	private static class UriBuilder
	{
		...
		{
			uri = new StringBuilder(serviceUri).append(SapcarintegrationConstants.SEPARATOR).append(entitySetName);
