 ...
 * {@link de.hybris.platform.sap.sapcarintegration.services.MultichannelOrderHistoryExtractorService}
 */
public class DefaultMultichannelOrderHistoryExtractorService extends DefaultCarOrderHistoryExtractorService
		implements MultichannelOrderHistoryExtractorService
{
	...
	@Override
	public List<CarMultichannelOrderHistoryData> extractMultichannelOrders(PaginationData paginationData, ODataFeed feed)
	{
	...
	protected CarMultichannelOrderHistoryData extractMultichannelOrder(final ODataEntry entry)
	{
		...
		if (props != null)
		{
			...
			order.setTransactionDate(
					new Date(DateUtil.formatDate(beginDateStr).getTime() + DateUtil.formatTime(beginTimeStr).getTime()));
			...
			order.setDistributionChannel(
					props.get("DistributionChannel") != null ? props.get("DistributionChannel").toString() : "");
			order.setOrganizationDivision(
					props.get("OrganizationDivision") != null ? props.get("OrganizationDivision").toString() : "");

			// extract document type
			order.setSalesDocumentType(props.get("SalesDocumentType") != null ? props.get("SalesDocumentType").toString() : "");

			// process status
			order.setOverallOrderProcessStatus(
					props.get("OverallOrderProcessStatus") != null ? props.get("OverallOrderProcessStatus").toString() : "");
			order.setOverallOrderProcessStatusDesc(
					props.get("OverallOrderProcessStatusDesc") != null ? props.get("OverallOrderProcessStatusDesc").toString() : "");
			...
			order.setTotalPriceWithTax(createPrice(order.getSubTotal().getValue().add(order.getTotalTax().getValue()),
					order.getSubTotal().getCurrencyIso()));
		}
	...
	@Override
	public CarMultichannelOrderHistoryData extractSalesDocumentHeader(ODataFeed feed)
	{
		final List<ODataEntry> entries = feed.getEntries();
	...
	@Override
	public void extractSalesDocumentEntries(CarMultichannelOrderHistoryData order, ODataFeed feed)
	{
	...
	protected CarOrderEntryData extractOrderEntry(final ODataEntry odataEntry, final int entryNumber)
	{
		...
		if (props != null)
		{
			...
			final BigDecimal quantity = props.get("OrderQuantity") != null ? new BigDecimal(props.get("OrderQuantity").toString())
					: BigDecimal.ONE;
			...
			orderEntry
					.setTotalPriceWithTax(createPriceWithTax(netAmount.getValue(), taxAmount.getValue(), netAmount.getCurrencyIso()));
	...
	{
		BigDecimal taxAmount = new BigDecimal(props.get("TaxAmount") != null ? props.get("TaxAmount").toString() : "0");
		String currency = props.get("TransactionCurrency") != null ? props.get("TransactionCurrency").toString() : "";
	 ...
	 * Current Store Config ---> Hybris web Order Otherwise its an SD ORder
	 * 
	 * @param order
	 * @return
	 */
	protected OrderChannelTypeEnum determineOrderChannelType(CarMultichannelOrderHistoryData order)
	{
		...
		else if (order.getSalesOrganization() != null && order.getDistributionChannel() != null
