 ...
 * {@link de.hybris.platform.sap.sapcarintegration.services.MultichannelOrderHistoryService}
 */
public class DefaultMultichannelOrderHistoryService extends DefaultCarOrderHistoryService
		implements MultichannelOrderHistoryService
{
	...
	@Override
	public CarMultichannelOrderHistoryData readSalesDocumentDetails(String customerNumber, String transactionNumber)
	{

		ODataFeed headerFeed = getMultichannelDataProviderService().readSalesDocumentHeaderFeed(customerNumber, transactionNumber);
		...
		ODataFeed itemFeed = getMultichannelDataProviderService().readSalesDocumentItemFeed(customerNumber, transactionNumber);
		// read and extract item data
		getMultichannelOrderHistoryExtractorService().extractSalesDocumentEntries(order, itemFeed);
		return order;
	}

	@Override
	public List<CarMultichannelOrderHistoryData> readMultiChannelTransactionsForCustomer(String customerNumber,
			PaginationData paginationData)
	{
		//
		ODataFeed feed = getMultichannelDataProviderService().readMultiChannelTransactionsFeed(customerNumber, paginationData);

		// extract trancations
		return getMultichannelOrderHistoryExtractorService().extractMultichannelOrders(paginationData, feed);
	}
