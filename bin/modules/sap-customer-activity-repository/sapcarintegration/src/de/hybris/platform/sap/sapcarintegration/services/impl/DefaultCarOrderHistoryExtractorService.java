 ...
 * {@link de.hybris.platform.sap.sapcarintegration.services.CarOrderHistoryExtractorService}
 */
public class DefaultCarOrderHistoryExtractorService extends AbstractCarOrderHistoryBaseExtractor
		implements CarOrderHistoryExtractorService
{
	...
	{
		BigDecimal taxExcludedAmount = new BigDecimal(
				props.get("TaxExcludedAmount") != null ? props.get("TaxExcludedAmount").toString() : "0");
		BigDecimal distributedHeaderTaxAmount = new BigDecimal(
				props.get("DistributedHeaderTaxAmount") != null ? props.get("DistributedHeaderTaxAmount").toString() : "0");
		String currency = props.get("TransactionCurrency") != null ? props.get("TransactionCurrency").toString() : "";
	...
	protected CarOrderHistoryData extractOrder(final ODataEntry entry)
	{
		...
		if (props != null)
		{
			...
			order.setTransactionDate(
					DateUtil.formatDateTime(props.get("BeginTimestamp") != null ? props.get("BeginTimestamp").toString() : ""));

			order.setTransactionIndex(
					props.get("TransactionIndex") != null ? Integer.valueOf(props.get("TransactionIndex").toString())
							: Integer.valueOf(0));
	...
	protected CarOrderEntryData extractOrderEntry(final ODataEntry odataEntry, final int entryNumber)
	{
		...
		if (props != null)
		{
			...
			final BigDecimal quantity = props.get("SalesQuantityInSalesUnit") != null
					? new BigDecimal(props.get("SalesQuantityInSalesUnit").toString())
					: BigDecimal.ONE;
