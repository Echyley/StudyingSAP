...
public class DefaultSalesOrderSimulationService implements SalesOrderSimulationService {
	...
	protected List<SalesOrderSimulateItemRequestData> getItemLevelDetails(ItemModel itemModel) {
		...
		if (itemModel instanceof ProductModel) {
			String plant = (((ProductModel) itemModel).getSapPlant()!=null)?((ProductModel) itemModel).getSapPlant().getCode():"";
			SalesOrderSimulateItemRequestData salesOrderSimulateItemData = getSalesOrderSimulateItemRequestData("",
					DEFAULT_ITEM_NUMBER, ((ProductModel) itemModel).getCode(), QUANTITY,plant);
			...
			for (AbstractOrderEntryModel abstractOderEntryModel : orderModel.getEntries()) {
				String plant = (abstractOderEntryModel.getProduct().getSapPlant()!=null)?abstractOderEntryModel.getProduct().getSapPlant().getCode():"";
				SalesOrderSimulateItemRequestData orderItemData = getSalesOrderSimulateItemRequestData(
						...
						abstractOderEntryModel.getProduct().getCode(), abstractOderEntryModel.getQuantity() + "",plant);
				orderItemData.setPricingElements(new ArrayList<>()); 
				
				if(isAllowedProductType(abstractOderEntryModel.getProduct())) {
	...
	protected boolean isAllowedProductType(ProductModel productModel) {
		return productModel.getSapProductTypes().contains(SAPProductType.PHYSICAL) ? true : false;
	}
	...
	protected List<SalesOrderSimulateItemRequestData> getItemLevelDetailsWithBulkQuantity(ItemModel itemModel) {
		...
		if (itemModel instanceof ProductModel) {
			String plant = (((ProductModel) itemModel).getSapPlant()!=null)?((ProductModel) itemModel).getSapPlant().getCode():"";
			SalesOrderSimulateItemRequestData salesOrderSimulateItemData = getSalesOrderSimulateItemRequestData("",
					DEFAULT_ITEM_NUMBER, ((ProductModel) itemModel).getCode(), BULK_QUANTITY,plant);
			items.add(salesOrderSimulateItemData);
		} 
		return items;
	}
	...
	protected SalesOrderSimulateItemRequestData getSalesOrderSimulateItemRequestData(String salesOrder, String itemNumber,
		...
		salesOrderSimulateItemData.setPlant(plant);
	...
	public Map<String, List<PriceInformation>> getPriceDetailsForProducts(List<ProductModel> productModels) {
		...
		for (ProductModel productModel : productModels) {
			String plant = (productModel.getSapPlant()!=null)?productModel.getSapPlant().getCode():"";
			SalesOrderSimulateItemRequestData salesOrderSimulateItemData = getSalesOrderSimulateItemRequestData("",
					(salesOrderItemNumber++) * ORDER_ENTRY_MULTIPLIER + "", productModel.getCode(), QUANTITY,plant);
			items.add(salesOrderSimulateItemData);
		}
		requestData.setItems(items);
	...
	protected void setOrderModel(ItemModel itemModel, Map<String, Object> itemInfoMap ,double deliveryCost, double paymentCost) {
		...
		for (AbstractOrderEntryModel orderEntry : ((AbstractOrderModel) itemModel).getEntries()) {
			final int orderEntryNumber = (orderEntry.getEntryNumber() + 1) * ORDER_ENTRY_MULTIPLIER;
				...
				if (itemModel instanceof OrderModel && orderEntry.getSapPricingConditions().isEmpty() && (sapPricingConditions != null)) {
	...
	private String getAmount(SalesOrderSimulateItemData item, ProceduresSubtotal subtotal) {
	    ...
	    switch (subtotal.getCode()) {
	        case NETAMOUNT:
	            return item.getNetAmount() != null ? item.getNetAmount() : "0";
	        case TAXAMOUNT:
	            return item.getTaxAmount() != null ? item.getTaxAmount() : "0";
	        case COSTAMOUNT:
	            return item.getCostAmount() != null ? item.getCostAmount() : "0";
	        case SUBTOTAL1AMOUNT:
	            return item.getSubtotal1Amount() != null ? item.getSubtotal1Amount() : "0";
	        case SUBTOTAL2AMOUNT:
	            return item.getSubtotal2Amount() != null ? item.getSubtotal2Amount() : "0";
	        case SUBTOTAL3AMOUNT:
	            return item.getSubtotal3Amount() != null ? item.getSubtotal3Amount() : "0";
	        case SUBTOTAL4AMOUNT:
	            return item.getSubtotal4Amount() != null ? item.getSubtotal4Amount() : "0";
	        case SUBTOTAL5AMOUNT:
	            return item.getSubtotal5Amount() != null ? item.getSubtotal5Amount() : "0";
	        case SUBTOTAL6AMOUNT:
	            return item.getSubtotal6Amount() != null ? item.getSubtotal6Amount() : "0";
	        default:
	            return "";
	    }
	}
	...
	public SapProductAvailability getStockAvailability(ProductModel productModel, BaseStoreModel baseStore) {
		...
		return stockLevelMap.get(plant);
