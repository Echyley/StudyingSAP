...
public class DefaultPricingService implements PricingService {
	...
	private CacheAccess sapSalesPricingCacheRegion;
	...
	public List<PriceInformation> getPriceForProduct(ProductModel product) {
		...
		if (isPriceCacheEnabled) {
			priceInformation = (List<PriceInformation>) getSapSalesPricingCacheRegion().get(getPriceCacheKey(product));
		}
		...
		if(priceInformation==null || priceInformation.isEmpty() ) {
			...
			try {
				if (isPriceCacheEnabled) {
					getSapSalesPricingCacheRegion().put(getPriceCacheKey(product), priceInformation);
				}
	...
	public CacheAccess getSapSalesPricingCacheRegion() {
		return sapSalesPricingCacheRegion;
	}

	public void setSapSalesPricingCacheRegion(CacheAccess sapSalesPricingCacheRegion) {
		this.sapSalesPricingCacheRegion = sapSalesPricingCacheRegion;
	}
