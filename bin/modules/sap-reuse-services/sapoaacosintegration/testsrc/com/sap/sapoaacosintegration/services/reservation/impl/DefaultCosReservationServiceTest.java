...
public class DefaultCosReservationServiceTest
{
	...
	public void setUp()
	{
		...
		defaultCosReservationService.setCommonUtils(commonUtils);
		...
		Mockito.lenient().when(request.getMethodType()).thenReturn(HttpMethod.PATCH);
		when(response.getBody()).thenReturn(listCosReservationResponse);
		Mockito.lenient().doNothing().when(reservationResultHandler).updateReservation(any(AbstractOrderModel.class),
	...
	public void deleteReservationItemTest_MultipleItem()
	{
		...
		abstractOrderModel.setEntries(listAbstractOrderEntryModel);
		Mockito.lenient().doReturn(response).when(restOperations).exchange(anyString(), eq(HttpMethod.DELETE),
