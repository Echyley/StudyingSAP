...
public class DefaultCosATPServiceTest
{
	...
	public void setUp()
	{
		...
		defaultCosATPService.setCommonUtils(commonUtils);


		final ResponseEntity<List<ArticleResponse>> response = new ResponseEntity<>(HttpStatus.OK);
		...
		Mockito.lenient().when(cosAtpResultHandler.extractATPAvailabilityFromArticleResponse(eq(listArticleResponse)))
		...
		//UnitModel
		unitModel.setCode("PCE");

		//ProductModel
		productModel.setCode("CODE");
		productModel.setUnit(unitModel);

		//when(productModel.getUnit().getCode()).thenReturn("PC");
	...
	public void callRestAvailabilityServiceForProductAndSourcesTest()
	{
		...
		um.setCode("UnitModelCode");
		productModel.setUnit(um);
		//Execute
