...
public class DefaultATPService extends DefaultCosATPService {
	...
	private List<ATPProductAvailability> exchangeRestTemplateAndExtractATPBatchResult(final String resourceServicePath,
		...
		try {
			ResponseEntity<ATPBatchResponse> response = getOaaRestService().getRestTemplate().exchange(finalUri, HttpMethod.GET, entity,
					ATPBatchResponse.class);
			batchResult = atpResultHandler.extractATPProductAvailabilityFromATPBatchResponse(Optional
					.ofNullable(response)
					.map(ResponseEntity<ATPBatchResponse>::getBody)
	...
	private List<ATPAvailability> exchangeRestTemplateAndExtractATPResult(final String resourceServicePath,
		...
		try {
			ResponseEntity<ATPResponse> response = getOaaRestService().getRestTemplate().exchange(finalUri, HttpMethod.GET, entity,
					ATPResponse.class);
			atpResult = atpResultHandler.extractATPAvailabilityFromATPResponse(Optional
					.ofNullable(response)
					.map(ResponseEntity<ATPResponse>::getBody)
					.orElseThrow(RuntimeException::new));
		} catch (final HttpClientErrorException e) {
			getOaaRestService().checkHttpStatusCode(e);
			LOG.error(ATP_ERROR_MESSAGE);
