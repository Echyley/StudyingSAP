...
public class DefaultReservationStrategy implements ReservationStrategy {
	 ...
	 * (non-Javadoc)
	 *
	 * @see com.sap.sapoaacarintegration.services.reservation.strategy.
	 * ReservationStrategy#updateReservation(de.hybris.platform
	 ...
	 *
	 * @see com.sap.sapoaacarintegration.services.reservation.strategy.
	 * ReservationStrategy#deleteReservation(de.hybris.platform
	...
	public boolean deleteReservation(final AbstractOrderModel abstractOrderModel) {
		if (getCommonUtils().isCAREnabled()) {
			...
			// In the default implementation it will return true - even when the CAR is not
			// responding
		}
		return true;
	}
	 ...
	 *
	 * @see com.sap.sapoaacarintegration.services.reservation.strategy.
	 * ReservationStrategy#deleteReservationItem(de.hybris.
	...
	public boolean deleteReservationItem(final AbstractOrderModel abstractOrderModel,
		...
		if (getCommonUtils().isCAREnabled()) {
			...
			if (!isOrderValidForDeletion(abstractOrderModel)
					|| !isOrderEntryValidForDeletion(abstractOrderEntryModel)) {
				LOG.info("No valid order entry found for reservation deletion");
				return true;
			}

			LOG.info("Delete Reservation Item: " + abstractOrderEntryModel.getEntryNumber().toString()
					+ " for Reservation: " + abstractOrderModel.getGuid());
	...
	protected boolean isOrderValidForDeletion(final AbstractOrderModel abstractOrderModel) {
		...
		if ((abstractOrderModel != null && StringUtils.isNotEmpty(abstractOrderModel.getGuid()))
				&& (abstractOrderModel.getSapBackendReservation() != null
						&& Boolean.TRUE.equals(abstractOrderModel.getSapBackendReservation()))) {
			isValid = false;
		}
