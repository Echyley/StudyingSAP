...
public class DefaultSourcingResultHandler implements SourcingResultHandler
{
	...
	public boolean hasSourcingResult(final SourcingResponse sourcing) {
		...
		return Optional.ofNullable(sourcing.getSourcingResults().getSourcingResults().get(0).getCartItems())
				.map(v -> Optional.ofNullable(v.getSourcingResultCartItems()).isPresent())
				.isPresent();
	}
