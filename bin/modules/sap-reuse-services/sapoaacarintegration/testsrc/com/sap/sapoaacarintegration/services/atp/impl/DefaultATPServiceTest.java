...
public class DefaultATPServiceTest
{
	...
	private DefaultATPService classUnderTest;
	private ServiceUtils serviceUtils;
	private DefaultATPResourcePathBuilder atpResourcePathBuilderMock;
	private RestServiceConfiguration restServiceConfiguration;
	private DefaultRestService oaaRestService = new DefaultRestService();

	@Before
	public void setup() throws SourcingException, URISyntaxException
	{
		//classUnderTest = EasyMock.createNiceMock(DefaultATPService.class);
		
		classUnderTest = new DefaultATPService();
		...
		oaaRestService.setRestServiceConfiguration(restServiceConfiguration);
		classUnderTest.setOaaRestService(oaaRestService);
		Assert.assertNotNull(classUnderTest.getOaaRestService());
		
		oaaRestService.initialize();
		
		//Mock ServiceUtils
		serviceUtils = EasyMock.createNiceMock(ServiceUtils.class);
		EasyMock.replay(serviceUtils);
		
		//Mock ResourcePathBuilder
		atpResourcePathBuilderMock = EasyMock.createNiceMock(DefaultATPResourcePathBuilder.class);
		EasyMock.expect(atpResourcePathBuilderMock.getServiceUtils()).andReturn(serviceUtils);
		EasyMock.replay(atpResourcePathBuilderMock);
		classUnderTest.setAtpResourcePathBuilder(atpResourcePathBuilderMock);
		...
		oaaRestService.setRestTemplate(restTemplateMock);
		Assert.assertNotNull(oaaRestService.getRestTemplate());
		...
		oaaRestService.setSessionService(sessionService);
	...
	public void callRestAvailabilityServiceForProduct() throws ATPException, URISyntaxException
	{
		...
		List<ATPAvailability> actATPResult =null;
		final ProductModel productModel = new ProductModel();
		productModel.setCode(ARTICLE_1);
		...
		actATPResult = classUnderTest.callRestAvailabilityServiceForProduct(CART_GUID, ITEM_ID_1,
				productModel);
	...
	public void callRestAvailabilityServiceForProductAndSource() throws ATPException, URISyntaxException
	{
		...
		// Call rest service method
		actATPResult = classUnderTest.callRestAvailabilityServiceForProductAndSource(CART_GUID,
