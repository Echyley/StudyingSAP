...
public class PricingBackendPPSTest {
	...
	private static final Logger LOG=Logger.getLogger(PricingBackendPPSTest.class);
	@Mock
	...
	@Test
	public void testSetGetI18NService() {
	...
	public void testReadPricesForCart() {
		...
		Mockito.when(ppsClientMock.callPPS(Mockito.any(PriceCalculate.class), Mockito.eq(sapConfig)))
				.thenReturn(response);
	...
	private void prepareData(final AbstractOrderModel order) {
		...
		Mockito.when(
				helperMock.createCalculateRequestSkeleton(Mockito.eq("Store1"), Mockito.any(GregorianCalendar.class)))
				.thenReturn(priceCalculate);
		final LineItemDomainSpecific lineItem = helperImpl.createSaleLineItem(0, product.getCode(), "ST",
				new BigDecimal(14));

		Mockito.when(helperMock.createSaleLineItem(Mockito.eq(0), Mockito.eq(product.getCode()), Mockito.eq("ST"),
				Mockito.any(BigDecimal.class))).thenReturn(lineItem);
	}

	@Test(expected = RuntimeException.class)
	public void testReadPricesForCartFails() {
		setMocks();
		final AbstractOrderModel order = new AbstractOrderModel();
		createOrderWithOneEntry(order);
		...
		Mockito.when(ppsClientMock.callPPS(Mockito.any(PriceCalculate.class), Mockito.eq(sapConfig)))
				.thenThrow(new RestClientException("bla"));
		cut.readPricesForCart(order);
		assertTrue(false);
	}
	...
	public void testReadPriceInformationForProductsPriceInfoNull() {
		...
		final ExtendedAmountType extendedAmountType = new ExtendedAmountType();
		extendedAmountType.setValue(new BigDecimal(99999.99));
		saleBase.setExtendedAmount(extendedAmountType);
		lineItem.setSale(saleBase);
		shoppingBasket.getLineItem().add(lineItem);

		body.setShoppingBasket(shoppingBasket);
		response.addPriceCalculateBodyItem(body);
		Mockito.when(ppsClientMock.callPPS(Mockito.any(PriceCalculate.class), Mockito.eq(sapConfig)))
				.thenReturn(response);
	...
	@Test
	public void testSetGetHighPrice() {
		cut.setHighPrice(0);
	...
	@Test
	public void testReadPriceInformationForProductsForInvalidProductsCase1() {
		final List<ProductModel> productModels = new ArrayList<>();
		productModels.add(product);
		setHelper();
		setMocks();
		Mockito.when(ppsClientMock.callPPS(Mockito.any(PriceCalculate.class), Mockito.eq(sapConfig))).thenReturn(null);
		Mockito.when(commonI18NServiceMock.getCurrentCurrency()).thenReturn(currency);
		final List<PriceInformation> result = cut.readPriceInformationForProducts(productModels, true);
	...
	@Test
	public void testReadPriceInformationForProductsForInvalidProductsCase2() {
		final List<ProductModel> productModels = new ArrayList<>();
	...
	@Test
	public void testReadPriceInformationForProductsForInvalidProductsCase3() {
		final List<ProductModel> productModels = new ArrayList<>();
	...
	@Test
	public void testReadPriceInformationForProductsForInvalidProductsCase4() {
		final List<ProductModel> productModels = new ArrayList<>();
