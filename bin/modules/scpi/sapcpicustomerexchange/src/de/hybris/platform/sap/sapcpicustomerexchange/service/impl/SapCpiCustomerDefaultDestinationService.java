 ...
 */
public class SapCpiCustomerDefaultDestinationService implements SapCpiCustomerDestinationService {
  ...
  public SAPLogicalSystemModel readSapLogicalSystem() {
    ...
    Objects.requireNonNull(logicalSystems, "No logical system is maintained in back-office for B2C customer replication to SCPI!");

    return isMdmEnabled()
            ? logicalSystems.stream().filter(ls -> ls.getSapLogicalSystemName().contentEquals(MDM_LOGICAL_SYSTEM))
            .findFirst()
            .orElseThrow(() -> new IllegalArgumentException("No logical system named MDM is maintained in back-office for B2C customer replication to MDM through SCPI!"))
            : logicalSystems.stream().filter(ls -> ls.isDefaultLogicalSystem())
            .findFirst()
            .orElseThrow(() -> new IllegalArgumentException("No logical system is maintained in back-office for B2C customer replication to SCPI!"));
  ...
  public String extractSapClient(SAPLogicalSystemModel sapLogicalSystem) {
    ...
    if (sapClientSplit.length == TARGET_URL_PIECES && StringUtils.isNotBlank(sapClientSplit[1]) && sapClientSplit[1].strip().matches("\\d\\d\\d")) {
      ...
      throw new IllegalArgumentException(String.format("The HTTP destination [%s] does not maintain sap-client properly for B2C customer IDoc destination!", targetURL));
    }
