...
public class KymaEventEmitTaskRunnerUnitTest extends HybrisJUnit4Test
{
    ...
    public void setUp() throws Exception
    {
        ...
        final ConsumedDestinationModel consumedDestinationModel = mock(ConsumedDestinationModel.class);
        ...
        doReturn(consumedDestinationModel).when(destinationService).getDestinationByIdAndByDestinationTargetId(any(), any());
        ...
        final JsonPublishRequestData convertedData = new JsonPublishRequestData();
    ...
    public void eventMessageSendsSuccessfullyOnLastTry()
    {
		...
		final Integer throwNumber = maxConsecutiveRetries - 1;
        ...
        assertTrue(listener.getEvents().stream().map(e -> (String) e.getMessage())
				.anyMatch(o -> o.contains(String.format("Can not send event to the %s, event type", getTargetName()))));
        assertTrue(listener.getEvents().stream().map(e -> (String) e.getMessage())
				.anyMatch(o -> o.contains(String.format("Starting consecutive retries to send event to the %s, event type",
						getTargetName()))));
        assertTrue(listener.getEvents().stream().map(e -> (String) e.getMessage())
