...
class CartTests extends BaseWSTest {
	...
	void testGetCartJSON() {
		def con = testUtil.getSecureConnection("/cart", 'GET', 'JSON', HttpURLConnection.HTTP_OK)
	...
	@Test
	void testGetCartXML() {
		def con = testUtil.getSecureConnection("/cart", 'GET', 'XML', HttpURLConnection.HTTP_OK)
	...
	void testGetCartAfterCurrencyChangeXML() {
		...
		con = testUtil.getSecureConnection('/cart', 'GET', 'XML', HttpURLConnection.HTTP_OK, null, cookieNoPath)
		...
		con = testUtil.getSecureConnection('/cart?curr=JPY', 'GET', 'XML', HttpURLConnection.HTTP_OK, null, cookieNoPath)

		body = con.inputStream.getText('UTF-8')
		println body
		testUtil.verifyXML(body)
		response = new XmlSlurper().parseText(body)
		...
		con = testUtil.getSecureConnection('/cart?curr=USD', 'GET', 'XML', HttpURLConnection.HTTP_OK, null, cookieNoPath)
	...
	void testAddPickupItemToCartWithWrongStoreNameFail() {
		...
		assert response.error.message == "Store with the name 'WrongStoreName' doesn't exist"
	}
	...
	void testUpdateCartEntry() {
		...
		// update cart entry
		con = testUtil.getSecureConnection('/cart/entry/0?qty=3', 'PUT', 'XML', HttpURLConnection.HTTP_OK, null, cookieNoPath)
		body = con.inputStream.text
		println body
		testUtil.verifyXML(body)
		response = new XmlSlurper().parseText(body)
	...
	void testRemoveCartEntry() {
		...
		// remove cart entry
		con = testUtil.getSecureConnection('/cart/entry/1', 'DELETE', 'JSON', HttpURLConnection.HTTP_OK, null, cookieNoPath)
		response = testUtil.verifiedJSONSlurper(con, false, false)
		assert response.statusCode == 'success'
	...
	void testPickupInStoreMode() {
		...
		con = testUtil.getSecureConnection('/cart/entry/0/store?storeName=WS-Nakano', 'PUT', 'XML', HttpURLConnection.HTTP_OK, null, cookieNoPath)
		...
		con = testUtil.getSecureConnection('/cart/entry/0/store', 'DELETE', 'XML', HttpURLConnection.HTTP_OK, null, cookieNoPath)
		body = con.inputStream.text
		testUtil.verifyXML(body)
		response = new XmlSlurper().parseText(body)
		assert response.name() == 'cartModification'
	...
	void testPickupInStoreWithWrongStoreName() {
		...
		// set cart entry as pickup in store
		con = testUtil.getSecureConnection('/cart/entry/0/store?storeName=wrongStoreName', 'PUT', 'XML', HttpURLConnection.HTTP_BAD_REQUEST, null, cookieNoPath)
		def error = con.errorStream.text;
		response = new XmlSlurper().parseText(error)
		assert response.error.type == 'ValidationError'
		assert response.error.subject == 'storeName'
	...
	void testPickupInStoreWithWrongEntryNumber() {
		...
		// set cart entry as pickup in store
		con = testUtil.getSecureConnection('/cart/entry/1/store?storeName=WS-Nakano', 'PUT', 'XML', HttpURLConnection.HTTP_BAD_REQUEST, null, cookieNoPath)
		def error = con.errorStream.text
		response = new XmlSlurper().parseText(error)
	...
	void testDeletePickupInStoreForWrongEntry() {
		...
		con = testUtil.getSecureConnection('/cart/entry/0/store', 'DELETE', 'XML', HttpURLConnection.HTTP_BAD_REQUEST, null, cookieNoPath)
	...
	void testDeletePickupInStoreWithWrongEntryNumber() {
		...
		con = testUtil.getSecureConnection('/cart/entry/1/store', 'DELETE', 'XML', HttpURLConnection.HTTP_BAD_REQUEST, null, cookieNoPath)
	...
	void testFailPickupInStoreWhenOutOfStock() {
		...
		con = testUtil.getSecureConnection("/cart/entry/0/store?storeName=${STORE_NAME}", 'PUT', 'XML', HttpURLConnection.HTTP_BAD_REQUEST, null, cookieNoPath, null)
		def error = con.errorStream.text;
		response = new XmlSlurper().parseText(error);
		assert response.error.type == 'InsufficientStockError'
		assert response.error.subjectType == 'entry'
	...
	void testEnableOrderPromotionByCustomer() {
		...
		def customerTests = new CustomerTests()
		def uid = customerTests.registerUser()
		def access_token = testUtil.getAccessToken(uid, customerTests.password, config.TRUSTED_CLIENT_ID, config.TRUSTED_CLIENT_SECRET)
	...
	void testEnableOrderPromotionByCustomerWithNonTrustedClientShouldFail() {
		...
		def customerTests = new CustomerTests()
		def uid = customerTests.registerUser()
		def access_token = testUtil.getAccessToken(uid, customerTests.password, config.CLIENT_ID, config.CLIENT_SECRET)
	...
	void testCartCreatePaymentInfo() {
		...
		postBody = "accountHolderName=Sven+Haiges&cardNumber=4111111111111111&cardType=visa&expiryMonth=01&expiryYear=2113&saved=true&defaultPaymentInfo=true&billingAddress.titleCode=mr&billingAddress.firstName=sven&billingAddress.lastName=haiges&billingAddress.line1=test1&billingAddress.line2=test2&billingAddress.postalCode=12345&billingAddress.town=somecity&billingAddress.country.isocode=US&billingAddress.region.isocode=US-NY"
		...
		assert response.paymentInfo.billingAddress.country.name == 'United States'
		assert response.paymentInfo.billingAddress.country.isocode == 'US'
		assert response.paymentInfo.billingAddress.region.isocode == 'US-NY'
		assert response.paymentInfo.billingAddress.postalCode == '12345'
		assert response.paymentInfo.billingAddress.email == uid
		assert response.paymentInfo.billingAddress.formattedAddress == 'test1, test2, New York, somecity, 12345'
	...
	void testCreatePaymentInfoWithoutCart() {
		...
		postBody = "accountHolderName=Sven+Haiges&cardNumber=4111111111111111&cardType=visa&expiryMonth=01&expiryYear=2114&saved=true&defaultPaymentInfo=true&billingAddress.titleCode=mr&billingAddress.firstName=sven&billingAddress.lastName=haiges&billingAddress.line1=test1&billingAddress.line2=test2&billingAddress.postcode=12345&billingAddress.town=somecity&billingAddress.country.isocode=US&billingAddress.region.isocode=US-NY"
	...
	void testAuthorizedValidPlaceOrder() {
		...
		// create a paymentinfo for this cart
		postBody = "accountHolderName=Sven+Haiges&cardNumber=4111111111111111&cardType=visa&expiryMonth=01&expiryYear=2114&saved=true&defaultPaymentInfo=true&billingAddress.titleCode=mr&billingAddress.firstName=sven&billingAddress.lastName=haiges&billingAddress.line1=test1&billingAddress.line2=test2&billingAddress.postalCode=12345&billingAddress.town=somecity&billingAddress.country.isocode=US&billingAddress.region.isocode=US-NY"
		con = testUtil.getSecureConnection("/cart/paymentinfo", 'POST', 'JSON', HttpURLConnection.HTTP_OK, postBody, cookieNoPath, access_token)
		response = testUtil.verifiedJSONSlurper(con, false, false);
	...
	void testPlaceOrderWithProductOutOfStock() {
		...
		try {
			setStockStatus(InStockStatus.FORCEOUTOFSTOCK, productId)
			...
			assert response.errors[0].subject == productId
			assert response.errors[0].message == "Product [${productId}] is currently out of stock"
		}
		finally {
			setStockStatus(InStockStatus.NOTSPECIFIED, productId)
		}
	}
	 ...
	 */
	void setStockStatus(InStockStatus status, String productId) {
		...
		stockService.setInStockStatus(product, warehouseService.getDefWarehouse(), status)
	}
	...
	void testApplyVoucherForCartByCustomer() {
		...
		def con = testUtil.getSecureConnection('/cart', 'GET', 'JSON', HttpURLConnection.HTTP_OK, null, cookieNoPath)
	...
	void testApplyRestrictedVoucherForCartByCustomer() {
		...
		def con = testUtil.getSecureConnection('/cart', 'GET', 'JSON', HttpURLConnection.HTTP_OK, null, cookieNoPath, access_token)
		def response = testUtil.verifiedJSONSlurper(con, false, false);
		assert response.totalItems == 1
		assert response.totalUnitCount == 2
	...
	def addToCart(code, qty = 1, cookie = null, access_token = null, storeName = null) {
		...
		if (access_token != null)
			...
			con = testUtil.getSecureConnection('/cart/entry', 'POST', 'XML', HttpURLConnection.HTTP_OK, postBody, cookie)
