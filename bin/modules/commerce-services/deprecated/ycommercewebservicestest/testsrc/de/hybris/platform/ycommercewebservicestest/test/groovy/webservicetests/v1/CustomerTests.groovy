...
import static de.hybris.platform.ycommercewebservicestest.constants.YcommercewebservicestestConstants.ADMIN_PASSWORD_PROPERTY
import static de.hybris.platform.ycommercewebservicestest.constants.YcommercewebservicestestConstants.ADMIN_USERNAME
...
class CustomerTests extends BaseWSTest {
	...
	static final public password = ('0'..'z').shuffled().take(10).join()
	...
	void testDeletePaymentInfo() {
		...
		assert response.creditCardPaymentInfo.size() == 1
		assert response.creditCardPaymentInfo[0].saved == 'false'
	}
	...
	void testDeletePaymentInfoJSON() {
		...
		assert response.paymentInfos[0].saved == false
	}
	...
	public void testGetAllCustomerGroupsWhenUserIsNotCustomer() {
		def access_token = testUtil.getAccessToken(ADMIN_USERNAME, getConfigurationProperty(ADMIN_PASSWORD_PROPERTY))
		def con = testUtil.getSecureConnection("/customers/current/customergroups", "GET", "JSON",
				HttpURLConnection.HTTP_UNAUTHORIZED, null, null, access_token);
	}
	...
	void testRestorePasswordEmptyLoginXML() {
		...
		def response = new XmlSlurper().parseText(error)
		assert response.error[0].type == 'IllegalArgumentError'
		assert response.error[0].message == 'The field [id] cannot be empty'
	}
	...
	void testRestorePasswordEmptyLoginJSON() {
		...
		def response = new JsonSlurper().parseText(error)
		assert response.errors[0].type == 'IllegalArgumentError'
		assert response.errors[0].message == 'The field [id] cannot be empty'
	}
	...
	void testRestorePasswordNonExistingLoginXML() {
		...
		// response OK, because forgottenpassword is asynchronous business process
		def response = testUtil.verifiedXMLSlurper(con)
		assert response.success == true
	}
