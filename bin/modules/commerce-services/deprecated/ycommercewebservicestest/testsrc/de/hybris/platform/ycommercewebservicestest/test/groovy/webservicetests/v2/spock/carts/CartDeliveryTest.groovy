...
class CartDeliveryTest extends AbstractCartTest {
	...
	def "Customer tries to set delivery address for a cart when response: #format and a B2B base-site is used"() {
		given: "a customer with cart and an address"
		def val = createAndAuthorizeCustomerWithCart(restClient, format, getBasePathWithB2BSite())
		def customer = val[0]
		def cart = val[1]
		def address = createAddress(restClient, customer, JSON, getBasePathWithB2BSite())

		when: "a customer tries to set delivery address"
		HttpResponseDecorator response = restClient.put(
				path: getBasePathWithB2BSite() + '/users/' + customer.id + '/carts/' + cart.code + '/addresses/delivery',
				query: ['addressId': address.id],
				contentType: format,
				requestContentType: URLENC
		)
	...
	def "Customer create delivery address for a cart when request: #requestFormat and response: #responseFormat"() {
		...
		with(response) {
			...
			data.phone == '11123234545'
			data.cellphone == '11123234546'
			data.town == 'Muenchen'
	...
	def "Customer create delivery address [with no titleCode] for a cart when request: #requestFormat and response: #responseFormat"() {
		given: "anonymous customer with cart, product in the cart and email"
		authorizeClient(restClient)
		def anonymous = ['id': 'anonymous']
		def userGuid = System.currentTimeMillis() + '@test.com'
		...
		where:
		requestFormat | responseFormat | postBody
		JSON          | JSON           | GOOD_ADDRESS_DE_JSON_NO_TITLECODE
		XML           | XML            | GOOD_ADDRESS_DE_XML_NO_TITLECODE
	}
