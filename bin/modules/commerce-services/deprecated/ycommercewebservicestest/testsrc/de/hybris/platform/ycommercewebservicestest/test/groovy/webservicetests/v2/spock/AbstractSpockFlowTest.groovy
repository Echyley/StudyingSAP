...
abstract class AbstractSpockFlowTest extends AbstractSpockTest {
	...
	protected static final String CUSTOMER_PASSWORD = ('0'..'9').shuffled().take(4).join();
	protected static final String CUSTOMER_PASSWORD_STRONG =  ('0'..'9').shuffled().take(4).join() + ('A'..'Z').shuffled().take(2).join() + ('a'..'z').shuffled().take(4).join() + "!";
	...
	def createAddressWithOptions(Map addressOptions = [:], RESTClient client, user) {
		...
		HttpResponseDecorator response = client.post(
				path: options.basePathWithSite + '/users/' + user.id + '/addresses',
				body: bodyContent,
