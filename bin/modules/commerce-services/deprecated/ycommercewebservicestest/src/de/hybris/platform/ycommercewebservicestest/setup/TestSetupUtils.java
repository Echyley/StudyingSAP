...
public class TestSetupUtils
{
	...
	private static final String SETUP_IMPEX_SERVICE_NAME = "setupImpexService";
	private static final String EMBEDDED_SERVER_CONTROLLER = "embeddedServerController";
	...
	private static void setupCommerce(final BiConsumer<YCommerceWebServicesTestSetup, SystemSetupContext> triggerEqualsValidator)
	{
		final YCommerceWebServicesTestSetup setup = getCommerceWebServicesTestSetup();
	...
	private static YCommerceWebServicesTestSetup getCommerceWebServicesTestSetup()
	{
		return Registry.getApplicationContext().getBean("yCommerceWebServicesTestSetup", YCommerceWebServicesTestSetup.class);
	}
	...
	private static SystemSetupContext createSystemSetupContext()
	{
		...
		{ "Go" });
		params.put("ycommercewebservicestest_sample", new String[]
		{ "true" });
		...
		return new SystemSetupContext(params, Type.ALL, Process.ALL, "ycommercewebservicestest");
	}
	...
	{
		final EmbeddedServerController controller = Registry.getApplicationContext().getBean(EMBEDDED_SERVER_CONTROLLER,
	...
	public static void updateCaptchaCheckEnabled(final boolean enable)
	{
		final SetupImpexService setupImpexService = Registry.getApplicationContext().getBean(SETUP_IMPEX_SERVICE_NAME,
				SetupImpexService.class);
		if (enable)
		{
			setupImpexService.importImpexFile("/ycommercewebservicestest/import/captcha/captchaEnabled.impex", false);
		}
		else
		{
			setupImpexService.importImpexFile("/ycommercewebservicestest/import/captcha/captchaDisabled.impex", false);
		}
	}

	public static void updateSapInvoiceEnabled(final boolean enable)
	{
		final SetupImpexService setupImpexService = Registry.getApplicationContext().getBean(SETUP_IMPEX_SERVICE_NAME,
				SetupImpexService.class);
		if (enable)
		{
			setupImpexService.importImpexFile("/ycommercewebservicestest/import/invoice/invoiceEnabled.impex", false);
		}
		else
		{
			setupImpexService.importImpexFile("/ycommercewebservicestest/import/invoice/invoiceDisabled.impex", false);
		}
	}

	public static void updateCartRerievalDateCheckEnabled(final boolean enable)
	{
		final SetupImpexService setupImpexService = Registry.getApplicationContext().getBean(SETUP_IMPEX_SERVICE_NAME,
				SetupImpexService.class);
		if (enable)
		{
			setupImpexService.importImpexFile("/ycommercewebservicestests/import/cartretrievaldate/cartRetrievalDateEnabled.impex",
					false);
		}
		else
		{
			setupImpexService.importImpexFile("/ycommercewebservicestests/import/cartretrievaldate/cartRetrievalDateDisabled.impex",
					false);
		}
	}

	public static boolean isEndpointDisabled(final String restrictedEndPointKey, final String restrictedEndpoint)
	{
		boolean disabled = false;
		final String disabledEndpoints = Config.getParameter(restrictedEndPointKey);
		if (StringUtils.isNotEmpty(disabledEndpoints))
		{
			final String[] disabledEndpointsArray = disabledEndpoints.split(",");
			disabled = Arrays.stream(disabledEndpointsArray).anyMatch(restrictedEndpoint::equals);
		}
		return disabled;
	}
}
