...
public class DefaultProductFacade<REF_TARGET> implements ProductFacade
{
	...
	/**
	 * @deprecated Since 6.0. Use {@link DefaultProductFacade#getProductForCodeAndOptions(String, Collection)} instead.
	 */
	...
	@Override
	@Nonnull
	public List<ConfigurationInfoData> getConfiguratorSettingsForCode(@Nonnull final String code)
	{
		final ProductModel productModel = getProductService().getProductForCode(code);
		return getConfiguratorSettingsService().getConfiguratorSettingsForProduct(productModel).stream()
				.map(cfg -> getConfigurationHandlerFactory().handlerOf(cfg.getConfiguratorType()).createProductInfo(cfg))
				.flatMap(Collection::stream).map(getProductConfigurationConverter()::convert).flatMap(Collection::stream)
				.collect(Collectors.<ConfigurationInfoData> toList());
	}
