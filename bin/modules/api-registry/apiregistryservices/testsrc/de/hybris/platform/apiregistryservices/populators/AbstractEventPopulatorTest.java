...
public class AbstractEventPopulatorTest extends ServicelayerTransactionalTest
{
	...
	public void getValuesFromEvent() throws Exception
	{
		...
		assertEquals("not correct size of result data", 2, valuesFromEvent.size());
		assertTrue("no totalPrice", valuesFromEvent.keySet().contains(ORDER_TOTAL_PRICE_KEY));
		assertEquals("totalPrice not correct", 0,
				ORDER_TOTAL_PRICE.compareTo((Double) valuesFromEvent.get(ORDER_TOTAL_PRICE_KEY)));
		assertTrue("no orderCode", valuesFromEvent.keySet().contains(ORDER_CODE_KEY));
		assertEquals("orderCode not correct", ORDER_CODE, valuesFromEvent.get(ORDER_CODE_KEY));
	}
	...
	public void testProcessEventConfiguration()
	{
		...
		assertEquals("not correct size of result data", 1, valuesFromEvent.size());
		assertTrue("no process code", valuesFromEvent.keySet().contains(PROCESS_CODE_KEY));
		assertEquals("process code not correct", PROCESS_CODE, valuesFromEvent.get(PROCESS_CODE_KEY));
	}
