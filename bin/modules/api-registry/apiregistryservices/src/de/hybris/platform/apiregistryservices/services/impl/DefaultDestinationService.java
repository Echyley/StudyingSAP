...
public class DefaultDestinationService<T extends AbstractDestinationModel> implements DestinationService
{
	...
	@Override
	public List<T> getAllDestinationsByCredentialId(final String credentialId)
	{
		return getDestinationDao().findAllDestinationsByCredentialId(credentialId);
	}

	@Override
	public Set<AbstractCredentialModel> getDeletableCredentialsByDestinationTargetId(final String destinationTargetId)
	{
		final var destinations = getDestinationsByDestinationTargetId(destinationTargetId);

		return destinations.stream()
		                   .filter(destination -> isCredentialDeletable(destination, destination.getCredential()))
		                   .map(AbstractDestinationModel::getCredential)
		                   .collect(Collectors.toSet());
	}

	private boolean isCredentialDeletable(final AbstractDestinationModel destination, final AbstractCredentialModel credential)
	{
		return Objects.nonNull(credential) && areAllDestinationsBelongingToDestinationTarget(
				getAllDestinationsByCredentialId(credential.getId()), destination.getDestinationTarget());
	}

	private boolean areAllDestinationsBelongingToDestinationTarget(final List<T> destinations,
			final DestinationTargetModel destinationTarget)
	{
		return destinations.stream().allMatch(des -> des.getDestinationTarget().equals(destinationTarget));
	}
}
