...
{
	private static final String DTM_STRING = "}={DTM:";
	private static final String CREDENTIAL_DESTINATION_STRING = " as credential ON {destination:";
	private static final String CREDENTIAL_STRING = "}={credential:";
	...
	protected static final String GET_ALL_NONTEMPLATE_D_QUERY = SELECT_PK_FROM + AbstractDestinationModel._TYPECODE
			+ " AS ADM JOIN "
			+ DestinationTargetModel._TYPECODE + " AS DTM ON {ADM:" + AbstractDestinationModel.DESTINATIONTARGET + DTM_STRING
	...
	protected static final String GET_ALL_EXPOSED_BY_CLIENT_QUERY = SELECT_STRING + AbstractDestinationModel.PK + "} " + "from {"
			+ AbstractDestinationModel._TYPECODE + " as destination " + "JOIN " + ExposedOAuthCredentialModel._TYPECODE
			+ CREDENTIAL_DESTINATION_STRING + AbstractDestinationModel.CREDENTIAL + CREDENTIAL_STRING
			...
			+ DestinationTargetModel._TYPECODE + " AS DTM ON {destination:" + AbstractDestinationModel.DESTINATIONTARGET + DTM_STRING
	...
	protected static final String GET_ACTIVE_EXPOSED_DESTINATIONS_BY_TARGET = SELECT_PK_FROM + ExposedDestinationModel._TYPECODE + " AS EDM JOIN "
			+ DestinationTargetModel._TYPECODE + " AS DTM ON {EDM:" + AbstractDestinationModel.DESTINATIONTARGET + DTM_STRING
	...
	protected static final String GET_DESTINATIONS_BY_ID_AND_TARGET= GET_ALL_D_BY_TARGET + " AND {d:"+ AbstractCredentialModel.ID
			+ "}=?"+ ID_PARAMETER;
	protected static final String GET_ALL_EXPOSED_DESTINATIONS_BY_CREDENTIAL_ID= SELECT_STRING+ExposedDestinationModel.PK+"} from {"
	...
	private static final String GET_ALL_DESTINATIONS_BY_CREDENTIAL_ID = SELECT_STRING + AbstractDestinationModel.PK + "} from {"
			+ AbstractDestinationModel._TYPECODE + " as destination JOIN "
			+ AbstractCredentialModel._TYPECODE + CREDENTIAL_DESTINATION_STRING + AbstractDestinationModel.CREDENTIAL
			+ CREDENTIAL_STRING + AbstractCredentialModel.PK + "}} WHERE {credential:" + AbstractCredentialModel.ID + "}=?"
			+ ID_PARAMETER;
	...
	@Override
	public List<T> findAllDestinationsByCredentialId(final String credentialId)
	{
		final FlexibleSearchQuery query = new FlexibleSearchQuery(GET_ALL_DESTINATIONS_BY_CREDENTIAL_ID);
		query.addQueryParameter(ID_PARAMETER, credentialId);
		final SearchResult<T> result = getFlexibleSearchService().search(query);
		return result.getResult();
	}
}
