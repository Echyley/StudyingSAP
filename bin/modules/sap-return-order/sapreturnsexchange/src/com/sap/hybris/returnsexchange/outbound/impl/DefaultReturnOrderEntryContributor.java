...
public class DefaultReturnOrderEntryContributor implements RawItemContributor<ReturnRequestModel>
{
	...
	protected Map<String, Object> createReturnEntryRow(final ReturnEntryModel returnEntry)
	{
		...
		Set<SapReturnOrderReasonModel> sapReturnReasons=returnEntry.getOrderEntry().getOrder().getStore().getSAPConfiguration().getSapReturnReasons();
		if (returnEntry instanceof RefundEntryModel)
		{
			final RefundEntryModel refundEntryModel = (RefundEntryModel) returnEntry;
			row.put(ReturnOrderEntryCsvColumns.RETURN_REASON_FOR_ENTRY, getReturnReasonForEntry(refundEntryModel.getReason(),sapReturnReasons));
		}
	...
	private String getReturnReasonForEntry(RefundReason refundReason, Set<SapReturnOrderReasonModel> sapReturnReasons) {
		if(refundReason != null) {
			for (SapReturnOrderReasonModel sapReturnReason: sapReturnReasons) {
				if(refundReason.equals(sapReturnReason.getRefundReason())) {
					return sapReturnReason.getSapReturnReasonCode();
				}
		    }
		}
		return null;
