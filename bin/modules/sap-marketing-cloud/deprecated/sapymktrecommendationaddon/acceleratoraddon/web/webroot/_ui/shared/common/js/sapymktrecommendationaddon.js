...
(function(global) {
    ...
    const sapProdRecoInitialized="sap-prod-reco-initialized";
    const dataSmarteditComponentType ='[id^="reco"][data-smartedit-component-type="CMSSAPOfferRecoComponent"]';
    const sapOfferRecoInitialized="sap-offer-reco-initialized";
    ...
    function retrieveProductRecommendations(id, sProductCode, sComponentId) {
        ...
        $.get(ajaxUrl, {
            ...
            if (!bIsInSmartEditMode() && oElement.data(sapProdRecoInitialized) === true) {
    ...
    function loadProductRecommendations() {
        ...
        for (var i = 0; i < divs.length; i++) {
            if (divs[i].id.search("reco") > -1 && ($(divs[i]).data(sapProdRecoInitialized) !== true)) {
                ...
                $(divs[i]).data(sapProdRecoInitialized, true);
            }
        }
    }
    ...
    function loadOfferRecommendations() {
        var divs = $(dataSmarteditComponentType);

        for (var i = 0; i < divs.length && ($(divs[i]).data(sapOfferRecoInitialized) !== true); i++) {
            var sComponentId = $("#" + divs[i].id).data("componentid");
            retrieveOfferRecommendations(divs[i].id, sComponentId);
            $(divs[i]).data(sapOfferRecoInitialized, true);
        }
    }
    ...
    function retrieveOfferRecommendations(id, sComponentId) {
        ...
        $.get(ajaxUrl, {
            ...
            // we need to check if we are NOT in smartEdit mode && if the
            // component has been initialized.
            if (!bIsInSmartEditMode() && oElement.data(sapOfferRecoInitialized) === true) {
    ...
    function attachOnClickOfferClickInteraction() {
        ...
        var sBaseUrl = $(dataSmarteditComponentType).data("base-url");
    ...
    function postOfferClickInteraction(sBaseUrl, sOfferId, sOfferContentId, sComponentId) {
        ...
        var baseUrl = $(dataSmarteditComponentType).data("base-url");
