...
public class SapCpiOmsOrderOutboundAction extends SapOmsSendOrderToDataHubAction {
  ...
  public void executeAction(OrderProcessModel process) throws RetryLaterException {
    ...
    final List<OrderHistoryEntryModel> orderHistoryList = new CopyOnWriteArrayList<>();
    final List<SAPOrderModel> sapOrders = new CopyOnWriteArrayList<>();
    final Set<Set<ConsignmentModel>> orderConsignmentsSets = groupOrderConsignments(order);
  ...
  protected Single<ResponseEntity<Map>> sendOrderToScpi(OrderModel order, Set<ConsignmentModel> consignments,
    ...
    Optional<ConsignmentModel> optionalConsignmentModel = consignments.stream().findFirst();
    String plantCode = "";
  ...
  private class ResponseEntitySubscriber extends Subscriber<ResponseEntity<Map>> {
    ...
    public void onError(Throwable error) {
      ...
      LOG.error(String.format("The OMS order [%s] has not been sent to the SAP backend through SCPI! %n%s", order.getCode(), error.getMessage()), error);
